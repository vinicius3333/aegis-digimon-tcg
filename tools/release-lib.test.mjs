...
import { bumpVersion, cardChangesForPaths, parseConventionalCommit, renderWebReleaseData, requiredBump } from "./release-lib.mjs";

test("parses conventional commits and identifies breaking changes", () => {
  assert.deepEqual(parseConventionalCommit("feat(ui): add changelog", "BREAKING CHANGE: removes the old release endpoint"), {
    type: "feat",
...
test("uses the largest semantic-version impact", () => {
  assert.equal(requiredBump([{ type: "fix", breaking: false }, { type: "feat", breaking: false }]), "minor");
...
test("renders optional release-note translations for the web client", () => {
  const output = renderWebReleaseData("1.0.0", [{
    version: "1.0.0",
    date: "2026-08-09",
    changes: [{ type: "added", description: "Initial production release.", translations: { "pt-BR": "Lançamento inicial de produção." } }],
  }]);
...
test("builds release notes only from unique changed card IDs", () => {
  assert.deepEqual(cardChangesForPaths([
