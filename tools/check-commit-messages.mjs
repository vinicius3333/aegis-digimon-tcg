...
const invalid = rawCommits.split("\x1e").filter(Boolean).map((entry) => entry.split("\x1f")).filter(([subject, body]) => !parseConventionalCommit(subject, body)).map(([subject]) => subject);

if (invalid.length) {
  throw new Error(`Use Conventional Commits. Invalid commit subject(s):\n${invalid.map((subject) => `- ${subject}`).join("\n")}`);
}
