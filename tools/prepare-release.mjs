...
const latestTag = git(["tag", "--merged", "HEAD", "--sort=-v:refname"])
  .split("\n")
...
if (
  requestedBump &&
  ["patch", "minor", "major"].indexOf(requestedBump) < ["patch", "minor", "major"].indexOf(detectedBump)
) {
...
console.log(
  `Prepared v${version} (${bump}) from ${commits.length} conventional commit(s). Review the generated release PR before merging.`,
);
