...
function mechanicQueries(card) {
  const text = [card.effectText, card.inheritedEffectText, card.securityEffectText, card.optionEffect]
    ...
    .join(" ");
  const candidates = [
    [/digivolve/i, "digivolution requirements cost reduction breeding area"],
    [/security|Recovery/i, "security effects recovery processing order"],
    [/attack|Alliance|Piercing|Blocker|Reboot|Retaliation|Jamming/i, "attack battle keyword timing"],
    [/delete|DP|leave the battle area/i, "DP reduction deletion leave prevention timing"],
    [/trash|face-down|under any of your.*Tamer/i, "face-down cards under Tamers stacking trash visibility"],
