...
Copy to the session scratchpad, replace `<SET>`, `<BASE-SHA>` and the
exemplar list, and point every lane at it.

---

# <SET> re-audit worker brief (card-only lane)
...
- Rulings: `node tools/kb/query.mjs card <ID>` and
  ...
  is authoritative. `docs/audits/<SET>-reaudit/KB-INDEX.md` lists the Q&A ids
  you must cover.
- Module and test: `apps/api/src/cards/<SET>/<ID>.ts` and `<ID>.test.ts`.
- Context: `docs/audits/<SET>-reaudit/REVIEW-NOTES.md` and `RUN.md` sections
...
Only `<ID>.ts`, `<ID>.test.ts` and `docs/audits/<SET>-reaudit/<ID>.md`.
...
Logs go to `docs/audits/<SET>-reaudit/logs/<ID>-<what>.log`.
