...
Run only after the active unrelated Vitest PID/PGIDs 82901 and 97051 have both cleared and the coordinator authorizes the gate. PID group 43774 references in the audit ledger are historical evidence for the original embargo. Use one explicit file per process with `--pool=forks --poolOptions.forks.singleFork=true --fileParallelism=false`; do not run these in parallel.

## Focused reconciliation

The coordinator has confirmed the focused EX6 queue through EX6-074 complete.
...
2. Obtain an explicit coordinator decision gate; focused completion and cleared
   external processes never authorize collection execution by themselves.
3. Run exactly one serial collection process only after that gate:
...
5. Only reproducible collection evidence plus every card's 10/10 assessment
   permits the required Orca worktree completion status and coordinator report.
...
6. Historical source-audit grouping retained for traceability; the focused queue
   is closed by coordinator reconciliation, with collection still gated above.
