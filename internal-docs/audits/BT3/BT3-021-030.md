...
- `apps/api/src/engine/effects/interpreter/registration/module.ts:323-341`
  ...
  authority and all ten registrations are checked for the required shape.
...
- `apps/api/src/engine/effects/interpreter/actions/play.ts` and the
  `playFromSecurity` path recognize a self `PlayWithoutCost` action during
  Security timing, allowing BT3-024's concise self-target IR to play the
  revealed card without paying its cost.
...
- Implementation: the snapshot was already `{ effects: [], coverage: "full",
  ...
  `apps/api/src/cards/BT3/BT3-022.ts` with the empty full-coverage IR and only
  `registerIrCard`, plus the set-index import.
- Proof: `BT3-022.test.ts` now imports its direct module, checks play at 5000
  DP/cost 3 with no effect activation, and checks a legal Blue Digi-Egg
...
- Status: the missing module-registration seam was corrected; no printed clause
  or residual ambiguity remains.
...
- Implementation: `BT3-024.ts` uses Security timing, a self target, and
  `PlayWithoutCost` with `payCost: false`, full coverage, and no residual. The
  traced Security interpreter recognizes the self card while it is still in
  security and relocates it before the normal security-card extraction.
- Proof: `BT3-024.test.ts` covers Airdramon winning against weaker BT1-010,
  ...
  (Q1063). Q1061's normal-battle-area identity is asserted by the field
  permanent result. BT2-079 and BT2-050 were checked as multi-check/security
  peers.
- Status: Security timing, self identity, free play, loss/win branches, and
  multi-check ordering are mapped. No trait-filtered behavior is printed or
  implied, and no direct behavior correction was required.
...
- Implementation: the snapshot was already `{ effects: [], coverage: "full",
  residual: [] }`, but the direct module was missing. Added
  `apps/api/src/cards/BT3/BT3-028.ts` with the empty full-coverage IR and only
  `registerIrCard`, plus the set-index import.
- Proof: `BT3-028.test.ts` now imports the direct module, checks play at 8000
  DP/cost 7 with no effect activation, and checks legal Blue Lv.4 BT2-024
...
- Added the missing full empty IR modules
  `apps/api/src/cards/BT3/BT3-022.ts` and `apps/api/src/cards/BT3/BT3-028.ts`.
- Added BT3-022 and BT3-028 to `apps/api/src/cards/BT3/index.ts`, and imported
  their modules from their focused tests so their direct registration is part
  of the proof path.
...
- No generated snapshot edits were needed: the direct IR records match the
  committed snapshot, and the direct modules remain the executable authority.
