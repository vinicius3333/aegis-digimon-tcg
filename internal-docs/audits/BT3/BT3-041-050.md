...
- `apps/api/src/engine/security/securityCheck.ts:161-355` resolves one security
  ...
  performs any battle, and then continues to the next check. This proves the
  BT3-049 timing and the analogous BT3-041 security-stack flow.
...
- KB: no card-specific entry. General Digimon, Digi-Egg, and evolution rules
  apply.
- Implementation: the snapshot was already `{ effects: [], coverage: "full",
  residual: [] }`, but the direct module was missing. Added
  `BT3-044.ts` with the empty full-coverage IR and only `registerIrCard`, plus
  its BT3 set-index import.
- Proof: `BT3-044.test.ts` now imports its direct module, checks play at 5000
  DP/cost 2 with no effect activation, and checks legal Green Digi-Egg
  `BT3-004` in the breeding area evolving into Aruraumon with the source still
  in the stack and no card effect.
- Status: the missing module-registration seam was corrected; no printed
  behavior or trait-filter ambiguity remains.
...
- Implementation: the snapshot was already `{ effects: [], coverage: "full",
  residual: [] }`, but the direct module was missing. Added
  `BT3-045.ts` with the empty full-coverage IR and only `registerIrCard`, plus
  its BT3 set-index import.
- Proof: `BT3-045.test.ts` now imports its direct module, checks play at 4000
  DP/cost 3 with no effect activation, and checks legal Green Digi-Egg
...
- Implementation: `BT3-049.ts` uses Security timing, a self target, and
  `PlayWithoutCost` with `payCost: false`, full coverage, and no residual. The
  security interpreter locates the revealed card while it remains in the stack,
  places it in the battle area, and avoids trashing the relocated instance.
- Proof: `BT3-049.test.ts` covers losing to stronger BT1-057 (Q1083), winning
  ...
  has still been processed. BT3-024 was compared as the same Security
  self-play mechanism.
- Status: Security timing, self identity, free play, both battle outcomes,
  normal field identity, and multi-check ordering are statically mapped. No
...
- Added the missing full empty IR modules
  `apps/api/src/cards/BT3/BT3-044.ts` and `apps/api/src/cards/BT3/BT3-045.ts`.
- Added BT3-044 and BT3-045 to `apps/api/src/cards/BT3/index.ts`, and imported
  both direct modules from their focused tests.
...
- No shared engine seam required a behavior change, and no generated snapshot
  edit was needed. All audited card modules use only `registerIrCard`.
