...
only `registerIrCard`. The generated snapshot at line 103588 was stale: it
had no action scaling and instead carried a controller-local scaling object at
the SubTrigger level. It now matches the direct action-level
`deletedByTrigger` trace. The old source filter also had an erroneous
`controllerDefault: "mine"` scope, which would ignore an opponent's deleted
Digimon; it now retains only `excludeSelf` and Digimon kind. `scaleFactor` reads the complete
`ctx.trigger.deletedPermanentIds` batch, while the deletion bus and combat
...
IR assertions that the errata scaling is attached to GainMemory and the
watcher has no controller-side restriction; its behavioral case deletes one
...
**Score and ambiguity.** `2/2 + 2/2 + 2/2 + 2/2 + 0/2 = 8/10`.
The generated snapshot drift is corrected; no unresolved implementation
...
snapshot at line 103775 was stale: it used a `GrantAuraToOpponents` shell with
effect text rather than the implemented GrantStatic grant. It now matches the
direct trace. `runGrantStaticAction` routes this grant to the shared
...
103830 now includes the same `upTo` flag; coverage is full and residual empty.
...
- Corrected BT3-092's generated snapshot to carry errata `deletedByTrigger`
  scaling on the GainMemory action, matching the direct module and complete
  simultaneous deletion batch semantics.
- Corrected BT3-092's direct module and generated snapshot to remove the
  erroneous own-controller source filter, allowing “another Digimon” on either
  side while retaining source exclusion.
- Added BT3-092 focused IR proof for the errata scaling while retaining the
  behavioral own/opponent simultaneous-deletion and Piercing proof.
- Corrected BT3-097's generated snapshot from the obsolete
  `GrantAuraToOpponents` shell to the implemented
  `GrantStatic(noSecurityOptionEffects)` action.
...
No unresolved card-behavior ambiguity remains after the direct and snapshot
corrections.
...
The direct/snapshot/proof corrections and this report are delivered in focused atomic
commits on the prepared feature branch. The branch is pushed with
`git push -u origin HEAD` without force; the coordinator completion message
