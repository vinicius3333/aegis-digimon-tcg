...
All ten direct modules register exactly once through `registerIrCard(cardId, compiled)` and contain no `registerCard` registration. Every direct module reports `coverage: "full"` with an empty residual list. No direct card behavior correction was necessary in this range; five focused test files were corrected or strengthened to remove proof ambiguity. The report and test edits are scoped to this range; `packages/shared/src/effects/effects.json` and the coordinator-owned `BT6-STATIC-AUDIT.md` were not edited.
...
The catalog record (`cards.json:58948-58970`) is Green Lv.4, 5000 DP, with Green Lv.3 evolution for 2. Its text allows this card from hand to digivolve onto one of the player’s Green Tamers as if the Tamer were a level 3 Digimon. The direct module (`BT6-049.ts:9-42`) registers a Static metadata-only `Digivolve` action onto an own Green Tamer, `asLevel: 3`, `from: "hand"`, and explicitly carries `digivolutionRequirement` with `baseIsTamer: true` and `baseColors: ["Green"]`.
...
The catalog record (`cards.json:58973-58995`) is Green Lv.4, 7000 DP, with Green Lv.3 evolution for 3. Its text has the same Green-Tamer-as-level-3 path as Arbormon and intrinsic Piercing. The direct module (`BT6-050.ts:9-54`) registers the Green-Tamer Static metadata-only Digivolve action with `asLevel: 3`, an explicit Green `digivolutionRequirement`, and Piercing both as intrinsic keyword metadata and a self-targeted permanent `GainKeyword` action. The two representations resolve to the same intrinsic Piercing capability: keyword projection is set-based and the combat seam reads Piercing as a boolean capability.
...
Direct behavior correction count: **0**. Focused-proof corrections: **5 test files**.
