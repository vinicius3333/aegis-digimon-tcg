...
`packages/shared/src/effects/effects.json` and the coordinator-owned
`BT6-STATIC-AUDIT.md` were not edited. No tests, typecheck, lint, formatting,
...
remain queued, while the other in-progress ranges still require coordinator
integration. This worker makes no collection-completion or 10/10 claim.
