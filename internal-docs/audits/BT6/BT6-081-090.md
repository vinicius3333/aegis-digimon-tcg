...
All assigned card modules have one exclusive `registerIrCard(cardId, compiled)` call and no duplicate legacy registration. Snapshot regeneration was deferred at worker handoff and was later completed by the coordinator; direct registered modules remain the authoring authority.

## Worker-handoff gates, ambiguity, and remaining queue

No focused card tests, same-mechanism tests, collection tests, typecheck, lint, formatting, browser/UI validation, or `git diff --check` were run by this worker. Those deferred execution gates account for the archival `0/2` rows. The coordinator later ran the bounded gates and records the resulting 10/10 collection closeout in `docs/audits/BT6-STATIC-AUDIT.md`.
...
At worker handoff, BT6-091 onward remained outside this dispatch's assigned range. The coordinator integration pass subsequently completed that queue, snapshot synchronization, execution gates, and final collection scoring.
