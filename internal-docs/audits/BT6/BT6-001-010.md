...
Audit mode: static-only, per coordinator dispatch  
...
Deferred by explicit instruction: focused card tests, same-mechanism tests, mechanism tests, collection tests, full test suite, typecheck, lint, formatting, browser/UI checks, and `git diff --check`. These must be run by the coordinator or a later gate-enabled pass.
