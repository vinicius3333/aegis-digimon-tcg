...
**Direct, snapshot, and tests.** The direct module uses RevealAdd count 3, one add target with a Knightmon name filter OR a DeadlyAxemon name filter, and bottom-deck remainder. The generated snapshot is structurally exact. The focused test proves the Knightmon branch and the deck remainder; the DeadlyAxemon OR branch was inspected statically and against the catalog's exact card name. Reveal primitive behavior prevents one revealed instance from being counted twice. No proven implementation gap or ambiguity was found.
