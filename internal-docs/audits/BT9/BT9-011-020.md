...
- Direct module: `BT9-011.ts` contains one inherited `YourTurn` `CostModifier`, the alternate `[Growlmon]` requirement, `coverage: "full"`, and an empty residual. It registers only through `registerIrCard("BT9-011", compiled)`.
...
- Rules and primitives: §§2-3-1.2/.3 and 2-3-2 distinguish exact card-name and trait matching. The `GrantCanAttackUnsuspended` primitive grants the permission to the self target, and `withSubTriggerTurnScope`/turn ownership enforce the Your Turn duration.
- Direct module: `BT9-013.ts` maps Blitz and the Your Turn permission separately, with a self target and exact `nameExact` checks for OmniShoutmon/X Antibody in the stack. It is full, residual-free, has the alternate requirement, and registers only through `registerIrCard("BT9-013", compiled)`.
...
- Snapshot: generated data matches the direct structured IR and has no card-specific drift requiring correction.
...
Deferred gates are: the ten focused card test files, shared mechanism/interpreter tests, collection tests, typecheck, lint, formatting, browser/UI checks, and `git diff --check`. They must be run by a later coordinator/validation pass before any final 10/10 claim.
