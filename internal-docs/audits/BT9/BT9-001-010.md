...
`1000`. The alternate exact-name Guilmon route is cost 0; registration at line
53 is exclusive. `DeletionMaxDpLedger` and the removal action were traced to
verify numeric-only ceiling behavior and source-scoped current stack use.
