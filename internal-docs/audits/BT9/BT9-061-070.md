...
Raijinmon available and one-memory scaling (`:41–64`).
...
Total: **2 corrected cards, 8 corrected direct card fields, and 4 shared
interpreter/type additions**. Focused proof was strengthened in
`BT9-066.test.ts` and `BT9-067.test.ts`; no prohibited execution gate was run.
...
- all ten focused card test files and the new BT9-067 placed-card scaling path;
