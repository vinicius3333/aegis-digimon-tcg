...
Three proven implementation gaps were corrected across two audited cards and one
...
opponent unsuspended Digimon and binds the deletion result. Only when that
binding exists does it optionally play an own purple level-3 Digimon from
trash without cost; the played permanent is bound and its On Play timing is
disabled. Security activates Main. `BT9-108.test.ts` checks the catalog and
drives deletion, free trash play, and On Play suppression. Although there is no
card-specific KB entry, §15-1 ordered processing, the play lifecycle, and the
timing-disable primitive establish the “if you do” dependency and suppression
scope.
...
target; that drift is not authoritative. No direct gap was found; provisional
score: **8/10**.
...
Direct behavior correction count: **2 cards; 5 direct card/IR field occurrences**.
Shared seam correction count: **1 field occurrence**. Total corrected behavior
fields/seams: **6** across **3 proven gaps**. Behavior ambiguity count: **0**.
...
| BT9-108 | Lacks deletion/play bindings, uses raw “you do,” and suppresses a broad target rather than the played permanent. |
