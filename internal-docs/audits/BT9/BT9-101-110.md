...
Two direct implementation gaps were corrected. BT9-102 used the non-dispatchable
...
that flag was removed so the normal printed evolution cost is used. The focused
BT9-102 and BT9-109 tests were strengthened as static proof. No generated
snapshot, historical `BT9-AUDIT.md`, or unrelated collection claim was changed.
...
There is one recorded ambiguity, described under BT9-109: the existing
`excludeCardsNamed` matcher is substring-based while the printed bracketed name
is exact. The direct module retains that existing matcher contract rather than
inventing a new exclusion primitive; no additional ambiguity was invented.
...
lock is implemented by the shared exact-instance restriction primitive, so
rule-driven stack changes remain distinct from effects as required by Q1606,
...
duplicate-permanent decisions. Its new static proof asserts that the inherited
evolution pays the normal cost and has no alternate-cost forcing flag. X
...
One ambiguity remains: `excludeCardsNamed: ["X Antibody"]` delegates to the
existing substring-based name-exclusion matcher, whereas the bracketed card
name is ordinarily exact under §2-3-1-2. The current card set and focused
coverage use the exact X Antibody card, and no new matcher was invented during
this audit. Provisional score: **8/10**.
...
Corrected-card count: **2**. Corrected card/IR field-occurrence count: **5**.
