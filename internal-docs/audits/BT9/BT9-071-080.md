...
**Implementation and proof.** The direct Security action self-targets and uses
`PlayWithoutCost` with `payCost: false` and `isSecurity: true`. The inherited
OnDeletion action uses `selfColorCount >= 2` and is marked inherited. The
focused tests cover catalog/evolution metadata, Security self-play without a
cost, deletion on the opponent's turn so memory credits BT9-074's controller,
a one-color negative host, and a color-granting host. BT3-014 is the color
grant peer and BT9-090 is the nearby Security self-play peer. Sections
...
**Correction and snapshot.** No new direct gap was found. The snapshot at
`effects.json:122223` is partial and contains `RawUnparsed` for the Security
self-play effect plus a raw inherited color condition. The direct module is a
complete full-coverage implementation; the snapshot was read only.
...
BT9-081 onward; the coordinator should run the deferred focused/mechanism/
