...
The inherited action had a proven target-identity gap: the old IR suspended one qualifying opposing Digimon and then resolved `Restrict` against an arbitrary opposing Digimon. The printed “That Digimon” requires the restriction to reference the first choice. The direct module now selects and binds one qualifying target as `suspendedTarget`, suspends from that reference, and restricts that same reference with `restriction: "unsuspend"` and `duration: "untilOpponentTurnEnd"`. This uses the established SelectBind/`fromSelectionRef` interpreter seam used by BT15-058, BT8-100, and other same-target peers.
...
The intended atomic commit contains the three corrected direct modules, their three strengthened focused tests, and this report. The worker branch is to be pushed non-force; the final stable commit hash and push result are communicated separately to the coordinator.
