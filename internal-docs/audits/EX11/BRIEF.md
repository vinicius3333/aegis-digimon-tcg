# EX11 card audit brief (shared by all batch agents)

Working directory (absolute, always use it, never the main repo root):
`/Users/viniciusluiz/aegis-digimon-tcg/.claude/worktrees/audit-ex11`
...
`apps/api/src/cards/EX11/AUDIT.md` already scores every EX11 card 10/10. Treat that as a claim
to falsify. The coordinator already proved two gaps the ledger missed:

- the coordinator-probed persisted records in `packages/shared/src/effects/effects.json` differ from the
...
- Catalog: `packages/shared/src/cards/data/cards.json` (field `cardId`, set `EX11`).
...
- Peers: neighboring EX11 cards and other sets sharing the trait, keyword, or effect vocabulary.
...
3. Behavioral proof: strengthen `<CARD-ID>.test.ts` following current neighboring EX11 tests
...
4. Peer and stack: add the trait-mix and realistic evolution-stack cases the skill's step 4
   requires. `EX11-062.vortex.test.ts` is a second suite for EX11-062; keep it green and
   treat it as part of that card's proof.
...
- Edit only `EX11-<ID>.ts` and `EX11-<ID>.test.ts` for cards in your batch (plus
  `EX11-062.vortex.test.ts` if EX11-062 is yours). Never edit `index.ts`, `AUDIT.md`,
  `EX11.audit.test.ts`, `EX11-catalog-sync.test.ts`, `effects.json`, other sets, or anything
...
Write `internal-docs/audits/EX11/EX11-<FIRST>-<LAST>.md` following the structure of
