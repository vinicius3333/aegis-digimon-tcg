  ...
  switch (action.kind) {
    ...
    case "PlayWithoutCost": {
      ...
      if (selfPlayTarget) {
        ...
        if (fromSecurity) {
          if (owner === undefined) {
            ...
            const played = await playEffectInstances(ctx, [self], { payCost: action.payCost });
          ...
          const played = await playEffectInstances(ctx, [self], {
            payCost: action.payCost,
            ...(action.breeding === true ? { breeding: true } : {}),
            ...(scaledReduction !== undefined ? { costDelta: scaledReduction } : {}),
            ...(action.suppressOnPlayEffects === true ? { suppressOnPlayEffects: true } : {}),
          });
          ctx.lastPlayedPermanentIds = (played ?? []).map((p) => p.permanentId);
        } else {
      ...
      if (action.payCost === true && ctx.fx.canAffordEffectPlay !== undefined) {
        ...
        for (const candidate of candidates) {
          ...
          const assembly = basePlayAffordable
            ? undefined
            : availableEffectPlayAssembly(ctx, candidate, reservedAssemblyMaterials);
          const play =
            basePlayAffordable ||
            (hasPermanent &&
              assembly !== undefined &&
              (await ctx.fx.canAffordEffectPlay!(candidate.instanceId, {
                costDelta: (costReduction ?? 0) + assembly.reduction,
                controllerSeat: ctx.source.ownerSeat,
              })));
          if (play && !basePlayAffordable && assembly !== undefined) {
            reservedAssemblyMaterials.push(...assembly.materialInstanceIds);
          }
          const use =
            canUseByColor &&
            (await ctx.fx.canAffordEffectPlay!(candidate.instanceId, {
      ...
      if (chosen.length > 0) {
        ...
        const digiXrosMaterialInstanceIdsByPlay: Record<string, string[]> = {};
        if (action.allowDigiXros === true && permanentIds.length > 0) {
          const eligibleDigiXrosIds = permanentIds.filter(
            (instanceId) =>
              digiXrosRequirementFor(
                candidates.find((candidate) => candidate.instanceId === instanceId)?.cardId ?? "",
              )?.[0] !== undefined,
          );
          ...
          try {
            ...
            for (const pendingId of eligibleDigiXrosIds) {
              const candidate = candidates.find((entry) => entry.instanceId === pendingId);
              if (candidate !== undefined) ctx.fx.consumeDigiXrosPlayExpansions?.(candidate.ownerSeat, pendingId);
            }
            throw error;
          }
          const reservedMaterials = new Set<string>();
          try {
            for (const pendingPlayId of permanentIds) {
              const playedCard = candidates.find((candidate) => candidate.instanceId === pendingPlayId);
              ...
              if (playedCard !== undefined && requirement !== undefined) {
                try {
                  ...
                  const defaultCandidates = [
                    ...looseCardsInZone(ctx, ownerSeat, "hand").filter(
                      (candidate) =>
                        ...
                        !permanentIds.includes(candidate.instanceId) &&
                  ...
                  if (singleUnderTamerHost) {
                    const hostIds = [
                      ...new Set(underTamerCandidates.map((candidate) => candidate.hostPermanentId)),
                  ...
                  if (
                    ...
                    digiXrosMaterialInstanceIdsByPlay[playedCard.instanceId] = selected;
        ...
        const played =
          permanentIds.length > 0
            ? await playEffectInstances(ctx, permanentCards, {
                payCost: action.payCost,
                ...
                ...((action as typeof action & { costOverride?: number }).costOverride !== undefined
                  ...
                  : {}),
                ...(Object.keys(digiXrosMaterialInstanceIdsByPlay).length > 0
                  ? { digiXrosMaterialInstanceIdsByPlay }
                  : {}),
                ...(action.suppressOnPlayEffects === true ? { suppressOnPlayEffects: true } : {}),
                hostPermanentIds,
              })
