...
describe("Main action readiness boundary", () => {
  it("rejects a public play while a timing window is active, then accepts after release", async () => {
    const s = setupEngine({
      0: { deck: ["BT1-009"], hand: [{ card: "BT1-009", as: "option" }] },
      1: { deck: ["BT1-009"], security: 3 },
    });
    await s.ready();
    const beforeHand = s.state.players[0]!.hand.length;
    ...
    expect(s.state.players[0]!.hand).toHaveLength(beforeHand);
