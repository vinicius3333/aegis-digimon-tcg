...
describe("deadline resolution", () => {
  it("rebinds a game only for the durable destination transfer epoch", async () => {
    const series = await startSeries();
    const gameId = await openGame(series.id, "room-source", START + 10_000);
    const sessionId = `session-${series.matchId}`;
    const transferId = `transfer-${series.matchId}`;
    await fixture.accounts.pool.query(
      `INSERT INTO room_sessions
         (id,mode,status,participants,tournament_match_id,tournament_game_id,owner_generation_id,owner_process_id,
          owner_room_id,owner_epoch,active_transfer_id,created_at,updated_at)
       VALUES ($1,'tournament','active','[]',$2,$3,'generation-b','process-b','room-destination',2,$4,$5,$5)`,
      [sessionId, series.matchId, gameId, transferId, START],
    );
    await fixture.accounts.pool.query(
      `INSERT INTO room_transfers
       ...
       VALUES ($1,$2,'generation-a','process-a','room-source','generation-b','process-b','room-destination',1,2,'destination_active',$3,$3)`,
      [transferId, sessionId, START],
    );

    expect(
      await fixture.store.rebindGameRoomForHandoff({
        gameId,
        sessionId,
        transferId,
        fromRoomId: "room-source",
        toRoomId: "room-destination",
        ownerEpoch: 1,
      }),
    ).toEqual({ ok: false, reason: "handoff_transfer_mismatch" });
    const rebound = await fixture.store.rebindGameRoomForHandoff({
      gameId,
      sessionId,
      transferId,
      fromRoomId: "room-source",
    ...
    expect(rebound).toEqual({
      ok: true,
      value: { gameId, roomId: "room-destination", replayed: false },
    });
    expect(
      await fixture.store.rebindGameRoomForHandoff({
        gameId,
    ...
    ).toEqual({ ok: true, value: { gameId, roomId: "room-destination", replayed: true } });
    expect((await fixture.store.recordGameResult(winner(gameId, "room-destination", fixture.alice, START + 20_000))).ok).toBe(
      true,
    );
  });

  it("rechecks the frozen-session fence inside the series deadline transaction", async () => {
    const series = await startSeries();
    const sessionId = `session-${series.matchId}`;
    const transferId = `transfer-${series.matchId}`;
    await fixture.accounts.pool.query(
      ...
      [sessionId, series.matchId, START],
    );
    await fixture.accounts.pool.query(
      `INSERT INTO room_transfers
         (id,session_id,from_generation_id,from_process_id,from_room_id,to_generation_id,to_process_id,to_room_id,
    ...
    ).toEqual({ ok: false, reason: "handoff_in_progress" });
    expect((await fixture.store.series(series.id))?.status).toBe("playing");
  });

  it("compensates a series deadline once for a frozen handoff and persists its replay receipt", async () => {
    const series = await startSeries();
    const first = await fixture.store.compensateSeriesDeadline({
      seriesId: series.id,
      transferId: "transfer-1",
      pausedAtMs: START + 100,
      resumedAtMs: START + 5_100,
      now: START + 5_100,
    });
    expect(first).toEqual({
      ...
      value: {
        seriesId: series.id,
        deadlineAtMs: series.seriesDeadlineAt! + 5_000,
        compensatedMs: 5_000,
        replayed: false,
      },
    });

    const replay = await fixture.store.compensateSeriesDeadline({
      seriesId: series.id,
      transferId: "transfer-1",
      pausedAtMs: START + 100,
      resumedAtMs: START + 5_100,
    ...
    expect((await fixture.store.series(series.id))?.seriesDeadlineAt).toBe(series.seriesDeadlineAt! + 5_000);
    const scheduled = await fixture.accounts.pool.query<{ due_at: string | number }>(
      "SELECT due_at FROM tournament_deadlines WHERE kind='series_deadline' AND subject_id=$1",
      [series.id],
    );
    expect(Number(scheduled.rows[0]?.due_at)).toBe(series.seriesDeadlineAt! + 5_000);
    await expect(
      fixture.store.compensateSeriesDeadline({
        seriesId: series.id,
        transferId: "transfer-1",
        pausedAtMs: START + 100,
        resumedAtMs: START + 5_101,
      }),
    ).rejects.toThrow("conflicting deadline compensation replay");
  });
