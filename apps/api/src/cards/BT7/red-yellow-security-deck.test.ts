...
describe("red-yellow security deck through BT10", () => {
  it("chains RizeGreymon's yellow color into Aquilamon, Tai & Kari, and MagnaAngemon recovery", async () => {
    ...
    s.state.memory = 7;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("magnaAngemon").instanceId })).toEqual({ ok: true });
  ...
  it("recovers to exactly 3 with Kazuchimon before Electric Rush boosts and unsuspends it", async () => {
    ...
    s.state.memory = 4;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("electricRush").instanceId })).toEqual({ ok: true });
