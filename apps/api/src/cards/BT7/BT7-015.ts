...
// 1. CostModifier had no `target`, so `if (!want) return false` made the whole
...
//    own play cost only (costType "memory"/play never touches digivolve cost).
// 2. The two independent "return all" actions (trait match, Option kind) would
...
// 3. Delete condition "7 or more returned": the Return records its tally via
