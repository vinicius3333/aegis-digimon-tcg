...
describe("BT7-017 Chaosdramon", () => {
  it("places a level-5 Cyborg as a source and deletes for each level-5 Cyborg source", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "AD1-002", as: "base" }], hand: [{ card: "BT7-017", as: "evolving" }, { card: "BT1-021", as: "cyborg" }] }, 1: { battleArea: [{ card: "BT2-047", as: "target" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 4;
