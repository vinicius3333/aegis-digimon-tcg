...
describe("BT7 Purple Hybrid Cherubimon historical deck", () => {
  it("chains Koichi, reduced evolution, Tamer toolbox deletion, and scaled On Deletion plays", async () => {
    ...
    expect(new Set(tamerDecision.options?.candidateInstanceIds)).toEqual(
      new Set([s.inst("playedKoichi").instanceId, s.inst("keptMimi").instanceId]),
    );
    expect(
      s.engine.applyIntent(0, {
        type: "respondDecision",
    ...
    expect(new Set(deleteDecision.options?.candidateInstanceIds)).toEqual(
      new Set([
    ...
    expect(s.state.players[1]!.battleArea[0]?.permanentId).toBe(s.perm("keptLevelFour").permanentId);
    expect(
      s.state.players[0]!.battleArea.filter(
        ({ topCard }) => topCard.cardId === "BT7-091" || topCard.cardId === "BT3-096",
      ),
    ).toHaveLength(2);
    ...
    expect(new Set(rookieDecision.options?.candidateInstanceIds)).toEqual(
      new Set([
    ...
    expect(
      s.engine.applyIntent(0, {
        ...
        response: {
          kind: "selectCards",
          instanceIds: [s.inst("playedRookieOne").instanceId, s.inst("playedRookieTwo").instanceId],
        },
      }),
    ).toEqual({ ok: true });
    await deletion;
    await settle(
      () =>
        s.state.players[0]!.battleArea.some(
          ({ topCard }) => topCard.instanceId === s.inst("playedRookieOne").instanceId,
        ) &&
        s.state.players[0]!.battleArea.some(
          ({ topCard }) => topCard.instanceId === s.inst("playedRookieTwo").instanceId,
        ),
    );

    expect(s.state.players[0]!.trash.some(({ instanceId }) => instanceId === s.inst("keptRookie").instanceId)).toBe(
      true,
    );
