...
describe("BT7-074 Antylamon", () => {
  it("plays a purple Tamer costing 3 or less from trash for free", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT10-074", as: "base" }], hand: [{ card: "BT7-074", as: "evolving" }], trash: [{ card: "BT7-091", as: "tamer" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 3;
