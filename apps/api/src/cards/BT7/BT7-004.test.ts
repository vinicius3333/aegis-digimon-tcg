...
describe("BT7-004 Koromon", () => {
  it("reveals the top card and lets you move it to the deck bottom when attacking", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT1-010", under: ["BT7-004"], as: "host" }], deck: [{ card: "BT1-011", as: "revealed" }, { card: "BT1-012", as: "second" }] }, 1: { security: ["BT1-101"] } }, { autoChooseOption: true, preferOptionIndex: 1 });
