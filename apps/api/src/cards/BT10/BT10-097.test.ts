...
describe("BT10-097 Blazing Memory Boost!", () => {
  ...
  it("cannot play Kiriha when it must be taken as one of fewer than 2 available Blue Flare cards", async () => {
    ...
    expect(s.state.players[0]!.battleArea.some((permanent) => permanent.topCard.instanceId === s.inst("kiriha").instanceId)).toBe(false);
  });
  ...
  it("executes the Blue Flare deck line from Blazing Memory Boost through Rush and Material Save", async () => {
    ...
    const boost = s.state.players[0]!.battleArea.find((permanent) =>
      permanent.topCard.instanceId === boostId,
    )!;
    const kiriha = s.state.players[0]!.battleArea.find((permanent) =>
      permanent.topCard.instanceId === s.inst("kiriha").instanceId,
    )!;
    ...
    await settle(() => s.state.memory === 9 && s.state.players[0]!.trash.some((card) => card.instanceId === boost.topCard.instanceId));
    ...
    await settle(() => ["firstFrozen", "secondFrozen"].every((alias) =>
      observe(s.engine).isRestricted(s.perm(alias), "attack") &&
      observe(s.engine).isRestricted(s.perm(alias), "block"),
    ));
    const metalGreymon = s.state.players[0]!.battleArea.find((permanent) =>
      permanent.topCard.instanceId === s.inst("metalGreymon").instanceId,
    )!;
