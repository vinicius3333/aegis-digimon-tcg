...
describe("BT10-021 MailBirdramon", () => {
  it("plays Kiriha Aonuma from hand when none is in play", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "BT10-021", as: "source" }, { card: "BT10-088", as: "kiriha" }] } }, { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true });
  ...
  it("does not treat MetalGreymon (X Antibody) as the named MetalGreymon target", async () => {
    ...
    expect(s.state.players[0]!.trash.some(
      (card) => card.instanceId === s.inst("metalGreymonX").instanceId,
    )).toBe(true);
  });
  ...
  it("restricts one opponent Digimon from attacking and blocking only with a Blue Flare host and 2 opponents", async () => {
    const s = setupEngine({
      ...
      1: { battleArea: [{ card: "BT1-010", as: "first" }, { card: "BT1-011", as: "second" }], security: ["BT1-012"] },
