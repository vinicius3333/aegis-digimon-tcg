...
describe("BT10-068 Gankoomon (X Antibody)", () => {
  it("plays Sistermon, gives all own Digimon +2000 DP, and protects them from bounce and DP reduction", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT6-067", as: "base" }], hand: [{ card: "BT10-068", as: "evolving" }, { card: "BT6-082", as: "sister" }] } }, { autoAcceptOptional: true, autoSelectCards: true }); s.state.memory = 1;
  ...
  it("restores a previously reduced Digimon before applying the +2000 DP bonus (Q1990)", async () => {
    const s = setupEngine({
      0: {
        ...
        hand: [{ card: "BT10-068", as: "evolving" }, { card: "BT6-082", as: "sister" }],
      },
