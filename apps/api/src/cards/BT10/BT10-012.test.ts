...
describe("BT10-012 Shoutmon X4B", () => {
  ...
  it("DigiXroses with Shoutmon X4 and Beelzemon, takes a Xros Heart card from under a Tamer, and returns two from trash", async () => {
    ...
    const x4b = player.battleArea.find(
    ...
    expect(x4b.stack.map((card) => card.instanceId)).toEqual(
      expect.arrayContaining([s.inst("shoutmonX4").instanceId, s.inst("beelzemon").instanceId]),
    );
