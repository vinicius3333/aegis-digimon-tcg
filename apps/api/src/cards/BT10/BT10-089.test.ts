...
describe("BT10-089 Akari Hinomoto", () => {
  ...
  it("plays Dorulumon from under another Tamer, then may suspend to draw for that Xros Heart play", async () => {
    ...
    const akari = s.state.players[0]!.battleArea.find(
      (permanent) => permanent.topCard.instanceId === s.inst("akari").instanceId,
    )!;
    expect(
      s.state.players[0]!.battleArea.some(
        (permanent) => permanent.topCard.instanceId === s.inst("dorulumon").instanceId,
      ),
    ).toBe(true);
  ...
  it("cannot play Dorulumon from the stack of a Digimon that evolved from a Tamer", async () => {
    ...
    expect(s.state.players[0]!.battleArea.filter((permanent) => permanent.topCard.cardId === "BT10-034")).toHaveLength(
      1,
    );
  });
