...
describe("BT10-106 Justice Kick", () => {
  it("deletes an opponent Digimon up to the play cost of the Justimon it played", async () => {
    const s = setupEngine({
      0: { battleArea: ["BT10-064"], hand: [{ card: "BT10-106", as: "option" }, { card: "BT10-067", as: "justimon" }] },
    ...
    await settle(() => s.state.players[0]!.battleArea.some((permanent) => permanent.topCard?.instanceId === s.inst("justimon").instanceId) && s.state.players[1]!.battleArea.length === 0);
