...
describe("BT10 DarkKnightmon / Nene / Immortal Ruler deck gauntlet", () => {
  it("selects exact trash materials, clears after De-Digivolve, gains Blocker, then splits its stack on deletion", async () => {
    ...
    const chosenMaterials = [
      s.inst("chosenSkullKnightmon").instanceId,
      s.inst("chosenDeadlyAxemon").instanceId,
    ];
    expect(materialRequest.options).toMatchObject({ min: 0, max: 2 });
    expect(new Set(materialRequest.options?.candidateInstanceIds)).toEqual(new Set([
      ...chosenMaterials,
    ...
    await settle(() =>
      ...
      !s.state.players[1]!.battleArea.some(({ permanentId }) =>
        permanentId === deleteTargetId
      ) &&
      s.state.pendingDecision === undefined &&
      s.events.some((event) =>
        event.kind === "effectResolved" && event.sourceCardId === "BT10-104"
      ),
      5000,
    );
    const darkKnightmon = s.state.players[0]!.battleArea.find(({ topCard }) =>
      topCard.cardId === "BT10-066"
    )!;
    ...
    expect(s.state.players[0]!.trash.some(({ instanceId }) =>
      instanceId === s.inst("unchosenSkullKnightmon").instanceId
    )).toBe(true);
    expect(s.state.players[1]!.trash.some(({ instanceId }) =>
      instanceId === s.inst("revealedRookie").instanceId
    )).toBe(true);
    ...
    await settle(() =>
      s.state.players[0]!.hand.some(({ instanceId }) =>
        instanceId === s.inst("chosenSkullKnightmon").instanceId
      ) &&
      s.state.players[0]!.battleArea.some(({ topCard }) =>
        topCard.instanceId === s.inst("chosenDeadlyAxemon").instanceId
      ),
      5000,
    );
