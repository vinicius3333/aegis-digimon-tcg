...
describe("ST8-05 Veedramon", () => {
  it("returns an opposing level 3 and trashes its sources when its host attacks with 8 cards", async () => {
    const s = setupEngine({ 0: { hand: Array(8).fill("ST8-02"), battleArea: [{ card: "ST8-07", as: "host", under: ["ST8-05"] }] }, 1: { battleArea: [{ card: "ST8-04", as: "target", under: [{ card: "ST8-01", as: "source" }] }], security: ["ST8-01"] } }, { autoSelectCards: true });
