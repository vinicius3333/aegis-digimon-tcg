...
describe("ST8-12 V-Wing Blade", () => {
  it("returns an opposing level 6 or lower and trashes all of its sources", async () => {
    const s = setupEngine(
      {
        0: { battleArea: ["ST8-04"], hand: [{ card: "ST8-12", as: "option" }] },
        1: { battleArea: [{ card: "ST8-09", as: "target", under: [{ card: "ST8-08", as: "source" }] }] },
      },
  ...
  it("activates its Main effect from security", async () => {
    const s = setupEngine(
      {
        0: { security: [{ card: "ST8-12", as: "option", faceUp: true }] },
        1: { battleArea: [{ card: "ST8-09", under: ["ST8-08"] }] },
      },
      { autoSelectCards: true },
    );
