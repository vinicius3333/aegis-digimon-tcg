...
describe("EX4-065 Trident Gaia", () => {
  it("deletes the highest-DP opposing Digimon", () => {
    expect(compiled.effects?.find((entry) => entry.trigger === "Main")?.actions?.find((action) => action.kind === "Delete")).toMatchObject({ kind: "Delete", target: { filter: { controller: "opponent", superlative: "highestDP" } } });
  });
  it("trashes the opponent's top security after a 13000-DP own Digimon deletion", () => {
    expect(compiled.effects?.find((entry) => entry.trigger === "Main")?.actions?.find((action) => action.kind === "SubTrigger")).toMatchObject({ kind: "SubTrigger", event: "onDeletionOf", once: true, sourceFilter: { dp: { op: "gte", value: 13000 } }, actions: [{ kind: "SecurityManipulation", op: "trashTop", controller: "opponent" }] });
