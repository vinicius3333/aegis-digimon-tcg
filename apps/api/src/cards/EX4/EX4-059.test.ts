...
const def = (id: string): CardDefinition => ({
  cardId: id,
  set: "TEST",
  nameEn: id,
...
describe("EX4-059 Cherubimon", () => {
  it("suspends another Digimon and binds its DP plus Piercing to the attack", async () => {
    ...
    const fx = {
      suspend: async (ids: string[]) => calls.push(["suspend", ids]),
      modifyDP: (...args: unknown[]) => calls.push(["modifyDP", ...args]),
      grantKeyword: (...args: unknown[]) => calls.push(["keyword", ...args]),
    ...
    expect(
      calls.some(
        (call) => Array.isArray(call) && call[0] === "keyword" && call[1] === "self" && call[2] === "Piercing",
      ),
    ).toBe(true);
  });
