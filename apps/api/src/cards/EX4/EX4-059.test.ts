...
describe("EX4-059 Cherubimon", () => {
  ...
  it("grants optional On Deletion replay to itself and one eligible ally", () => {
    ...
    expect(effect?.actions?.[0]).toMatchObject({ kind: "GainTriggeredEffect", gainedTrigger: "OnDeletion", duration: "untilOpponentTurnEnd" });
    ...
    expect((effect?.actions?.[0] as any)?.gainedActions?.[0]).toMatchObject({ kind: "PlayWithoutCost", from: ["trash"], payCost: false, optional: true });
  });
});
