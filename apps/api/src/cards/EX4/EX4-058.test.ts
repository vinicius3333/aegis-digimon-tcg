...
describe("EX4-058 Ravemon", () => {
  ...
  it("trashes an opponent hand card at eight or more cards, otherwise adds security to hand", () => {
    ...
    expect(actions?.[0]).toMatchObject({ kind: "Trash", chooser: "opponent", condition: { kind: "zoneCount", op: "gte", value: 8 } });
    expect(actions?.[1]).toMatchObject({ kind: "SecurityManipulation", op: "toHand", condition: { kind: "zoneCount", op: "lte", value: 7 } });
  });

  it("trashes one opposing hand card when the opponent has eight cards", () => {
    const actions = compiled.effects?.find((entry) => entry.trigger === "OnDeletion")?.actions;
    expect(actions?.[0]).toMatchObject({ kind: "Trash", chooser: "opponent", target: { filter: { controller: "opponent", zone: "hand" }, count: 1 }, condition: { kind: "zoneCount", seat: "opponent", zone: "hand", op: "gte", value: 8 } });
  });
});
