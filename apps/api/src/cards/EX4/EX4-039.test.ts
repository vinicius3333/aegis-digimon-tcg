...
describe("EX4-039 Gabumon", () => {
  it("reveals three, adds Garurumon and Agumon/Greymon/Omnimon, and returns the rest to deck top", () => {
    expect(compiled.effects?.find((entry) => entry.trigger === "OnPlay")?.actions?.[0]).toMatchObject({ kind: "RevealAdd", revealCount: 3, rest: "deckTop", add: [{ filter: { nameOrTrait: [{ match: "name", tokens: ["Garurumon"] }] } }, { filter: { nameOrTrait: [{ match: "name", tokens: ["Agumon", "Greymon", "Omnimon"] }] } }] });
  });
