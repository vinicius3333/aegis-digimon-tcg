...
function definition(cardId: string): CardDefinition {
  return { cardId, set: "TEST", nameEn: cardId, kinds: [CardKind.Digimon], colors: ["Black"] as never, playCost: 5, dp: 1000, level: 5, evoCosts: [], maxCountInDeck: 4 };
}
...
describe("EX4-051 BlitzGreymon", () => {
  it("can De-Digivolve up to three opposing Digimon through modal option one", async () => {
    ...
    const opponents = ["A", "B", "C"].map((id) => ({ permanentId: id, topCard: card(id, 1), stack: [], linked: [], isSuspended: false, inBreeding: false } as unknown as Permanent));
    const players = [{ battleArea: [self], security: [], hand: [], deck: [], trash: [] }, { battleArea: opponents, security: [], hand: [], deck: [], trash: [] }];
    const definitions = new Map<string, CardDefinition>([["EX4-051", definition("EX4-051")], ...opponents.map((p) => [p.topCard!.cardId, definition(p.topCard!.cardId)] as const)]);
    ...
    const fx = { deDigivolve: (id: string, amount: number) => { calls.push([id, amount]); } } as unknown as Primitives;
    ...
    const source: CardSource = { instanceId: selfCard.instanceId, cardId: "EX4-051", ownerSeat: 0 as Seat, definition: definitions.get("EX4-051")!, permanent: () => self, isOnBattleArea: () => true, isOwnersTurn: () => true, hasColor: () => true };
