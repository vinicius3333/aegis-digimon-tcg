...
describe("EX4-006 Guilmon", () => {
  ...
  it("gains Rush from the combined trashes threshold and not below it", async () => {
    const card = "BT1-010";
    const enough = setupEngine({ 0: { battleArea: [{ card: "EX4-006", as: "guilmon" }], trash: Array(10).fill(card) }, 1: { trash: Array(10).fill(card) } });
    await enough.ready();
    ...
    const short = setupEngine({ 0: { battleArea: [{ card: "EX4-006", as: "guilmon" }], trash: Array(9).fill(card) }, 1: { trash: Array(10).fill(card) } });
    await short.ready();
