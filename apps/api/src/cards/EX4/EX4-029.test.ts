...
describe("EX4-029 Antylamon", () => {
  ...
  it("places the top deck card into security at three or fewer security", () => {
    expect(compiled.effects?.find((entry) => entry.trigger === "EndOfAttack")?.actions?.[0]).toMatchObject({ kind: "SecurityManipulation", op: "placeFromDeck", toTop: true, condition: { kind: "securityAtMost", value: 3 } });
  });
  ...
  it("restores security and reduces an opponent after an attack", async () => {
    const s = setupEngine({
      0: {
        ...
        battleArea: [{ card: "EX4-029", as: "source" }, { card: "BT1-009", as: "inheritedHost", under: ["EX4-029"] }, { card: "BT1-009", as: "other", suspended: true }],
      },
    ...
    await advance(s.engine).fire(EffectTiming.OnEndAttack, s.perm("inheritedHost"));
