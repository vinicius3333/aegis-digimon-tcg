...
describe("EX4-019 MachGaogamon", () => {
  ...
  it("returns an opposing level 4 Digimon on digivolving", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX4-019", as: "mach" }] }, 1: { battleArea: [{ card: "BT4-009", as: "target" }] } }, { autoSelectCards: true });
    await s.ready();
  ...
  it("unsuspends itself when the opponent has eight cards in hand", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT4-009", as: "host", suspended: true, under: ["EX4-019"] }] }, 1: { hand: Array(8).fill("BT1-001") } });
    await s.ready();
