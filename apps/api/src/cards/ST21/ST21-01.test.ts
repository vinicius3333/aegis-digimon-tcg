...
describe("ST21-01 Tsunomon", () => {
  it("returns exactly one ADVENTURE Digimon from trash when the stack is deleted", async () => {
    ...
    expect(hand.filter((cardId) => cardId === "ST21-03")).toHaveLength(1);
    expect(hand).not.toContain("BT1-001");
    const trash = (s.state.players[0] as PlayerState).trash.map((card) => card.cardId);
    expect(trash.filter((cardId) => cardId === "ST21-03")).toHaveLength(0);
    expect(trash.filter((cardId) => cardId === "BT1-001")).toHaveLength(1);
  });
});
