...
describe("BT20-026 MegaSeadramon (X Antibody)", () => {
  ...
  it("publishes MegaSeadramon's exact identity and blue level-4 evolution routes", () => {
    expect(getCardDefinition("BT20-026")).toMatchObject({
      colors: ["Blue"],
      kinds: ["Digimon"],
      level: 5,
      playCost: 8,
  ...
  it("bottoms only a level-4-or-lower Digimon and locks another Digimon from effect suspension", async () => {
    ...
    await settle(
      () =>
        s.state.players[1]!.deck.at(-1)?.cardId === "BT20-023" &&
        observe(s.engine).isRestricted(s.perm("locked"), "suspend"),
    );
