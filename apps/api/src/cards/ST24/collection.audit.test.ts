...
const st24Ids = allCards()
  .filter((card) => /^ST24-\d{2}$/.test(card.cardId))
  .sort((a, b) => Number(a.cardId.slice(5)) - Number(b.cardId.slice(5)))
  .map((card) => card.cardId);
const expectedIds = Array.from({ length: 15 }, (_, index) => `ST24-${String(index + 1).padStart(2, "0")}`);
