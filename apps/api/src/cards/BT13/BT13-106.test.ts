...
describe("BT13-106 Odin's Breath", () => {
  ...
  it("applies the DP reduction and Security Attack -1 to every opposing Digimon at six total security", async () => {
    const s = setupEngine(
      {
        ...
        1: { battleArea: [{ card: "BT13-111", as: "first" }, { card: "BT13-111", as: "second" }], security: ["BT1-001", "BT1-001", "BT1-001"] },
      },
    ...
    await settle(() => s.perm("first").currentDP === 10000 && observe(s.engine).keywordAmount(s.perm("first"), "SecurityAttack") === -1);
