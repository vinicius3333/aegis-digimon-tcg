...
describe("BT13-059 Examon", () => {
  ...
  it("suspends an opponent Digimon on play and keeps the selected target restricted", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "BT13-059", as: "examon" }] }, 1: { battleArea: [{ card: "BT1-015", as: "target" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    await s.ready();
    s.state.memory = 30;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("examon").instanceId })).toEqual({ ok: true });
