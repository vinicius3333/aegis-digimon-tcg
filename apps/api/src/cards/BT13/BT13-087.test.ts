...
describe("BT13-087 Dynasmon", () => {
  ...
  it("deletes opposing level 4 Digimon when a Royal Knight is played", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT13-087", as: "dynasmon" }], hand: [{ card: "BT13-075", as: "royal" }] }, 1: { battleArea: [{ card: "BT13-081", as: "target" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    await s.ready();
    s.state.memory = 12;
