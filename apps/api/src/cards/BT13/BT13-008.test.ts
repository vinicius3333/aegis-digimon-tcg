...
describe("BT13-008 Agumon", () => {
  it("digivolves from Koromon for 0 memory through its alternate requirement", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT11-005", as: "koromon" }], hand: [{ card: "BT13-008", as: "agumon" }] } });
    s.state.memory = 3;
    await s.ready();

    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("koromon").permanentId, instanceId: s.inst("agumon").instanceId, alternateRequirementIndex: 0 })).toEqual({ ok: true });
  ...
  it("makes one Marcus Damon a 3000 DP Digimon that cannot digivolve for the turn", async () => {
    const s = setupEngine({
      0: {
        battleArea: [{ card: "BT13-008", as: "agumon" }, { card: "BT12-092", as: "marcus" }],
        security: ["BT1-001"],
      },
  ...
  it("once per turn may delete only an opposing Digimon with 3000 DP or less when a red or yellow Tamer suspends", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "BT1-015", as: "host", under: ["BT13-008"] }, { card: "BT12-092", as: "marcus" }] },
    ...
    expect(s.state.players[1]!.battleArea.some((permanent) => permanent.permanentId === s.perm("large").permanentId)).toBe(true);
  ...
  it("does not delete when the inherited optional effect is declined", async () => {
    const s = setupEngine(
      { 0: { battleArea: [{ card: "BT1-015", as: "host", under: ["BT13-008"] }, { card: "BT12-092", as: "marcus" }] }, 1: { battleArea: [{ card: "BT1-012", as: "target" }] } },
