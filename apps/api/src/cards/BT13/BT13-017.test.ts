...
describe("BT13-017 Jesmon", () => {
    ...
    const s = setupEngine(
      {
        0: {
          battleArea: [
            { card: "BT1-012", as: "allyA" },
            { card: "BT1-015", as: "allyB" },
          ],
          hand: [{ card: "BT13-017", as: "jesmon" }],
        },
  ...
  it("when digivolving applies the same scaled deletion budget", async () => {
    ...
    const tooLarge = setupEngine(
      {
        0: {
          battleArea: [{ card: "BT13-016", as: "base" }, { card: "BT1-012" }],
          hand: [{ card: "BT13-017", as: "jesmon" }],
        },
    ...
    expect(
      tooLarge.engine.applyIntent(0, {
        type: "digivolve",
        permanentId: tooLarge.perm("base").permanentId,
  ...
  it("gives all allied Digimon +1000 DP for each other Sistermon or Royal Knight", async () => {
    ...
    const baseDp = new Map(
      ["jesmon", "sistermon", "royalKnight", "other"].map((alias) => [alias, s.perm(alias).currentDP]),
    );
    await s.ready();
