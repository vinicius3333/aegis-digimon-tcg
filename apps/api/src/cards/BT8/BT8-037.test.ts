...
describe("BT8-037 Dinohyumon", () => {
  ...
  it("does not trigger when the opponent attacks", async () => {
    const s = setupEngine({
      0: {
        battleArea: [{ card: "BT8-042", as: "host", under: ["BT8-037"] }, { card: "BT8-017", as: "wouldReduce" }],
        security: ["BT8-034"],
      },
    ...
    expect(s.engine.applyIntent(1, { type: "attack", attackerPermanentId: s.perm("opponentAttacker").permanentId, target: { kind: "player" } })).toEqual({ ok: true });
    await settle();
  ...
  it("digivolves for 2 from both yellow and red level-3 Digimon", async () => {
    const s = setupEngine({ 0: {
      battleArea: [{ card: "BT8-034", as: "yellowBase" }, { card: "BT8-008", as: "redBase" }],
      hand: [{ card: "BT8-037", as: "yellowEvolution" }, { card: "BT8-037", as: "redEvolution" }],
    } });
    s.state.memory = 5;

    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("yellowBase").permanentId, instanceId: s.inst("yellowEvolution").instanceId })).toEqual({ ok: true });
    await settle();
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("redBase").permanentId, instanceId: s.inst("redEvolution").instanceId })).toEqual({ ok: true });
    await settle();
