...
describe("BT8-035 Candlemon", () => {
  ...
  it("gains memory only once after two purple Digimon are played in the turn", async () => {
    const s = setupEngine({ 0: {
      battleArea: [{ card: "BT8-041", as: "host", under: ["BT8-035"] }],
      hand: [{ card: "BT8-073", as: "first" }, { card: "BT8-073", as: "second" }],
    } });
    s.state.memory = 10;
  ...
  it("digivolves for 0 from a purple level-2 stack", async () => {
    ...
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("purpleEgg").permanentId, instanceId: s.inst("candlemon").instanceId })).toEqual({ ok: true });
    await settle();
