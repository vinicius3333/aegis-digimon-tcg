...
describe("BT8-080 Myotismon", () => {
  it("mills 2, then plays Yukio Oikawa from trash for free", async () => {
    const s = setupEngine(
      {
        0: {
          ...
          hand: [{ card: "BT8-080", as: "evolving" }],
          deck: ["BT1-009", { card: "BT8-093", as: "yukio" }, "BT1-010"],
        },
      },
    ...
    expect(
      s.state.players[0]!.battleArea.some((permanent) => permanent.topCard?.instanceId === s.inst("yukio").instanceId),
    ).toBe(true);
  ...
  it("plays Yukio Oikawa when a Myotismon-named host with this inherited effect is deleted", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [{ card: "BT8-083", as: "host", under: ["BT8-080"] }],
          hand: [{ card: "BT8-093", as: "yukio" }],
        },
      },
      { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true },
    );
    ...
    expect(
      s.state.players[0]!.battleArea.some(
        (permanent) => permanent.topCard?.instanceId === s.inst("yukio").instanceId && permanent.isSuspended,
      ),
    ).toBe(true);
  });
  ...
  it("does not play Yukio Oikawa when the deleted host is not Myotismon-named", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [{ card: "BT1-009", as: "host", under: ["BT8-080"] }],
          hand: [{ card: "BT8-093", as: "yukio" }],
        },
      },
      { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true },
