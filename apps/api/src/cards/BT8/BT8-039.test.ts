...
describe("BT8-039 Rapidmon", () => {
  it("suspends one opposing Digimon per Tamer, then gives up to 3 suspended Digimon -5000 DP", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT1-064", as: "base" }, "BT1-085", "BT1-086"], hand: [{ card: "BT8-039", as: "evolving" }] }, 1: { battleArea: [{ card: "BT2-047", as: "first" }, { card: "BT2-047", as: "second" }, { card: "BT2-047", as: "third", suspended: true }, { card: "BT2-047", as: "fourth", suspended: true }] } }, { autoSelectCards: true });
    s.state.memory = 4;
  ...
  it("does not apply the exact-name special cost to Terriermon Assistant", async () => {
    ...
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("assistant").permanentId, instanceId: s.inst("rapidmon").instanceId })).toEqual({ ok: true });
    await settle();
