...
describe("BT8-085 Yolei Inoue", () => {
  ...
  it("gains 1 memory at the start of its main phase when a red Digimon is in play", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT8-085", as: "yolei" }, { card: "BT8-013", as: "red" }] } });
    s.state.turnSeat = 0;
    s.state.memory = 0;
  ...
  it("plays itself from security without paying memory", async () => {
    const s = setupEngine({ 0: { security: [{ card: "BT8-085", as: "securityYolei", faceUp: true }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    await s.ready();
    await advance(s.engine).fireForInstance(EffectTiming.SecuritySkill, s.inst("securityYolei"));
    expect(s.state.players[0]!.battleArea.some(permanent => permanent.topCard.instanceId === s.inst("securityYolei").instanceId)).toBe(true);
  });
});
