...
describe("BT8-070 BlackWarGreymon", () => {
  it("publishes and applies one combined play-cost-6 budget for opposing Digimon and Tamers", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT10-065", under: ["BT1-021", "BT8-058"], as: "base" }], hand: [{ card: "BT8-070", as: "evolving" }] }, 1: { battleArea: [{ card: "BT1-009", as: "digimon" }, { card: "BT8-093", as: "tamer" }, { card: "BT1-015", as: "tooExpensive" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 4;
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("evolving").instanceId })).toEqual({ ok: true });
    await settle(() => s.decisions.some(({ req }) => req.kind === "chooseTargets" && req.options?.maxTotalPlayCost === 6));
    expect(s.decisions.some(({ req }) => req.kind === "chooseTargets" && req.options?.maxTotalPlayCost === 6)).toBe(true);
