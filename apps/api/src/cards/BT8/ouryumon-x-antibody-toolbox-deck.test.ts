...
describe("BT8 Ouryumon X Antibody toolbox deck", () => {
  it("tracks duplicate hosts by permanent, applies Ouryumon's errata globally, and caps it once per turn", async () => {
    ...
    expect(firstDecision.options?.candidateInstanceIds).toEqual(
      expect.arrayContaining([s.perm("firstHost").permanentId, s.perm("secondHost").permanentId]),
    );
    expect(firstDecision.options?.candidateInstanceIds).not.toContain(s.perm("firstHost").topCard.instanceId);
    expect(
      s.engine.applyIntent(0, {
    ...
    expect(secondDecision.options?.candidateInstanceIds).not.toContain(s.perm("firstHost").permanentId);
    expect(secondDecision.options?.candidateInstanceIds).toContain(s.perm("secondHost").permanentId);
    expect(
      s.engine.applyIntent(0, {
        type: "respondDecision",
    ...
    await settle(() => s.perm("secondHost").stack.some(({ instanceId }) => instanceId === secondOptionId));
