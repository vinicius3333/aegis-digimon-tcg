...
describe("BT8-025 Hookmon", () => {
  ...
  it("digivolves for 2 from both blue and black level-3 Digimon", async () => {
    const s = setupEngine({ 0: {
      battleArea: [{ card: "BT8-021", as: "blueBase" }, { card: "BT8-060", as: "blackBase" }],
      hand: [{ card: "BT8-025", as: "blueEvolution" }, { card: "BT8-025", as: "blackEvolution" }],
    } });
    s.state.memory = 5;

    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("blueBase").permanentId, instanceId: s.inst("blueEvolution").instanceId })).toEqual({ ok: true });
    await settle();
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("blackBase").permanentId, instanceId: s.inst("blackEvolution").instanceId })).toEqual({ ok: true });
    await settle();
