...
describe("BT8-067 MetalGreymon", () => {
  it("de-digivolves one opposing Digimon, then deletes a 3000-DP-or-less Digimon", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "BT10-061", as: "base" }], hand: [{ card: "BT8-067", as: "evolving" }] },
        1: {
          battleArea: [
            { card: "BT8-067", under: ["BT1-009", "BT1-015"], as: "stacked" },
  ...
  it("lets a Dragonkin host attack an unsuspended Digimon without granting Vortex", async () => {
    ...
    await settle(() => s.perm("host").attackablePermanentIds.includes(s.perm("unsuspendedTarget").permanentId));
    expect(
      s.engine.applyIntent(0, {
        type: "attack",
