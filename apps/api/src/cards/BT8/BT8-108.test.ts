...
describe("BT8-108 Mist Memory Boost", () => {
  it("trashes two deck cards, draws one, and enters the battle area", async () => {
    const s = setupEngine({
      0: { battleArea: ["BT8-041"], hand: [{ card: "BT8-108", as: "option" }], deck: ["BT8-001", "BT8-002", "BT8-003"] },
    ...
    expect(s.state.players[0]!.trash.map((card) => card.cardId)).toEqual(expect.arrayContaining(["BT8-001", "BT8-002"]));
    expect(s.state.players[0]!.hand.map((card) => card.cardId)).toContain("BT8-003");
  });

  it("places itself in its owner's battle area from Security", async () => {
    const s = setupEngine({
      0: { security: [{ card: "BT8-108", as: "option", faceUp: true }] },
    }, { autoSelectCards: true });
