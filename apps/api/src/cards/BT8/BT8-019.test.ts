...
describe("BT8-019 Zhuqiaomon", () => {
  it("keeps itself and the opponent's chosen Digimon, deletes all others and gains memory per deletion", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "AD1-002", as: "base" }, { card: "BT1-009", as: "ally" }], hand: [{ card: "BT8-019", as: "evolving" }] }, 1: { battleArea: [{ card: "BT1-010", as: "spared" }, { card: "BT1-011", as: "deletedOne" }, { card: "BT1-012", as: "deletedTwo" }] } }, { autoSelectCards: true });
    s.state.memory = 4;
