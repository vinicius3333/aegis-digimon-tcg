...
describe("BT8-079 SkullSatamon", () => {
  ...
  it("inherits once-per-turn memory when its deck is trashed", async () => {
    ...
    s.state.memory = 0;
    await advance(s.engine).fireSubTrigger("onDiscardLibrary", { addedToHand: { instanceIds: [instance("BT1-009", 0, false).instanceId], byEffect: { ownerSeat: 0, isDigimonEffect: false } } });
    expect(s.state.memory).toBe(1);
    await advance(s.engine).fireSubTrigger("onDiscardLibrary", { addedToHand: { instanceIds: [instance("BT1-010", 0, false).instanceId], byEffect: { ownerSeat: 0, isDigimonEffect: false } } });
    expect(s.state.memory).toBe(1);
  });
});
