...
describe("BT3-007 Agumon", () => {
  it("plays with its catalogued 4000 DP and has no effect resolution", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "BT3-007", as: "agumon" }] } });
    s.state.memory = 2;
  ...
  it("digivolves from a red level 2 Digi-Egg for 1 memory", async () => {
    const s = setupEngine({
      0: {
        battleArea: [{ card: "BT3-001", as: "base" }],
        hand: [{ card: "BT3-007", as: "agumon" }],
