...
describe("BT3-105 Breath of the Gods", () => {
  it("grants Reboot and protection from DP reduction and returns", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT3-059", as: "target" }], hand: [{ card: "BT3-105", as: "option" }] } }, { autoSelectCards: true });
    s.state.memory = 4;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("option").instanceId })).toEqual({ ok: true });
    await settle(() => observe(s.engine).hasKeyword(s.perm("target"), "Reboot") && observe(s.engine).isRestricted(s.perm("target"), "dpImmune") && observe(s.engine).isRestricted(s.perm("target"), "beReturned"));
  ...
  it("prevents the opponent's Digimon from attacking players from security", async () => {
    const s = setupEngine({ 0: { security: [{ card: "BT3-105", as: "securityOption", faceUp: true }] }, 1: { battleArea: [{ card: "BT3-059", as: "opponent" }] } });
