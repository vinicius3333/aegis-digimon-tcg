...
describe("BT3-103 Hidden Potential Discovered!", () => {
  it("suspends a Digimon to reduce the next green digivolution cost by 5", async () => {
    ...
    const s = setupEngine({ 0: { battleArea: [{ card: "BT3-054", as: "base" }, { card: "BT3-044", as: "payer" }], hand: [{ card: "BT3-103", as: "option" }, { card: "BT3-057", as: "evolving" }] } }, { autoAcceptOptional: true, autoSelectCards: true, preferInstanceIds: preferred });
