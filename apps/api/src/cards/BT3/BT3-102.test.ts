...
describe("BT3-102 Code Cracking", () => {
  it("lets the opponent trash their top security card", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT3-032"], hand: [{ card: "BT3-102", as: "option" }], deck: ["BT3-033"] }, 1: { security: [{ card: "BT3-034", as: "security" }] } }, { autoAcceptOptional: true });
    s.state.memory = 6;
  ...
  it("recovers 1 when the opponent declines to trash security", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT3-032"], hand: [{ card: "BT3-102", as: "option" }], deck: [{ card: "BT3-033", as: "recovered" }] }, 1: { security: ["BT3-034"] } }, { autoDeclineOptional: true });
    s.state.memory = 6;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("option").instanceId })).toEqual({ ok: true });
