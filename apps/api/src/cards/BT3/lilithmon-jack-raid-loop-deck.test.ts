...
describe("BT3 Lilithmon Jack Raid loop deck gauntlet", () => {
  it("returns two exact Option copies and grants memory only for the first use", async () => {
    ...
    const recursionRequest = s.decisions.find(
      ({ req }) => req.decisionId === recursionDecision?.decisionId,
    )?.req;
    ...
    await settle(() =>
      s.state.players[0]!.trash.some(({ instanceId }) => instanceId === firstJackRaidId) &&
      s.state.memory === 2,
    );
    ...
    await settle(() =>
      s.state.players[0]!.trash.some(({ instanceId }) => instanceId === secondJackRaidId),
    );
