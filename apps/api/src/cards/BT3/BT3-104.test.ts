...
describe("BT3-104 Positron Laser", () => {
  it("prevents attacks and blocks, then returns a suspended stack with blue present", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT3-020", "BT3-044"], hand: [{ card: "BT3-104", as: "option" }] }, 1: { battleArea: [{ card: "BT3-045", as: "returned", suspended: true, under: [{ card: "BT3-044", as: "source" }] }, { card: "BT3-046", as: "restricted" }] } }, { autoSelectCards: true });
    s.state.memory = 8;
  ...
  it("applies the Security attack restriction and return clause", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT3-020"], security: [{ card: "BT3-104", as: "securityOption", faceUp: true }] }, 1: { battleArea: [{ card: "BT3-045", as: "returned", suspended: true }, { card: "BT3-046", as: "restricted" }] } }, { autoSelectCards: true });
