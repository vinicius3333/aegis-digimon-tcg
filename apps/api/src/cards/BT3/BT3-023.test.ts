...
describe("BT3-023 Angemon", () => {
  it("trashes the bottom digivolution card of an opposing Digimon when attacking", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT3-026", as: "host", under: ["BT3-023"] }] }, 1: { battleArea: [{ card: "BT1-019", as: "target", under: ["BT1-010"] }], security: ["BT1-011"] } }, { autoSelectCards: true });
