...
describe("BT3-100 Desperado Blaster", () => {
  it("trashes two bottom sources and suspends the now-sourceless Digimon with green present", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT3-020", "BT3-044"], hand: [{ card: "BT3-100", as: "option" }] }, 1: { battleArea: [{ card: "BT3-020", as: "target", under: [{ card: "BT3-021", as: "first" }, { card: "BT3-022", as: "second" }] }] } }, { autoSelectCards: true });
    s.state.memory = 5;
  ...
  it("activates its full Main effect from security", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT3-020", "BT3-044"], security: [{ card: "BT3-100", as: "securityOption", faceUp: true }] }, 1: { battleArea: [{ card: "BT3-020", as: "target", under: ["BT3-021", "BT3-022"] }] } }, { autoSelectCards: true });
