...
describe("ST6-16 Nail Bone", () => {
  it("plays a purple level 3 and level 4 from trash without their On Play effects", async () => {
    const s = setupEngine({ 0: { battleArea: ["ST6-03"], hand: [{ card: "ST6-16", as: "option" }], deck: ["ST6-01", "ST6-03"], trash: [{ card: "ST6-04", as: "lv3" }, { card: "ST6-08", as: "lv4" }, { card: "ST6-15", as: "wouldReturn" }] } }, { autoSelectCards: true });
    s.state.memory = 10;
  ...
  it("may play only the available purple level 3 and ignores invalid trash cards", async () => {
    ...
    await settle(() =>
      s.state.players[0]!.battleArea.some((permanent) =>
        permanent.topCard.instanceId === s.inst("onlyValid").instanceId
      )
    );
