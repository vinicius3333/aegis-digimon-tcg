...
describe("ST6 purple deletion and revival toolbox deck", () => {
  it("keeps duplicate permanent choices distinct, gains Matt memory, and suppresses revived On Play", async () => {
    ...
    expect(new Set(costDecision.options?.candidateInstanceIds)).toEqual(new Set([
      s.perm("firstDracmon").permanentId,
      s.perm("secondDracmon").permanentId,
    ]));
    expect(costDecision.options?.candidateInstanceIds).not.toContain(
      s.perm("firstDracmon").topCard.instanceId,
    );
    expect(s.engine.applyIntent(0, {
    ...
    expect(new Set(targetDecision.options?.candidateInstanceIds)).toEqual(new Set([
      s.perm("firstEnemy").permanentId,
      s.perm("secondEnemy").permanentId,
    ]));
    expect(s.engine.applyIntent(0, {
      type: "respondDecision",
    ...
    await settle(() =>
      ...
      s.state.players[0]!.trash.some(
        ({ instanceId }) => instanceId === firstDracmonInstanceId,
      ) &&
      s.state.players[1]!.trash.some(
        ({ instanceId }) => instanceId === secondEnemyInstanceId,
      ) &&
    ...
    expect(s.state.players[0]!.trash.some(
      ({ instanceId }) => instanceId === deathClawId,
    )).toBe(true);
    expect(s.state.players[0]!.battleArea.some(
      ({ permanentId }) => permanentId === s.perm("secondDracmon").permanentId,
    )).toBe(true);
    expect(s.state.players[1]!.battleArea.some(
      ({ permanentId }) => permanentId === s.perm("firstEnemy").permanentId,
    )).toBe(true);
    ...
    expect(s.state.players[0]!.battleArea.some(
      ({ topCard }) => topCard.instanceId === firstDracmonInstanceId,
    )).toBe(true);
    expect(s.state.players[0]!.trash.some(
      ({ instanceId }) => instanceId === deathClawId,
    )).toBe(true);
