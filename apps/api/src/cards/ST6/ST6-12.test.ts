...
describe("ST6-12 VenomMyotismon", () => {
  it("gives up to 2 of your Digimon Retaliation through the opponent's next turn", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "ST6-11", as: "base" }, { card: "ST6-11", as: "ally" }], hand: [{ card: "ST6-12", as: "evolving" }, { card: "ST6-12", as: "allyEvolution" }] } }, { autoSelectCards: true });
    s.state.memory = 10;
