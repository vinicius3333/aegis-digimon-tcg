...
describe("EX2-061 Henry Wong", () => {
  it("may suspend to suspend an opponent when Gargomon or Rapidmon attacks", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX2-027", as: "attacker" }, { card: "EX2-061", as: "henry" }] }, 1: { battleArea: [{ card: "EX2-014", as: "target" }], security: ["BT1-001"] } }, { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true });
    await s.ready();
