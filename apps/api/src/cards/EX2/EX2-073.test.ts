...
describe("EX2-073 Gallantmon: Crimson Mode", () => {
  it("deletes every opposing Digimon tied for highest DP when digivolving", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX2-011", as: "base" }], hand: [{ card: "EX2-073", as: "evolution" }] }, 1: { battleArea: ["EX2-029", "EX2-043", "EX2-019"] } }, { autoSelectCards: true, autoOrderTriggers: true });
    s.state.memory = 10;
  ...
  it("uses the Gallantmon X line to reach 10 cards in trash and remove 2 security before the check", async () => {
    ...
    s.state.memory = 6;
    const highestIds = [
      s.perm("highestA").topCard.instanceId,
      s.perm("highestB").topCard.instanceId,
    ];
    ...
    await settle(() => highestIds.every((instanceId) =>
