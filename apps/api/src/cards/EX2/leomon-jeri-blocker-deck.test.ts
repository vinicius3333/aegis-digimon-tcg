  ...
  it("plays one duplicate Leomon, draws on attack, then converts a real block deletion into memory and a second draw", async () => {
    ...
    await settle(() => {
      ...
      return s.decisions.length > priorDecisionCount &&
        req?.sourceCardId === "EX2-058" &&
        req.kind === "selectCards";
    });

    const playDecision = s.decisions.at(-1)!.req;
    expect(new Set(playDecision.options?.candidateInstanceIds)).toEqual(
      new Set([firstLeomonId, secondLeomonId]),
    );
    expect(s.engine.applyIntent(0, {
    ...
    await settle(() =>
      s.state.players[0]!.battleArea.some(
        ({ topCard }) => topCard.instanceId === secondLeomonId,
      ) &&
      s.state.memory === 6 &&
    ...
    const leomon = s.state.players[0]!.battleArea.find(
      ({ topCard }) => topCard.instanceId === secondLeomonId,
    );
    expect(leomon).toBeDefined();
    const leomonPermanentId = leomon!.permanentId;
    const newJeri = s.state.players[0]!.battleArea.find(
      ({ topCard }) => topCard.instanceId === newJeriId,
    );
    ...
    await settle(() =>
      s.events.filter(({ kind }) => kind === "blockWindowOpened").length > blockWindowCount &&
      newJeri!.isSuspended,
    );
    ...
    expect(s.state.players[0]!.hand.filter(
      ({ instanceId }) => startingDrawIds.has(instanceId),
    )).toHaveLength(2);
