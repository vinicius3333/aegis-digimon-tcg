...
describe("EX2-053 ADR-08 Optimizer", () => {
  it("reveals three and plays a cost-10-or-lower D-Reaper with a loaded Mother", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX2-007", as: "mother", under: ["EX2-046", "EX2-046", "EX2-046", "EX2-046", "EX2-046"] }], hand: [{ card: "EX2-053", as: "optimizer" }], deck: [{ card: "EX2-050", as: "played" }, "BT1-001", "BT1-002"] } }, { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true });
    s.state.memory = 20;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("optimizer").instanceId })).toEqual({ ok: true });
