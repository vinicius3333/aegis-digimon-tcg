...
describe("EX2-063 Kazu Shioda", () => {
  it("may suspend when a Machine becomes suspended to draw 1 then trash 1", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX2-031", as: "machine" }, { card: "EX2-063", as: "kazu" }], hand: ["BT1-001"], deck: ["BT1-002"] }, 1: { security: ["BT1-003"] } }, { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true });
    await s.ready();
