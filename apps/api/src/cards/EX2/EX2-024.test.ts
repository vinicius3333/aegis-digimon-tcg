...
describe("EX2-024 Sakuyamon", () => {
  it("unsuspends a Digimon and returns one Plug-In Option per Tamer when digivolving", async () => {
    ...
    const s = setupEngine({ 0: { battleArea: [{ card: "EX2-023", as: "base", suspended: true }, { card: "EX2-019", as: "ally", suspended: true }, "EX2-060"], hand: [{ card: "EX2-024", as: "evolution" }], trash: [{ card: "EX2-066", as: "plugin" }] } }, { autoSelectCards: true, autoOrderTriggers: true, preferInstanceIds });
    ...
    await settle(() => (!s.perm("ally").isSuspended || !s.perm("base").isSuspended) && s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("plugin").instanceId));
