...
describe("EX2-056 Takato Matsuki", () => {
  it("may suspend to gain 1 memory when an opposing Digimon is deleted", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX2-009", as: "attacker" }, { card: "EX2-056", as: "takato" }] }, 1: { battleArea: [{ card: "EX2-019", as: "target", dp: 1000 }], security: ["BT1-001"] } }, { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true });
    s.state.memory = 3;
    await s.ready();
