...
describe("EX2 D-Reaper historical deck gauntlet", () => {
  it("converts Mother's Searchers into a free Rush Reaper, repeated attacks, inherited DP, and Gatekeeper recovery", async () => {
    ...
    await settle(() => {
      const reaper = s.state.players[0]!.battleArea.find((permanent) => permanent.topCard?.cardId === "EX2-055");
      return (
        reaper !== undefined && s.perm("mother").stack.length === 1 && observe(s.engine).hasKeyword(reaper, "Rush")
      );
    });
