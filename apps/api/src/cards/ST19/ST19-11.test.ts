...
describe("ST19-11 Chaperomon", () => {
  it("reduces one opposing Digimon by 3000 with fewer than three total Digimon", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "ST19-11", as: "chap" }] }, 1: { battleArea: [{ card: "BT1-010", as: "target", dp: 7000 }] } });
    await advance(s.engine).fire(EffectTiming.OnPlay, s.perm("chap"));
  ...
  it("reduces the same target by 6000 when both players have three total Digimon", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "ST19-11", as: "chap" }, { card: "BT1-010", as: "ally" }] },
