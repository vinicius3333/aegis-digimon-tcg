...
describe("ST19-06 Doggymon", () => {
    ...
    const s = setupEngine(
      {
        ...
        1: {
          battleArea: [
            { card: "BT1-010", as: "targetA" },
            { card: "BT1-011", as: "targetB" },
          ],
        },
      },
  ...
  it("applies the same effect on deletion and matches the catalog text", () => {
    expect(getCardDefinition("ST19-06")).toMatchObject({
      effectText:
        "[On Play] [On Deletion] 1 of your opponent's Digimon gains ＜Security Attack -1＞until the end of their turn.",
    });
  });
  ...
  it("triggers the Security Attack reduction when deleted in a real battle", async () => {
    ...
    await settle(
      () =>
        !s.state.players[1]!.battleArea.some((permanent) => permanent.permanentId === s.perm("doggy").permanentId) &&
