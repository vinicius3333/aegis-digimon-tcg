...
describe("EX6-041 Infermon", () => {
  ...
  it("publicly deletes a Diaboromon to digivolve Infermon into one from hand for free", async () => {
    const s = setupEngine(
      {
        0: {
          ...
          hand: [{ card: "EX6-041", as: "infermon" }, { card: "BT17-059", as: "diaboromon" }],
        },
      },
    ...
    await settle(() => s.state.players[0]!.battleArea.some((perm) => perm.topCard?.instanceId === s.inst("diaboromon").instanceId));
    expect(s.state.players[0]!.battleArea.some((perm) => perm.topCard?.instanceId === s.inst("diaboromon").instanceId)).toBe(true);
