...
describe("EX6-066 Sea of Destruction", () => {
  ...
  it("allows the place cost to resolve before comparing the placed card's level", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "EX6-014", as: "host" }], hand: [{ card: "EX6-066", as: "option" }, { card: "BT1-033", as: "placed" }] },
        1: { battleArea: [{ card: "BT1-033", as: "sameOne" }, { card: "BT1-033", as: "sameTwo" }, { card: "BT1-009", as: "different" }] },
      },
    ...
    expect(s.state.players[1]!.battleArea.map((perm) => perm.topCard?.instanceId)).toEqual([s.inst("different").instanceId]);
  });
