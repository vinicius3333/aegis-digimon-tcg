...
describe("EX6-065 Kokomon", () => {
  ...
  it("activates Main from security and revives a Legend-Arms card when one of your Digimon leaves", () =>
    expect(compiled.effects?.find((entry) => entry.trigger === "Security")?.actions[0]?.kind).toBe("ActivateMain"));
});
