...
export const compiled: CompiledCard = {
  ...
  effects: generated.effects.map((effect) => ({
    ...
    actions: effect.actions.map((action) => {
      if (action.kind === "Delete") return { ...action, target: { ...action.target, upTo: true }, trackCount: deletedCountSource };
      if (action.kind === "SecurityManipulation" && action.op === "trashTop") return { ...action, amount: undefined, amountFromNamedCount: { base: 7, countSource: deletedCountSource, per: -1, floor: 0 } };
      return action;
    })
  })),
  coverage: "full",
