...
export const compiled: CompiledCard = {
  ...
  effects: generated.effects.map((effect) => {
    if (effect === generatedMain) {
      return {
        ...
        actions: [{
          ...
          target: { filter: { controller: "opponent", kind: ["Digimon"], relativeTo: { attr: "dp", op: "lte", selectionRef: "placementTarget" } }, count: 1 },
          ...
          additionalCosts: [{
            ...
            underOrFilters: [{ controller: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Legend-Arms"], match: "trait" }] }],
    ...
    if (effect === generatedDisable) {
      return {
        ...effect,
        actions: [{ kind: "DisableSecurityEffect", target: { filter: { isSelfRef: true }, count: 1, isSelf: true }, sourceKind: "any", duration: "untilEachTurnEnd", condition: { kind: "selfHasNameContaining", names: ["RagnaLoardmon"], raw: "this Digimon is [RagnaLoardmon]" }, raw: "the [Security] effects on cards checked by this Digimon don't activate" }]
      };
    }
    return effect;
  }),
