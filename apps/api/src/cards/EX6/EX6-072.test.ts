...
describe("EX6-072 Mega Digimon Assembly!", () => {
  ...
  it("publicly performs the Main DNA digivolution with a level 6 field material and hand material", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX6-056", as: "fieldMaterial" }, { card: "BT11-095", as: "whiteSource" }], hand: [{ card: "EX6-072", as: "option" }, { card: "BT1-009", as: "handMaterial" }, { card: "EX6-062", as: "result" }] }, 1: { battleArea: [{ card: "EX6-056", as: "opponentLevel6" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 10;
    await s.ready();
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("option").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.battleArea.some((perm) => perm.topCard?.instanceId === s.inst("result").instanceId));
    expect(s.state.players[0]!.battleArea.some((perm) => perm.topCard?.instanceId === s.inst("result").instanceId)).toBe(true);
  });
});
