...
describe("BT2-044 Tyrannomon", () => {
  ...
  it("Q1016 adds the one available category when both categories are not revealed", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [{ card: "BT2-042", as: "base" }],
    ...
    expect(s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("greenTamer").instanceId)).toBe(true);
    expect(s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("level6").instanceId)).toBe(false);
    expect(s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("redTamer").instanceId)).toBe(false);
  ...
  it("Q1017 accepts a level 4 Digimon at the level-5-or-lower boundary", async () => {
    ...
    expect(choice.options?.candidateInstanceIds).toEqual(
      expect.arrayContaining([level4Id, s.inst("level5").instanceId]),
    );
    expect(
      s.engine.applyIntent(0, {
