...
describe("BT2-096 Kaiser Nail", () => {
  it("returns an opposing level 5 or lower Digimon and unsuspends yours with a blue Tamer", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT2-021", as: "mine", suspended: true }, "BT2-085"], hand: [{ card: "BT2-096", as: "option" }] }, 1: { battleArea: [{ card: "BT2-045", as: "target", under: [{ card: "BT2-001", as: "source" }] }] } }, { autoSelectCards: true });
    s.state.memory = 7;
  ...
  it("activates its full Main effect from security", async () => {
    const s = setupEngine({ 0: { security: [{ card: "BT2-096", as: "securityOption", faceUp: true }], battleArea: [{ card: "BT2-021", as: "mine", suspended: true }, "BT2-085"] }, 1: { battleArea: [{ card: "BT2-045", as: "target", under: [{ card: "BT2-001", as: "source" }] }] } }, { autoSelectCards: true });
