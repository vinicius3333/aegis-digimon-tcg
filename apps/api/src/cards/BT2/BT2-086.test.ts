...
describe("BT2-086 Rina Shinomiya", () => {
  it("may suspend itself to give an attacking blue Digimon +1000 DP", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT2-086", as: "rina" }, { card: "BT2-021", as: "attacker" }] }, 1: { security: ["BT1-001", "BT1-002"] } }, { autoAcceptOptional: true });
  ...
  it("adds a revealed Digimon with Vee in its name to hand", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "BT2-086", as: "source" }], deck: [
      { card: "BT2-021", as: "veemon" }, "BT2-022", "BT2-023",
