...
describe("BT2-069 Gabumon", () => {
  ...
  it("does not activate while Gabumon is the top card instead of a digivolution card", async () => {
    const s = setupEngine({
      0: {
    ...
    await advance(s.engine).verb.deletePermanent([s.perm("gabumon").permanentId]);
  ...
  it("proves the legal purple hatch stack, turn cycle, move, and public battle deletion", async () => {
    ...
    const s = setupEngine(
      {
        0: {
          ...
          hand: [
            { card: "BT2-069", as: "gabumon" },
            { card: "BT2-074", as: "host" },
        ...
        1: {
          battleArea: [{ card: "BT2-083", as: "attacker" }],
          security: ["BT1-010"],
    ...
    for (const alias of ["gabumon", "host"] as const) {
      expect(
        s.engine.applyIntent(0, {
          type: "digivolve",
