...
describe("BT2-083 Millenniummon", () => {
  it("returns an opposing Digimon to deck bottom and trashes its sources", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT2-066", as: "base" }], hand: [{ card: "BT2-083", as: "evolving" }] }, 1: { deck: ["BT1-010"], battleArea: [{ card: "BT2-020", under: ["BT2-013"], as: "target" }] } }, { autoSelectCards: true });
  ...
  it("may play itself from trash after being deleted with digivolution cards", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT2-083", as: "millenniummon", under: ["BT2-066", "BT2-057"] }] } }, { autoAcceptOptional: true });
