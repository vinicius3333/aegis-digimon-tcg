...
describe("AD1-004 WarGreymon", () => {
  ...
  it("allows Greymon-name, ADVENTURE, and Hero level-5 digivolution routes for cost 3", async () => {
    for (const baseCardId of ["BT1-021", "ST20-04", "AD1-011"]) {
      ...
      expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("wargreymon").instanceId })).toEqual({ ok: true });
  ...
  it("has no Security Attack bonus with only two distinct Tamer colors", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "AD1-004", dp: 12000, as: "wargreymon" }, { card: "AD1-019", as: "twoColorTamer" }] } });
    await s.ready();
  ...
  it("may make one of its Digimon attack at the end of the turn", async () => {
    const s = setupEngine(
      { 0: { battleArea: [{ card: "AD1-004", as: "wargreymon" }, { card: "BT1-010", as: "attacker" }] }, 1: { security: ["BT1-001"] } },
    ...
    const s = setupEngine(
      {
        ...
        1: { battleArea: [{ card: "BT1-010", dp: 7000, as: "eligible" }, { card: "BT1-010", dp: 8000, as: "tooLarge" }], security: ["BT1-001"] },
      },
      { autoSelectCards: true },
    );
  ...
  it("activates but cannot delete when inherited by a non-Greymon or Omnimon attacker, per Q6055", async () => {
    const s = setupEngine(
      { 0: { battleArea: [{ card: "AD1-011", dp: 8000, as: "attacker", under: ["AD1-004"] }] }, 1: { battleArea: [{ card: "BT1-010", dp: 3000, as: "wouldBeEligible" }], security: ["BT1-001"] } },
      { autoSelectCards: true },
    );
    ...
    expect(s.state.players[1]!.battleArea.some((permanent) => permanent.permanentId === s.perm("wouldBeEligible").permanentId)).toBe(true);
  });
