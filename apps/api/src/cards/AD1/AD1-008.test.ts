...
describe("AD1-008 Gallantmon", () => {
  ...
  it("uses either printed alternate level-5 route for cost 3", async () => {
    for (const baseCard of ["BT9-014", "AD1-011"]) {
      ...
      expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("gallantmon").instanceId, alternateRequirementIndex: baseCard === "BT9-014" ? 0 : 1 })).toEqual({ ok: true });
