...
describe("AD1-003 WarGrowlmon", () => {
  ...
  it("allows both printed level-4 Growlmon-name and Hero digivolution routes for cost 3", async () => {
    for (const baseCardId of ["BT12-010", "BT21-066"]) {
      const s = setupEngine({ 0: { battleArea: [{ card: baseCardId, as: "base" }], hand: [{ card: "AD1-003", as: "wargrowlmon" }], deck: ["BT1-001"] } });
      s.state.memory = 3;
    ...
    const s = setupEngine(
      {
        ...
        1: { battleArea: [{ card: "BT1-010", as: "eligible", dp: 6000 }, { card: "BT1-010", as: "tooLarge", dp: 7000 }] },
      },
      { autoSelectCards: true, autoAcceptOptional: true },
    );
    s.state.memory = 7;

    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("wargrowlmon").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.battleArea.some((permanent) => permanent.topCard?.cardId === "BT12-089") && s.state.players[1]!.battleArea.length === 1);
  ...
  it("plays both Takato and Guilmon when an inherited holder leaves in battle, per Q6053", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "AD1-008", dp: 12000, suspended: true, as: "gallantmon", under: ["BT12-089", "BT12-007", "AD1-003"] }] },
  ...
  it("plays the sole available Takato when no Guilmon is in the stack, per Q6054", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "AD1-008", dp: 12000, suspended: true, as: "gallantmon", under: ["BT12-089", "AD1-003"] }] },
        1: { battleArea: [{ card: "AD1-005", dp: 13000, as: "attacker" }] },
      },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
    ...
    expect(s.engine.applyIntent(1, { type: "attack", attackerPermanentId: s.perm("attacker").permanentId, target: { kind: "permanent", permanentId: s.perm("gallantmon").permanentId } })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.battleArea.some((permanent) => permanent.topCard?.cardId === "BT12-089"), 5000);
