...
describe("AD1-006 Shoutmon X7", () => {
  ...
  it("publishes and uses all six exact DigiXros slots at reduction 2 each", async () => {
    expect(digiXrosRequirementFor("AD1-006")).toEqual([
      {
        materials: ["OmniShoutmon", "ZeigGreymon", "Ballistamon", "Dorulumon", "Starmons", "Sparrowmon"].map((name) => ({ names: [name] })),
        count: 2,
      },
    ]);
    ...
    s.state.memory = 1;
    const materialInstanceIds = ["omniShoutmon", "zeigGreymon", "ballistamon", "dorulumon", "starmons", "sparrowmon"].map((alias) => s.inst(alias).instanceId);

    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("x7").instanceId, digiXros: { materialInstanceIds } })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.battleArea.some((permanent) => permanent.topCard?.cardId === "AD1-006" && permanent.stack.length === 6));

    expect(s.state.memory).toBe(0);
    expect(s.state.players[0]!.battleArea.find((permanent) => permanent.topCard?.cardId === "AD1-006")?.stack).toHaveLength(6);
  });
  ...
  it("allows level-6 Xros Heart and Blue Flare digivolution routes for cost 2", async () => {
    for (const baseCardId of ["BT10-015", "BT19-026"]) {
      const s = setupEngine({ 0: { battleArea: [{ card: baseCardId, as: "base" }], hand: [{ card: "AD1-006", as: "x7" }], deck: ["BT1-001"] } });
      s.state.memory = 2;

      expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("x7").instanceId })).toEqual({ ok: true });
    ...
    const s = setupEngine(
      {
        ...
        1: { battleArea: [{ card: "BT1-010", dp: 13000, as: "eligible" }, { card: "BT1-010", dp: 14000, as: "tooLarge" }], security: ["BT1-001"] },
      },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
  ...
  it("with no Tamer, may play a qualifying source card and rejects non-matching sources, per Q6059/Q6063", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "AD1-006", dp: 13000, suspended: true, as: "x7", under: [{ card: "BT10-009", as: "xrosHeart" }, { card: "BT1-001", as: "invalid" }] }] },
  ...
  it("uses newly placed cards and itself as DigiXros materials for the played card, per Q6060/Q6061", async () => {
    ...
    expect(played.stack.map((card) => card.instanceId)).toEqual(expect.arrayContaining([
      x7InstanceId,
      s.inst("ballistamon").instanceId,
      s.inst("dorulumon").instanceId,
      s.inst("starmons").instanceId,
  ...
  it("may decline the leave effect and lets the whole stack go to trash", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "AD1-006", dp: 13000, suspended: true, as: "x7", under: [{ card: "BT10-009", as: "source" }] }] },
        1: { battleArea: [{ card: "BT1-010", dp: 14000, as: "attacker" }] },
      },
