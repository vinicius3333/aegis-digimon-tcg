...
describe("AD1-014 MetalGarurumon", () => {
  ...
  it("uses all three printed alternate level-5 routes for cost 3", async () => {
    for (const baseCard of ["BT1-040", "ST21-04", "AD1-011"]) {
      ...
      expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("metal").instanceId, digivolutionRequirementIndex: baseCard === "BT1-040" ? 0 : baseCard === "ST21-04" ? 1 : 2 })).toEqual({ ok: true });
