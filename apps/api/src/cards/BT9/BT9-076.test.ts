...
describe("BT9-076 Maycrackmon: Vicious Mode", () => {
  it("matches catalog and Q1869/Q1870 discarded-card and effective-color IR", () => {
    expect(getCardDefinition("BT9-076")).toMatchObject({
      cardId: "BT9-076",
      nameEn: "Maycrackmon: Vicious Mode",
    ...
    for (const trigger of ["OnPlay", "WhenDigivolving"]) {
      expect(compiled.effects.find((effect) => effect.trigger === trigger)).toMatchObject({
        actions: [
          { kind: "Trash", bindResultAs: "discardedCard", optional: true },
          { kind: "Delete", condition: { kind: "bindingContains" } },
          { kind: "ModifyDP", amount: -3000, condition: { kind: "bindingContains" } },
        ],
      });
    }
    expect(compiled).toMatchObject({
      ...
      effects: [
        ...
        expect.anything(),
        {
          trigger: "OnDeletion",
          isInherited: true,
          actions: [{ kind: "GainMemory", amount: 2, condition: { kind: "selfColorCount", value: 2 } }],
        },
      ],
    });
  });
  ...
  it("applies both branches when the discarded card is purple and yellow", async () => {
    ...
    preferred.push(s.inst("dualDiscard").instanceId, s.perm("level3").permanentId, s.perm("dpTarget").permanentId);
  ...
  it("does nothing after the controller declines to trash a hand card", async () => {
    ...
    await settle(() => s.state.players[0]!.battleArea.some((permanent) => permanent.topCard.cardId === "BT9-076"));
