...
describe("BT9-068 Gaiomon", () => {
  it("de-digivolves an opponent when it has a black digivolution card", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT10-013", as: "base" }], hand: [{ card: "BT9-068", as: "evolving" }] }, 1: { battleArea: [{ card: "BT2-047", as: "target", under: [{ card: "BT2-045", as: "bottom" }] }] } }, { autoSelectCards: true });
    s.state.memory = 5;
