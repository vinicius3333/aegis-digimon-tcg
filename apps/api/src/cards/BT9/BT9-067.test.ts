  ...
  it("matches errata, Q1854 partial placement, and Q1855 color thresholds in IR", () => {
    expect(getCardDefinition("BT9-067")).toMatchObject({
      ...
      evoCosts: [
        ...
        { color: "Black", level: 6, memoryCost: 2 },
      ],
      forms: ["Mega"],
      attributes: ["Virus"],
      types: ["Machine"],
      imageId: "BT9-067-Errata",
    });
    for (const trigger of ["OnPlay", "WhenDigivolving"]) {
      expect(compiled.effects.find((effect) => effect.trigger === trigger)).toMatchObject({
        actions: [
          ...
          { kind: "PlaceUnder" },
          { kind: "PlaceUnder" },
          { kind: "GainMemory", amount: 1, scaling: { unit: "placedCards" } },
        ],
      });
          ...
          actions: [
            {
              kind: "ModifyDP",
              amount: 3000,
              condition: { kind: "selfDigivolutionStackDistinctColorCount", value: 3 },
            },
            {
              kind: "DeDigivolve",
              amount: 1,
              condition: { kind: "selfDigivolutionStackDistinctColorCount", value: 4 },
            },
          ],
        },
      ],
