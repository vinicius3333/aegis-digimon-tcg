...
describe("BT9-105 Soul Digitalization", () => {
  it("uses the chosen revealed play cost, trashes the reveal, then places that X Antibody card", async () => {
    ...
    expect(referenceChoice.options?.candidateInstanceIds).toEqual([
    ...
    expect(s.state.players[1]!.battleArea.some(({ permanentId }) =>
      permanentId === s.perm("tooExpensive").permanentId
    )).toBe(true);
    expect(s.state.players[1]!.trash.some(({ instanceId }) =>
      instanceId === eligibleCardId
    )).toBe(true);
    expect(s.perm("host").stack.some(({ instanceId }) =>
      instanceId === s.inst("reference").instanceId
    )).toBe(true);
  ...
  it("still resolves the Then placement when no X Antibody card was revealed", async () => {
    ...
    await settle(() => s.perm("host").stack.some(({ instanceId }) =>
      instanceId === s.inst("existingX").instanceId
