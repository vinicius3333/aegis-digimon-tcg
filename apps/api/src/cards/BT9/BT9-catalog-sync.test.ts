...
const bt9Directory = fileURLToPath(new URL(".", import.meta.url));
...
const expectedCardIds = Array.from({ length: 112 }, (_, index) => `BT9-${String(index + 1).padStart(3, "0")}`);
const cardIds = committedCards
  .map(({ cardId }) => cardId)
  .filter((cardId) => cardId.startsWith("BT9-"))
  .sort();

describe("BT9 persisted IR", () => {
  it("has exactly the expected 112 cards in the committed card catalog", () => {
    expect(cardIds).toEqual(expectedCardIds);
  });

  it("has exactly the committed BT9 card IDs in the effects catalog", () => {
    expect(
      Object.keys(effects)
        .filter((cardId) => cardId.startsWith("BT9-"))
        .sort(),
  ...
  it("has exactly 112 IR-only production modules", () => {
    const productionFiles = readdirSync(bt9Directory)
      .filter((fileName) => /^BT9-\d{3}\.ts$/.test(fileName))
      .sort();
    expect(productionFiles).toEqual(cardIds.map((cardId) => `${cardId}.ts`));

    for (const cardId of cardIds) {
      const source = readFileSync(join(bt9Directory, `${cardId}.ts`), "utf8");
  ...
  it("has one focused test per card and no RawUnparsed or TypeScript suppressions", () => {
    const files = new Set(readdirSync(bt9Directory));

    for (const cardId of cardIds) {
      expect(files).toContain(`${cardId}.test.ts`);
      const source = readFileSync(join(bt9Directory, `${cardId}.ts`), "utf8");
      const testSource = readFileSync(join(bt9Directory, `${cardId}.test.ts`), "utf8");
