...
describe("BT9-107 Metal Impulse", () => {
  it("repeats De-Digivolve 1 on one chosen target, stops at level 3, then chooses a deletion", async () => {
    ...
    expect(new Set(dedigivolveChoice.options?.candidateInstanceIds)).toEqual(new Set([
      s.perm("dedigivolveTarget").permanentId,
    ...
    expect(new Set(deleteChoice.options?.candidateInstanceIds)).toEqual(new Set([
      s.perm("dedigivolveTarget").permanentId,
      s.perm("deleteTarget").permanentId,
    ]));
    expect(s.engine.applyIntent(0, {
    ...
    expect(s.state.players[1]!.battleArea.some(({ permanentId }) =>
      permanentId === s.perm("dedigivolveTarget").permanentId
    )).toBe(true);
  ...
  it("skips De-Digivolve when zero cards are discarded but still performs the Then deletion", async () => {
    ...
    expect(s.state.players[0]!.hand.some(({ instanceId }) =>
      instanceId === s.inst("kept").instanceId
    )).toBe(true);
  });
  ...
  it("stops repeated De-Digivolve at an exposed X Antibody and rule-trashes its remaining stack (Q1921)", async () => {
    ...
    await settle(() => !s.state.players[1]!.battleArea.some(
      ({ permanentId }) => permanentId === hostPermanentId,
    ));

    expect(s.state.players[1]!.trash.map(({ instanceId }) => instanceId)).toEqual(
      expect.arrayContaining(hostCardIds),
    );
  });
});
