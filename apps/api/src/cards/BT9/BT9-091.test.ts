...
describe("BT9-091 Meiko Mochizuki", () => {
  it("adds Meicoomon and trashes the other revealed cards", async () => {
    ...
    await settle(() =>
      player.hand.some((card) => card.instanceId === s.inst("meicoomon").instanceId) &&
      player.trash.length === 2,
    );
  ...
  it("suspends to gain 1 memory when Meicoomon is played from security", async () => {
    ...
    expect(s.state.players[0]!.battleArea.some((permanent) => permanent.topCard.instanceId === s.inst("meicoomon").instanceId)).toBe(true);
  });
  ...
  it("offers each of 2 Meikos exactly once for one two-color Digimon play", async () => {
    ...
    const meikoPrompts = s.decisions.filter(({ req }) =>
      req.kind === "optional" && req.sourceCardId === "BT9-091",
    );
  ...
  it("does not trigger for a purple and red 2-color Digimon", async () => {
    ...
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("wrongColors").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.battleArea.some((permanent) => permanent.topCard.instanceId === s.inst("wrongColors").instanceId));
  ...
  it("Security plays Meiko without paying its cost", async () => {
    ...
    expect(s.state.players[0]!.battleArea.some((permanent) => permanent.topCard.instanceId === s.inst("meiko").instanceId)).toBe(true);
