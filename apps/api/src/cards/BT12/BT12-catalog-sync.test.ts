...
const bt12Directory = fileURLToPath(new URL(".", import.meta.url));
...
const expectedCardIds = Array.from({ length: 112 }, (_, index) => `BT12-${String(index + 1).padStart(3, "0")}`);
const cardIds = committedCards
  .map(({ cardId }) => cardId)
  .filter((cardId) => cardId.startsWith("BT12-"))
  .sort();

describe("BT12 persisted IR", () => {
  it("has exactly the expected 112 cards in the committed card catalog", () => {
    expect(cardIds).toEqual(expectedCardIds);
  });

  it("has exactly the committed BT12 card IDs in the effects catalog", () => {
    expect(
      Object.keys(effects)
        .filter((cardId) => cardId.startsWith("BT12-"))
        .sort(),
  ...
  it("has exactly 112 IR-only production modules", () => {
    const productionFiles = readdirSync(bt12Directory)
      .filter((fileName) => /^BT12-\d{3}\.ts$/.test(fileName))
      .sort();
    expect(productionFiles).toEqual(cardIds.map((cardId) => `${cardId}.ts`));

    for (const cardId of cardIds) {
      const source = readFileSync(join(bt12Directory, `${cardId}.ts`), "utf8");
      ...
      expect(source).not.toMatch(/_(?:late|mid)Handwritten/);
