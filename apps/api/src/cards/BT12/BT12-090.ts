  ...
  return ctx.game.player(source.ownerSeat).battleArea.some((permanent) => {
    ...
    return (
      (isDigimon(definition) &&
        matchNameOrTrait(definition, { tokens: ["Free"], match: "trait" })) ||
      (isTamer(definition) &&
        matchNameOrTrait(definition, { tokens: ["Ken Ichijoji"], match: "name" }))
    );
  });
}

function isExactlyBlueGreen(ctx: EffectContext, permanent: Permanent): boolean {
  const colors = ctx.game.effectiveColors?.(permanent) ??
    ...
    if (timing === EffectTiming.OnAllyAttack) {
      return [whenAttacking({
        ...
        canActivate: (ctx) => {
          ...
          return self !== undefined && !self.isSuspended && attacker !== undefined &&
