...
if (watcher?.kind === "SubTrigger") {
  const missing = watcher.actions.findIndex((action) => action.kind === "RawUnparsed");
  if (missing >= 0) watcher.actions[missing] = {
    kind: "Digivolve",
    target: { sourceRef: "triggerSubject", filter: { kind: ["Digimon"] }, count: 1 },
    into: { controllerDefault: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Imperialdramon"], match: "name" }] },
    from: ["hand"],
    payCost: true,
    optional: true,
