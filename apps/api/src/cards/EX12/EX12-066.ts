...
const cardId = "EX12-066";

function hasGammamonOrVB(def: CardDefinition): boolean {
  return def.nameEn.includes("Gammamon") || (def.types ?? []).includes("VB");
}
        ...
        staticModifier({
          ...
          resolve: async (ctx) => {
            ...
            ctx.fx.subscribeSubTrigger({
              ...
              once: false,
              description: `${cardId}: When Gammamon/VB attacks, suspend + modal choice.`,
              matches: (subCtx) => {
                ...
                return isDigimon(def) && hasGammamonOrVB(def);
              },
              run: async (subCtx) => {
                ...
                const digivolveCards = Array.from(owner.hand).filter((c) => {
                  ...
                  return isDigimon(def) && hasGammamonOrVB(def) && (def.level ?? 99) <= 6;
                });
                const optionCards = Array.from(owner.hand).filter((c) => {
                  const def = subCtx.game.definitionOf(c);
                  return (def.kinds ?? []).includes(CardKind.Option) && hasGammamonOrVB(def);
                });
