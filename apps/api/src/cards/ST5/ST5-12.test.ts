...
describe("ST5-12 Machinedramon", () => {
  it("is fully represented as an up-to-two Reboot grant", () => {
    expect(runtimeCompiledCard("ST5-12")).toMatchObject({ coverage: "full", residual: [], effects: [{ trigger: "WhenDigivolving", actions: [{ kind: "GainKeyword", keyword: { keyword: "Reboot" }, duration: "untilOpponentTurnEnd", target: { count: 2, upTo: true } }] }] });
  });
