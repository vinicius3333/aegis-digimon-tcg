...
describe("ST5 Machinedramon Reboot/Blocker deck", () => {
  it("binds identical guards by permanent, grants two Reboots, and leaves the blocker suspended", async () => {
    ...
    expect(blockerDecision.options?.candidateInstanceIds).toEqual(
    ...
    expect(blockerDecision.options?.candidateInstanceIds).not.toContain(s.perm("firstGuard").topCard.instanceId);
    expect(
      s.engine.applyIntent(0, {
    ...
    expect(
      s.engine.applyIntent(0, {
        ...
        response: {
          kind: "chooseTargets",
          instanceIds: [s.perm("firstGuard").permanentId, s.perm("secondGuard").permanentId],
        },
      }),
    ).toEqual({ ok: true });
    await settle(
