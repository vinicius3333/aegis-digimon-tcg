...
describe("BT18-084 AncientSphinxmon", () => {
  it("deletes only an opponent's unsuspended Digimon and preserves the leave-play replacement", () => {
    ...
    expect(compiled.effects[0]).toMatchObject({ trigger: "OnPlay", actions: [{ kind: "Delete", target: { filter: { controller: "opponent", unsuspended: true, kind: ["Digimon"] } } }] });
    expect(compiled.effects[2]).toMatchObject({ trigger: "AllTurns", actions: [{ kind: "Replacement", event: "wouldLeavePlay", actions: [{ kind: "PlayWithoutCost", from: ["digivolutionCards"], payCost: false, optional: true, target: { filter: { levelComparison: { op: "lte", value: 4 }, nameOrTrait: [{ tokens: ["Dark Animal", "Mythical Beast", "Hybrid"], match: "trait" }] } } }] }] });
  });

  it("executes the On Play unsuspended-target boundary through the GameEngine", async () => {
    const s = setupEngine({
      0: { hand: [{ card: "BT18-084", as: "ancient" }] },
      1: { battleArea: [{ card: "BT1-009", as: "live" }, { card: "BT1-009", as: "suspended", suspended: true }] },
