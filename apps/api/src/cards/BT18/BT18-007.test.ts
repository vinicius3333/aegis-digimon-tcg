...
describe("BT18-007 Gazimon", () => {
  it("reveals three and adds one Millenniummon and one Composite/Wicked God card", async () => {
    ...
    const s = setupEngine({ 0: { hand: [{ card: "BT18-007", as: "gazimon" }], deck: [{ card: "BT18-019" }, { card: "BT19-075" }, { card: "BT1-009" }, { card: "BT1-010" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 10;
