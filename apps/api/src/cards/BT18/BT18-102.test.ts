...
describe("BT18-102 Susanoomon", () => {
  it("covers Blast Digivolve, both deletion timings, and the Hybrid stack rule", () => {
    ...
    expect(compiled.effects[1]).toMatchObject({
      ...
      actions: [
        { kind: "Delete", target: { filter: { dp: { op: "lte", value: 10000 } }, count: 1 } },
        { kind: "CostModifier", costType: "dpDeletion", amount: 2000 },
      ],
    });
    expect(compiled.effects[2]).toMatchObject({
      trigger: "WhenAttacking",
      actions: [{ kind: "Delete" }, { kind: "CostModifier", costType: "dpDeletion", amount: 2000 }],
    });
      ...
      actions: [
        {
          ...
          op: "placeAsSecurity",
          toTop: false,
          source: { filter: { kind: ["Tamer"], zone: "digivolutionCards" }, count: 5, upTo: true },
        },
