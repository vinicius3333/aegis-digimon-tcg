...
describe("BT18-095 Wind to Flame, Ice to Sword", () => {
  it("covers five Hybrid placements and the EmperorGreymon alternate digivolution", () => {
    ...
    expect(compiled.effects[0]).toMatchObject({
      ...
      actions: [
        ...
        { kind: "Digivolve", payCost: false, from: ["hand", "trash"], ignoreRequirements: true, into: { nameOrTrait: [{ tokens: ["EmperorGreymon"], match: "name" }] } },
      ],
    });
  });
  ...
  it("executes Security by playing an inherited-effect Tamer and returning this Option to hand", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "BT18-095", as: "option", faceUp: true }], hand: [{ card: "BT18-088", as: "tamer" }] } },
