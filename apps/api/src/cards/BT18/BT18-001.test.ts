...
describe("BT18-001 DemiMeramon", () => {
  it("deletes an opposing Digimon at 3000 DP or less when its host attacks with a red Tamer", async () => {
    ...
    expect(compiled.effects[0]).toMatchObject({ trigger: "WhenAttacking", isInherited: true, frequency: "OncePerTurn", actions: [{ kind: "Delete", target: { filter: { controller: "opponent", kind: ["Digimon"], dp: { op: "lte", value: 3000 } } }, condition: { kind: "youHave", filter: { kind: ["Tamer"], colors: ["Red"] } } }] });
    const s = setupEngine({ 0: { battleArea: [{ card: "BT1-030", as: "host", under: ["BT18-001"] }, { card: "BT1-085", as: "tamer" }] }, 1: { battleArea: [{ card: "BT1-030", dp: 3000, as: "small" }, { card: "BT1-030", dp: 4000, as: "large" }] } }, { autoSelectCards: true });
    await s.ready();
