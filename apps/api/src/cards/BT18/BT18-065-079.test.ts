...
describe("BT18 Vemmon/Velgrmon inherited and scaling clauses", () => {
  it("reacts only when a Vemmon leaves this stack for the bottom of the deck", () => {
    ...
    expect(inherited).toMatchObject({ trigger: "AllTurns", isInherited: true, frequency: "OncePerTurn", actions: [{ kind: "SubTrigger", event: "onDigivolutionCardReturnToDeckBottom", sourceFilter: { nameOrTrait: [{ tokens: ["Vemmon"], match: "name" }] }, actions: [{ kind: "Unsuspend" }, { kind: "GainKeyword", keyword: { keyword: "Blocker" } }] }] });
  });
  ...
  it("scales Velgrmon's DP by the cards trashed by this effect", () => {
    for (const trigger of ["OnPlay", "WhenDigivolving"]) {
      ...
      expect(effect?.actions?.[1]).toMatchObject({ kind: "ModifyDP", scaling: { unit: "namedCount", countSource: "trashedThisEffect" } });
    }
  });
});
