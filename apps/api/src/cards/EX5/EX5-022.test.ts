...
describe("EX5-022 Mihiramon", () => {
  ...
  it("trashes one digivolution card from an opposing Digimon when one of your Digimon is played", () => {
    expect(compiled.effects?.find((entry) => entry.trigger === "YourTurn")?.actions?.[0]).toMatchObject({ kind: "SubTrigger", event: "whenPlayed", sourceFilter: { controller: "mine", kind: ["Digimon"] }, actions: [{ kind: "TrashDigivolution", target: { filter: { controller: "opponent", digivolutionCards: "hasAny" } }, amount: 1, fromTop: true }] });
  });
});
