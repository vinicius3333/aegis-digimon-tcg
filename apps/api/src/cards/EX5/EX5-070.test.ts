...
describe("EX5-070 X Antibody Proto Form", () => {
  ...
  it("registers the inherited leave-field return and security placement effect", () => {
    ...
    expect(getEffectModule("EX5-070")!.effectsForTiming(EffectTiming.OnLeaveFieldAnyone, source)[0]?.isInherited).toBe(
      true,
    );
  });
  it("recognizes X Antibody by both name and the card's treated-as trait", () => {
    expect(
      getEffectModule("EX5-070")!.effectsForTiming(EffectTiming.OnUseOption, {
        instanceId: "source",
        cardId: "EX5-070",
        ownerSeat: 0,
        definition: {},
        ...
        isOwnersTurn: () => true,
        hasColor: () => true,
      } as never)[0]?.description,
    ).toContain("X Antibody");
  });
});
