...
describe("EX5-064 Koh & Sayo", () => {
  ...
  it("offers free evolution from hand with the compound suspend and Light Fang/Night Claw cost", () => {
    for (const trigger of ["OnPlay", "Main"] as const) {
      expect(compiled.effects?.find((entry) => entry.trigger === trigger)?.actions[0]).toMatchObject({ kind: "Digivolve", from: ["hand"], payCost: false, optional: true, cost: { kind: "compound", costs: [{ kind: "suspend" }, { kind: "placeOwnTopAtStackBottom", target: { filter: { nameOrTrait: [{ match: "trait", tokens: ["Light Fang", "Night Claw"] }] } } }] } });
    }
  });
});
