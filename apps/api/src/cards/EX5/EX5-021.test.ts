...
describe("EX5-021 Majiramon", () => {
  ...
  it("gains memory when using an Option costing at least one and as an inherited trait effect", () => {
    expect(compiled.effects?.find((entry) => entry.trigger === "YourTurn")?.actions?.[0]).toMatchObject({ kind: "SubTrigger", event: "whenOptionUsed", fireCondition: { kind: "triggerOptionCostAtLeast", value: 1 }, actions: [{ kind: "GainMemory", amount: 1 }] });
    expect(compiled.effects?.find((entry) => entry.trigger === "WhenAttacking")?.actions?.[0]).toMatchObject({ kind: "GainMemory", amount: 1, condition: { kind: "selfHasTrait" } });
  });
});
