...
describe("LM-031 Black Scramble", () => {
  it("returns black trash to deck top and plays a small black Digimon", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "LM-031", as: "option" }], trash: ["BT4-066", "BT4-063"] },
        1: { battleArea: ["BT4-063"] },
      },
    ...
    await settle(() => (s as ReturnType<typeof setupEngine> & { top?: string }).top === "BT4-066");
    expect((s as ReturnType<typeof setupEngine> & { top?: string }).top).toBe("BT4-066");
  ...
  it("does not activate when the opponent has no Digimon", async () => {
    const s = setupEngine(
      { 0: { battleArea: [{ card: "LM-031", as: "option" }], trash: ["BT4-063"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
    await s.ready();
    arm(s);
    const { turn } = await start(s);
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-031")).toBe(true);
    await finish(s, turn);
  });

  it("plays a qualifying black Digimon from security and returns itself to hand", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-031", as: "securityOption", faceUp: true }], trash: ["BT4-063"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
    await advance(s.engine).fireForInstance(EffectTiming.SecuritySkill, s.inst("securityOption"));
    await settle(() => s.state.players[0]!.hand.some((c) => c.cardId === "LM-031"));
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "BT4-063")).toBe(true);
    expect(s.state.players[0]!.hand.some((c) => c.cardId === "LM-031")).toBe(true);
  });
});
