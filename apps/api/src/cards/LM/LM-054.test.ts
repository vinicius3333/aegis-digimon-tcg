...
describe("LM-054 Treadmill Training", () => {
  it("reveals two, adds a yellow or black card, bottoms the rest and places itself", async () => {
    const s = setupEngine(
      {
        0: {
          hand: [{ card: "LM-054", as: "option" }],
          deck: ["BT1-047", "BT1-013"],
        },
      },
    ...
    await settle(() => s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-054"), 2000);
    ...
    const s = setupEngine(
      {
        ...
        0: { hand: [{ card: "LM-054", as: "option" }], deck: ["BT1-013", "BT1-013"] },
      },
  ...
  it("loses the waiver once a copy of itself is already in the battle area", async () => {
    ...
    s.state.memory = 2;
    await s.ready();
    s.putOnBoard(0, { card: "LM-054", as: "copy" });
  ...
  it("reveals two and places itself from security", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-054", as: "securityOption", faceUp: true }], deck: ["BT1-047", "BT1-013"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
  ...
  it("matches committed metadata and publishes fully covered compiled IR", () => {
    const definition = getCardDefinition("LM-054");
    const compiled = runtimeCompiledCard("LM-054");
    expect(definition?.nameEn).toBe("Treadmill Training");
    expect(definition?.colors).toEqual(["Yellow", "Black"]);
