...
describe("LM-058 Parkour Training", () => {
  it("reveals two, adds a blue or green card, bottoms the rest and places itself", async () => {
    const s = setupEngine(
      {
        0: {
          hand: [{ card: "LM-058", as: "option" }],
    ...
    await settle(() => s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-058"), 2000);
    ...
    const s = setupEngine(
      {
        ...
        0: { hand: [{ card: "LM-058", as: "option" }], deck: ["BT1-013", "BT1-013"] },
      },
  ...
  it("loses the waiver once a copy of itself is already in the battle area", async () => {
    ...
    s.state.memory = 2;
    await s.ready();
    s.putOnBoard(0, { card: "LM-058", as: "copy" });
  ...
  it("digivolves for the printed cost reduced by 2 through its Delay clause", async () => {
    ...
    expect(
      s.engine.applyIntent(0, {
        ...
        effectKey: `LM-058/ir-${EffectTiming.OnDeclaration}-0`,
      }),
    ).toEqual({ ok: true });
  ...
  it("reveals two and places itself from security", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-058", as: "securityOption", faceUp: true }], deck: ["BT1-028", "BT1-013"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
  ...
  it("matches committed metadata and publishes fully covered compiled IR", () => {
    const definition = getCardDefinition("LM-058");
    const compiled = runtimeCompiledCard("LM-058");
    expect(definition?.nameEn).toBe("Parkour Training");
    expect(definition?.colors).toEqual(["Blue", "Green"]);
