  ...
  effectsForTiming(timing: EffectTiming, source: CardSource): Effect[] {
    if (timing === EffectTiming.None) {
      return [
        staticModifier({
          source,
          effectKey: `${cardId}/black-color-waiver`,
          description:
            "You may also use this card if you have a black Digimon or Tamer in play, " +
            "treating it as also being a black card.",
          when: (ctx) => {
            const owner = ctx.game.player(source.ownerSeat);
            return Array.from(owner.battleArea).some((p) => {
              if (p.topCard === undefined) return false;
              const def = ctx.game.definitionOf(p.topCard);
              return (isDigimon(def) || isTamer(def)) && def.colors.includes(CardColor.Black);
            });
          },
          resolve: async () => {},
        }),
    ...
    if (timing === EffectTiming.OnUseOption) {
      return [
        activated({
          ...
          description:
            "[Main] Reveal the top 3 cards of your deck. Add 1 green or black Digimon card " +
          ...
          resolve: async (ctx) => {
            ...
            if (deckCards.length > 0) {
              const candidates = deckCards.filter((c) => {
                ...
                return isDigimon(def) && (def.colors.includes(CardColor.Green) || def.colors.includes(CardColor.Black));
              });
              let added: string[] = [];
