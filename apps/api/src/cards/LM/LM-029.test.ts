...
describe("LM-029 Yellow Scramble", () => {
  it("returns yellow trash to deck top and plays a small yellow Digimon", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "LM-029", as: "option" }], trash: ["BT1-055", "BT1-046"] },
        1: { battleArea: ["BT1-046"] },
      },
    ...
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "BT1-046")).toBe(true);
    await finish(s, turn);
  });

  it("does not activate when the opponent has no Digimon", async () => {
    const s = setupEngine(
      { 0: { battleArea: [{ card: "LM-029", as: "option" }], trash: ["BT1-046"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
    await s.ready();
    arm(s);
    const { turn } = await start(s);
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-029")).toBe(true);
    await finish(s, turn);
  });

  it("plays a qualifying yellow Digimon from security and returns itself to hand", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-029", as: "securityOption", faceUp: true }], trash: ["BT1-046"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
    await advance(s.engine).fireForInstance(EffectTiming.SecuritySkill, s.inst("securityOption"));
    await settle(() => s.state.players[0]!.hand.some((c) => c.cardId === "LM-029"));
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "BT1-046")).toBe(true);
    expect(s.state.players[0]!.hand.some((c) => c.cardId === "LM-029")).toBe(true);
  });
});
