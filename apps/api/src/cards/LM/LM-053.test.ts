...
// "Purple also meets this card's colour requirements" (Q4063/Q4064): the printed requirement
// still has to be met — by Black or by Purple — it is not waived outright.
describe("LM-053 Obsidian Memory Boost!", () => {
  it("reveals three, adds a black or purple Digimon, bottoms the rest and places itself", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [{ card: "BT2-052", as: "colorSource" }],
          hand: [{ card: "LM-053", as: "option" }],
    ...
    expect(s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "LM-053")).toBe(true);
  });
  ...
  it("is refused with no black or purple colour source in play", async () => {
    const s = setupEngine(
      {
        0: {
  ...
  it("places itself in the battle area from security", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-053", as: "securityOption", faceUp: true }] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );

    await advance(s.engine).fireForInstance(EffectTiming.SecuritySkill, s.inst("securityOption"));
    await settle(() => s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "LM-053"), 2000);

    expect(s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "LM-053")).toBe(true);
  });

  it("matches committed metadata and publishes fully covered compiled IR", () => {
    const definition = getCardDefinition("LM-053");
    const compiled = runtimeCompiledCard("LM-053");
    expect(definition?.nameEn).toBe("Obsidian Memory Boost!");
