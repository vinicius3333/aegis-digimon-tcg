...
describe("LM-056 Image Training", () => {
  it("reveals two, adds a blue or purple card, bottoms the rest and places itself", async () => {
    const s = setupEngine(
      {
        0: {
          hand: [{ card: "LM-056", as: "option" }],
          deck: ["BT1-028", "BT1-013"],
        },
      },
    ...
    await settle(() => s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-056"), 2000);
    ...
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-056")).toBe(true);
  });
    ...
    const s = setupEngine(
      {
        ...
        0: { hand: [{ card: "LM-056", as: "option" }], deck: ["BT1-013", "BT1-013"] },
      },
  ...
  it("loses the waiver once a copy of itself is already in the battle area", async () => {
    ...
    s.state.memory = 2;
    await s.ready();
    s.putOnBoard(0, { card: "LM-056", as: "copy" });
  ...
  it("digivolves for the printed cost reduced by 2 through its Delay clause", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [
            { card: "LM-056", as: "option" },
            { card: "BT1-029", as: "host" },
          ],
          hand: [{ card: "BT1-115", as: "evolution" }],
        },
      },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
    ...
    expect(
      s.engine.applyIntent(0, {
        ...
        effectKey: `LM-056/ir-${EffectTiming.OnDeclaration}-0`,
      }),
    ).toEqual({ ok: true });
    await settle(() => s.perm("host").topCard?.cardId === "BT1-115", 2000);

    expect(s.perm("host").topCard?.cardId).toBe("BT1-115");
    expect(s.state.memory).toBe(0);
  });

  it("reveals two and places itself from security", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-056", as: "securityOption", faceUp: true }], deck: ["BT1-028", "BT1-013"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
  ...
  it("matches committed metadata and publishes fully covered compiled IR", () => {
    const definition = getCardDefinition("LM-056");
    const compiled = runtimeCompiledCard("LM-056");
    expect(definition?.nameEn).toBe("Image Training");
