      ...
      actions: [
        {
          ...
          add: [
            {
              filter: { controllerDefault: "mine", kind: ["Digimon"], colors: ["Blue", "Black"] },
              count: 1,
              to: "hand",
            },
          ],
  ...
  it("Static WaiveColorRequirement only calls waiveColorRequirement when owner has Blue in play", async () => {
    ...
    const ctxWithBlue = makeContext(recorderWithBlue, source, {
      ...
      definitionOf: (card) => ({
        cardId: card.cardId,
        kinds: ["Digimon"],
        colors: card.cardId === "BT1-027" ? ["Blue"] : [],
        playCost: 0,
        dp: 0,
        evoCosts: [],
        maxCountInDeck: 4,
    ...
    const s = setupEngine(
      {
        0: {
          battleArea: ["BT10-022"],
          hand: [{ card: "LM-049", as: "option" }],
          deck: ["BT10-022", "BT1-064", "BT1-009"],
        },
      },
