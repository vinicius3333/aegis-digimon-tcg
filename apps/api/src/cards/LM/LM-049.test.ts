...
function makeSource(instanceId = "INST#1", cardId = "LM-049") {
  return {
    instanceId,
    cardId,
    ownerSeat: 0 as Seat,
    definition: { cardId, kinds: ["Option"], colors: ["Black"], playCost: 3, dp: 0, evoCosts: [], maxCountInDeck: 4 } as never,
  ...
  effects: [
    {
      ...
      actions: [{ kind: "WaiveColorRequirement", target: { filter: { isSelfRef: true }, count: 1, isSelf: true }, color: "blue" }],
    },
    {
      trigger: "Main",
      actions: [
        { kind: "RevealAdd", revealCount: 3, add: [{ filter: { controllerDefault: "mine", kind: ["Digimon"], colors: ["Blue", "Black"] }, count: 1, to: "hand" }], rest: "deckBottom" },
...
describe("LM-049 Midnight Memory Boost! — hand-corrected IR", () => {
  it("Security effect has isSecurity flag and resolves to placeOptionAsPermanent", async () => {
    ...
    expect(ctx._placed).toContain("INST#LM049");
  });

  it("Static WaiveColorRequirement only calls waiveColorRequirement when owner has Blue in play", async () => {
    const module = irCardModule("LM-049", COMPILED);
    const source = makeSource("INST#LM049", "LM-049");
    ...
    // No Blue Digimon: waiver must NOT fire.
    const greenDigi = {
    ...
    const recorderNoBlue = makeRecorder();
    const ctxNoBlue = makeContext(recorderNoBlue, source, {
      battleArea: [greenDigi],
      definitionOf: (card) => ({ cardId: card.cardId, kinds: ["Digimon"], colors: card.cardId === "BT1-064" ? ["Green"] : [], playCost: 0, dp: 0, evoCosts: [], maxCountInDeck: 4 }),
    });
    await staticEffect.resolve(ctxNoBlue as never);
    expect(recorderNoBlue.calls.some((c) => c.verb === "waiveColorRequirement")).toBe(false);

    // With a Blue Digimon: waiver MUST fire.
    const blueDigi = {
      permanentId: "blue-digi", controllerSeat: 0,
      topCard: { instanceId: "bd1", cardId: "BT1-027", ownerSeat: 0, faceUp: true },
      stack: [], linked: [], baseDP: 0, currentDP: 0, isSuspended: false, inBreeding: false,
    };
    const recorderWithBlue = makeRecorder();
    const ctxWithBlue = makeContext(recorderWithBlue, source, {
      battleArea: [blueDigi],
    ...
    await staticEffect.resolve(ctxWithBlue as never);
    expect(recorderWithBlue.calls.some((c) => c.verb === "waiveColorRequirement")).toBe(true);
  });

  it("plays with a blue color source, adds a black Digimon, and places itself", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT10-022"], hand: [{ card: "LM-049", as: "option" }], deck: ["BT10-022", "BT1-064", "BT1-009"] } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 3;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("option").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-049"));
    expect(s.state.players[0]!.hand.some((card) => card.cardId === "BT10-022")).toBe(true);
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-049")).toBe(true);
  });
});
