...
// "Blue also meets this card's colour requirements": the printed requirement
// still has to be met — by Black or by Blue — it is not waived outright.
describe("LM-049 Midnight Memory Boost!", () => {
  it("reveals three, adds a black or blue Digimon, bottoms the rest and places itself", async () => {
    const s = setupEngine(
      {
        0: {
    ...
    expect(s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "LM-049")).toBe(true);
  });
  ...
  it("counts a blue Digimon in the breeding area too", async () => {
    const s = setupEngine(
      {
        0: {
  ...
  it("is refused with no black or blue colour source in play", async () => {
    const s = setupEngine(
      {
        0: {
  ...
  it("places itself in the battle area from security", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-049", as: "securityOption", faceUp: true }] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );

    await advance(s.engine).fireForInstance(EffectTiming.SecuritySkill, s.inst("securityOption"));
    await settle(() => s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "LM-049"), 2000);

    expect(s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "LM-049")).toBe(true);
  });

  it("matches committed metadata and publishes fully covered compiled IR", () => {
    const definition = getCardDefinition("LM-049");
    const compiled = runtimeCompiledCard("LM-049");
    expect(definition?.nameEn).toBe("Midnight Memory Boost!");
