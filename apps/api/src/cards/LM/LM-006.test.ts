...
describe("LM-006 Cthyllamon", () => {
  it.each(["OnPlay", "WhenDigivolving"] as const)(
    "trashes the bottom 3 cards then restricts every empty-stack opponent Digimon on %s",
    (trigger) => {
      ...
      expect(effect.actions).toEqual([
        ...
        expect.objectContaining({
          ...
          target: expect.objectContaining({
            count: "all",
            filter: expect.objectContaining({ digivolutionCards: "none", controller: "opponent" }),
          }),
        }),
      ]);
    },
