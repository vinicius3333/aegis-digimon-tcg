...
describe("LM-060 Shadow Training", () => {
  it("reveals two, adds a green or purple card, bottoms the rest and places itself", async () => {
    const s = setupEngine(
      {
        0: {
          hand: [{ card: "LM-060", as: "option" }],
          deck: ["BT1-065", "BT1-013"],
        },
      },
    ...
    await settle(() => s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-060"), 2000);
    ...
    const s = setupEngine(
      {
        ...
        0: { hand: [{ card: "LM-060", as: "option" }], deck: ["BT1-013", "BT1-013"] },
      },
  ...
  it("loses the waiver once a copy of itself is already in the battle area", async () => {
    ...
    s.state.memory = 2;
    await s.ready();
    s.putOnBoard(0, { card: "LM-060", as: "copy" });
  ...
  it("digivolves for the printed cost reduced by 2 through its Delay clause", async () => {
    ...
    expect(
      s.engine.applyIntent(0, {
        ...
        effectKey: `LM-060/ir-${EffectTiming.OnDeclaration}-0`,
      }),
    ).toEqual({ ok: true });
  ...
  it("reveals two and places itself from security", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-060", as: "securityOption", faceUp: true }], deck: ["BT1-065", "BT1-013"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
  ...
  it("matches committed metadata and publishes fully covered compiled IR", () => {
    const definition = getCardDefinition("LM-060");
    const compiled = runtimeCompiledCard("LM-060");
    expect(definition?.nameEn).toBe("Shadow Training");
    expect(definition?.colors).toEqual(["Green", "Purple"]);
