...
describe("LM-032 Purple Scramble", () => {
  it("returns purple trash to deck top and plays a small purple Digimon", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "LM-032", as: "option" }], trash: ["BT4-077", "BT4-078"] },
        1: { battleArea: ["BT4-078"] },
      },
    ...
    await settle(() => (s as ReturnType<typeof setupEngine> & { top?: string }).top === "BT4-077");
    expect((s as ReturnType<typeof setupEngine> & { top?: string }).top).toBe("BT4-077");
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "BT4-078")).toBe(true);
    await finish(s, turn);
  });
  it("does not activate when the opponent has no Digimon", async () => {
    const s = setupEngine(
      { 0: { battleArea: [{ card: "LM-032", as: "option" }], trash: ["BT4-078"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
    await s.ready();
    arm(s);
    const { turn } = await start(s);
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-032")).toBe(true);
    await finish(s, turn);
  });
  it("plays a qualifying purple Digimon from security and returns itself to hand", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-032", as: "securityOption", faceUp: true }], trash: ["BT4-078"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
    await advance(s.engine).fireForInstance(EffectTiming.SecuritySkill, s.inst("securityOption"));
    await settle(() => s.state.players[0]!.hand.some((c) => c.cardId === "LM-032"));
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "BT4-078")).toBe(true);
    expect(s.state.players[0]!.hand.some((c) => c.cardId === "LM-032")).toBe(true);
  });
});
