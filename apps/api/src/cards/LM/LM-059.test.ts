...
describe("LM-059 Heat Training", () => {
  it("reveals two, adds a yellow or red card, bottoms the rest and places itself", async () => {
    const s = setupEngine(
      {
        0: {
          hand: [{ card: "LM-059", as: "option" }],
          deck: ["BT1-047", "BT1-028"],
        },
      },
    ...
    await settle(() => s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-059"), 2000);
    ...
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-059")).toBe(true);
  });
    ...
    const s = setupEngine(
      {
        ...
        0: { hand: [{ card: "LM-059", as: "option" }], deck: ["BT1-028", "BT1-028"] },
      },
  ...
  it("loses the waiver once a copy of itself is already in the battle area", async () => {
    ...
    s.state.memory = 2;
    await s.ready();
    s.putOnBoard(0, { card: "LM-059", as: "copy" });
  ...
  it("digivolves for the printed cost reduced by 2 through its Delay clause", async () => {
    ...
    expect(
      s.engine.applyIntent(0, {
        ...
        effectKey: `LM-059/ir-${EffectTiming.OnDeclaration}-0`,
      }),
    ).toEqual({ ok: true });
  ...
  it("reveals two and places itself from security", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-059", as: "securityOption", faceUp: true }], deck: ["BT1-047", "BT1-028"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
  ...
  it("matches committed metadata and publishes fully covered compiled IR", () => {
    const definition = getCardDefinition("LM-059");
    const compiled = runtimeCompiledCard("LM-059");
    expect(definition?.nameEn).toBe("Heat Training");
    expect(definition?.colors).toEqual(["Yellow", "Red"]);
