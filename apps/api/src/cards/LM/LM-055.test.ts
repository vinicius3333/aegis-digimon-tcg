...
describe("LM-055 Sprint Dash Training", () => {
  it("reveals two, adds a green or red card, bottoms the rest and places itself", async () => {
    const s = setupEngine(
      {
        0: {
          hand: [{ card: "LM-055", as: "option" }],
          deck: ["BT1-065", "BT1-028"],
        },
      },
    ...
    await settle(() => s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-055"), 2000);
    ...
    expect(s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId === "LM-055")).toBe(true);
  });
    ...
    const s = setupEngine(
      {
        ...
        0: { hand: [{ card: "LM-055", as: "option" }], deck: ["BT1-028", "BT1-028"] },
      },
  ...
  it("loses the waiver once a copy of itself is already in the battle area", async () => {
    ...
    s.state.memory = 2;
    await s.ready();
    s.putOnBoard(0, { card: "LM-055", as: "copy" });
  ...
  it("digivolves for the printed cost reduced by 2 through its Delay clause", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [
            { card: "LM-055", as: "option" },
            { card: "BT1-067", as: "host" },
          ],
          hand: [{ card: "BT6-050", as: "evolution" }],
        },
      },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
    ...
    expect(
      s.engine.applyIntent(0, {
        ...
        effectKey: `LM-055/ir-${EffectTiming.OnDeclaration}-0`,
      }),
    ).toEqual({ ok: true });
    await settle(() => s.perm("host").topCard?.cardId === "BT6-050", 2000);

    expect(s.perm("host").topCard?.cardId).toBe("BT6-050");
    expect(s.state.memory).toBe(0);
  });

  it("reveals two and places itself from security", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "LM-055", as: "securityOption", faceUp: true }], deck: ["BT1-065", "BT1-028"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
  ...
  it("matches committed metadata and publishes fully covered compiled IR", () => {
    const definition = getCardDefinition("LM-055");
    const compiled = runtimeCompiledCard("LM-055");
    expect(definition?.nameEn).toBe("Sprint Dash Training");
    expect(definition?.colors).toEqual(["Green", "Red"]);
