...
import { compiled } from "./ST1-02.js";

describe("ST1-02 Biyomon", () => {
  it("is a vanilla level 3 red Digimon with its catalog stats", async () => {
    const s = setupEngine({ 0: { battleArea: ["ST1-02"] } });
    await s.ready();
    const card = getCardDefinition("ST1-02")!;
    expect(s.state.players[0]!.battleArea[0]!.baseDP).toBe(card.dp);
    expect(card.level).toBe(3);
    expect(card.playCost).toBe(2);
    expect(card.dp).toBe(3000);
