...
const compiled: CompiledCard = {
  effects: [
    ...
    { trigger: "Main", actions: [{ kind: "SelectBind", target: { filter: { controller: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Greymon"], match: "name" }] }, count: 1, bindAs: "budgetDigimon" } }, { kind: "Delete", target: { filter: { controller: "opponent", kind: ["Digimon", "Tamer"] }, count: "all", upTo: true, totalPlayCostBudgetFromSelectionRef: "budgetDigimon" } }, { kind: "Attack", target: { filter: { controllerDefault: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Greymon"], match: "name" }] }, count: 1 }, optional: true, attackPlayer: true }] },
