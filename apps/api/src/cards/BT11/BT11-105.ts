...
const compiled: CompiledCard = {
  effects: [
    ...
    { trigger: "Main", actions: [
      { kind: "PlaceUnder", target: { filter: { zone: "trash", controllerDefault: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Vemmon", "Destromon"], match: "name" }] }, count: 1 }, from: ["trash"], underFilter: { controller: "mine", kind: ["Digimon"] }, position: "bottom", bindHostAs: "fusionizeHost" },
      { kind: "Digivolve", target: { filter: { controller: "mine", kind: ["Digimon"] }, count: 1 }, into: { controllerDefault: "mine", nameOrTrait: [{ tokens: ["Destromon", "Galacticmon"], match: "name" }] }, from: ["trash"], payCost: true, optional: true },
    ] },
