...
const compiled: CompiledCard = { effects: [
  { trigger: "Main", actions: [{ kind: "PlaceUnder", target: { filter: { zone: "trash", controller: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Bagra Army"], match: "trait" }] }, count: 3, upTo: true, from: ["trash"] }, underFilter: { controller: "mine", kind: ["Digimon", "Tamer"] }, optional: true }, { kind: "SelectBind", target: { filter: { controller: "opponent", kind: ["Digimon"] }, count: 1, bindAs: "movedDigimon" }, optional: true }, { kind: "PlaceUnder", target: { filter: {}, count: 1, fromSelectionRef: "movedDigimon" }, targetIsPermanent: true, underFilter: { controller: "opponent", kind: ["Digimon"] }, optional: true }] },
