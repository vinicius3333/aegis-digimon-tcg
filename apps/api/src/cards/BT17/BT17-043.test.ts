...
describe("BT17-043 Terriermon", () => {
  it("triggers once per turn from Terriermon/Lopmon or any green Tamer played by an effect", () => {
    ...
    expect(effect!.actions[0]).toMatchObject({ event: "whenPlayed", sourceFilter: { controller: "mine", byEffect: true, orFilters: [{ kind: ["Digimon"], nameOrTrait: [{ tokens: ["Terriermon", "Lopmon"], match: "name" }] }, { kind: ["Tamer"], colors: ["Green"] }] } });
  });
