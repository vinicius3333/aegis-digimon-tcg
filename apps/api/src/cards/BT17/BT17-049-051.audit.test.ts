...
describe("BT17-049–051 clause audits", () => {
  it("preserves evolution, inherited, and cost boundaries", () => {
    ...
    expect(c050.effects?.[0]).toEqual(
      expect.objectContaining({ trigger: "Main", actions: [expect.objectContaining({ kind: "Modal", choose: 1 })] }),
    );
    expect(c051.effects?.map((e) => e.trigger)).toEqual(expect.arrayContaining(["OnPlay", "WhenDigivolving"]));
