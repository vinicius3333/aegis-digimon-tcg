...
describe("BT17-031", () => {
  ...
  it("adds one named Digimon and Rika while bottom-decking the remainder", async () => {
    ...
    expect(s.state.players[0]!.hand.map((card) => card.instanceId)).toEqual(expect.arrayContaining([kyubimonId, rikaId]));
  ...
  it("reduces Security Attack only for an option use cost of at least 2", async () => {
    ...
    await advance(s.engine).fireSubTrigger("whenOptionUsed", { usedOptionCost: 2, subjectPermanentId: "qualifying-option" });
