...
const bt17Directory = fileURLToPath(new URL(".", import.meta.url));
const catalog = JSON.parse(readFileSync(effectsPath, "utf8")) as CompiledEffects;
const cardIds = Array.from({ length: 102 }, (_, index) => `BT17-${String(index + 1).padStart(3, "0")}`);

describe("BT17 persisted IR", () => {
  it("has exactly the 102 BT17 catalog records", () => {
    expect(
      Object.keys(catalog)
        .filter((cardId) => cardId.startsWith("BT17-"))
        .sort(),
    ).toEqual(cardIds);
  });
  ...
  it("has exactly 102 IR-only production modules", () => {
    const productionFiles = readdirSync(bt17Directory)
      .filter((fileName) => /^BT17-\d{3}\.ts$/.test(fileName))
      .sort();
    expect(productionFiles).toEqual(cardIds.map((cardId) => `${cardId}.ts`));

    for (const cardId of cardIds) {
      const source = readFileSync(join(bt17Directory, `${cardId}.ts`), "utf8");
