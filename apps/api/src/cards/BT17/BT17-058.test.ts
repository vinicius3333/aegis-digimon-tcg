...
describe("BT17-058 GroundLocomon", () => {
  it("reveals three and places one black level-5-or-lower Digimon underneath on both entry timings", () => {
    ...
    for (const effect of effects) {
      expect(effect.actions[0]).toMatchObject({
        kind: "RevealAdd",
        revealCount: 3,
        add: [{ filter: { controllerDefault: "mine", kind: ["Digimon"], colors: ["Black"], levelComparison: { op: "lte", value: 5 } }, count: 1, to: "placeUnder", underFilter: { isSelfRef: true } }],
        rest: "trash",
      });
    }
  });
  ...
  it("once per turn plays a level-5-or-lower Machine from its digivolution cards", () => {
    ...
    expect(effect).toMatchObject({
      ...
      actions: [{ kind: "PlayWithoutCost", from: ["digivolutionCards"], payCost: false, optional: true, target: { filter: { controller: "mine", kind: ["Digimon"], levelComparison: { op: "lte", value: 5 }, nameOrTrait: [{ tokens: ["Machine"], match: "trait" }], zone: "digivolutionCards", hostFilter: { isSelfRef: true } } } }],
    });
  });
  ...
  it("places the revealed card only under GroundLocomon and trashes the rest", async () => {
    ...
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("groundLocomon").instanceId })).toEqual({ ok: true });
