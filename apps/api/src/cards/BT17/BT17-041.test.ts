  ...
  it("plays a Tamer before scaling the on-play DP reduction", async () => {
    const s = setupEngine({
      0: {
        ...
        hand: [{ card: "BT17-041", as: "burst" }, { card: "BT12-092", as: "playedTamer" }],
      },
    ...
    expect(s.state.players[0]!.battleArea.some((permanent) => permanent.topCard?.instanceId === playedTamerId)).toBe(true);
  });
