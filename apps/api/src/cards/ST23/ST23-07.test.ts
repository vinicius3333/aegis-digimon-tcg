...
describe("ST23-07 Armalizamon", () => {
  it("plays a Glowing Dawn Tamer when digivolving with no own Tamers", async () => {
    ...
    await settle(() => s.perm("base").topCard?.cardId === "ST23-07" && s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "ST23-13"));
    expect(s.perm("base").topCard?.cardId).toBe("ST23-07");
    expect(s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "ST23-13")).toBe(true);
  });
});
