...
describe("ST23-03 Cougarmon", () => {
  it("recovers one security after digivolving", async () => {
    ...
    await settle(
      () =>
        s.perm("base").topCard?.cardId === "ST23-03" &&
        s.state.players[0]!.hand.some((card) => card.instanceId === securityId) &&
        s.state.players[0]!.security.some((card) => card.instanceId === recoveryId),
    );
    expect(s.perm("base").topCard?.cardId).toBe("ST23-03");
    expect(s.state.players[0]!.security).toHaveLength(1);
    expect(s.state.players[0]!.hand.some((card) => card.instanceId === securityId && card.cardId === "BT1-001")).toBe(
      true,
    );
    expect(s.state.players[0]!.security).toHaveLength(1);
