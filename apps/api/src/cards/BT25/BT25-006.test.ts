...
describe("BT25-006 Dorimon", () => {
  it("trashes one hand card when the opponent attacks, then unsuspends one Titan Digimon", () => {
    ...
    expect(effect?.actions?.[0]).toMatchObject({ event: "whenOpponentAttacks", cost: { kind: "trash", target: { filter: { zone: "hand", controller: "mine" }, count: 1 } } });
    const watcher = effect?.actions?.[0] as { actions?: unknown[] } | undefined;
    expect(watcher?.actions?.[0]).toMatchObject({ kind: "Unsuspend", target: { filter: { controller: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Titan"], match: "trait" }] }, count: 1 } });
  });
});
