...
/** BT25-094 Cosmic Area (Q6444-Q6449). */
const cardId = "BT25-094";
...
function eligibleTs(definition: CardDefinition, maximumLevel?: number): boolean {
  return (
    ...
    definition.colors.some((color) => color === "Red" || color === "Blue") &&
...
function fieldTargets(ctx: EffectContext, source: CardSource): Permanent[] {
  return ctx.game
    ...
    .battleArea.filter((permanent) => {
      ...
      if (!isDigimon(definition) || !permanentHasTrait(ctx.game, permanent, "TS")) return false;
      const colors = ctx.game.effectiveColors?.(permanent) ?? definition.colors;
      return colors.some((color) => color === "Red" || color === "Blue");
    });
}

function hasApollomonOrDianamon(ctx: EffectContext, source: CardSource): boolean {
  return ctx.game.player(source.ownerSeat).battleArea.some((permanent) => {
    if (permanent.inBreeding || permanent.topCard === undefined) return false;
    const definition = ctx.game.definitionOf(permanent.topCard);
    return isDigimon(definition) && (definition.nameEn.includes("Apollomon") || definition.nameEn.includes("Dianamon"));
  });
}
    ...
    if (timing === EffectTiming.None) {
      return [
        ...
        securityStatic({
          ...
          optional: false,
          description: "[Security][Your Turn] Your red or blue TS Digimon gain Alliance and conditional Rush.",
          when: (ctx) => ctx.game.state.turnSeat === source.ownerSeat,
          resolve: async (ctx) => {
            const grantsRush = hasApollomonOrDianamon(ctx, source);
    ...
    if (timing === EffectTiming.SecuritySkill) {
      return [
        security({
          ...
          description: "[Security] Play 1 level 4 or lower red or blue TS Digimon from hand or trash for free.",
