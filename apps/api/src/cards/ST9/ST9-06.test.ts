...
describe("ST9-06 Imperialdramon: Dragon Mode", () => {
  it("plays eligible blue and green level-4 sources when digivolving", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "ST9-05", as: "base", under: ["ST9-04", "ST9-09"] }], hand: [{ card: "ST9-06", as: "dragon" }] } }, { autoAcceptOptional: true, autoOrderTriggers: true, autoSelectCards: true });
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("dragon").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.battleArea.length === 3);
    expect(s.state.players[0]!.battleArea.map((p) => p.topCard.cardId)).toEqual(expect.arrayContaining(["ST9-04", "ST9-09"]));
  });

  it("plays neither eligible source when its optional effect is declined", async () => {
    const s = setupEngine(
      { 0: { battleArea: [{ card: "ST9-05", as: "base", under: ["ST9-04", "ST9-09"] }], hand: [{ card: "ST9-06", as: "dragon" }] } },
  ...
  it("executes the full Imperialdramon DNA line through two attacks and source replay", async () => {
    ...
    await settle(() =>
      ...
      s.state.players[0]!.battleArea.some((permanent) => permanent.topCard.instanceId === s.inst("paildramon").instanceId),
    );
    const imperial = s.state.players[0]!.battleArea.find((permanent) =>
      permanent.topCard.instanceId === s.inst("paildramon").instanceId,
    )!;
