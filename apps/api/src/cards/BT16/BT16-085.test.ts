...
describe("BT16-085", () => {
  ...
  it("gains memory and may trash three opposing digivolution cards during DNA digivolution", () => {
    expect(compiled.effects?.[1]).toMatchObject({ trigger: "YourTurn", actions: [{ kind: "SubTrigger", event: "whenOneOfYoursDigivolves", actions: [{ kind: "GainMemory", amount: 1, optional: true, abortOnDecline: true, cost: { kind: "suspend" } }, { kind: "TrashDigivolution", amount: 3, optional: true, abortOnDecline: true, condition: { kind: "isDnaDigivolving" } }] }] });
  });
