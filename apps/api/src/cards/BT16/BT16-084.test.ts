...
describe("BT16-084", () => {
  ...
  it("gains memory by suspending itself when a red or yellow Digimon digivolves", () => {
    expect(compiled.effects?.[1]).toMatchObject({ trigger: "YourTurn", actions: [{ kind: "SubTrigger", event: "whenOneOfYoursDigivolves", actions: [{ kind: "GainMemory", amount: 1, optional: true, abortOnDecline: true, cost: { kind: "suspend" } }] }, { kind: "ModifyDP", amount: -3000, condition: { kind: "isDnaDigivolving" } }] });
  });
