...
describe("BT16-066", () => {
  it("offers the opponent a hand trash and gains memory if they decline", () => {
    for (const effect of compiled.effects?.slice(0, 2) ?? []) {
      expect(effect.actions?.[0]).toMatchObject({ kind: "Trash", controller: "opponent", optional: true });
      expect(effect.actions?.[1]).toMatchObject({ kind: "GainMemory", amount: 1, condition: { kind: "ifThisEffectDidNotAct" } });
    }
  });
