...
describe("BT5-102 Wisselen", () => {
  ...
  it("gains 2 memory when you control a Digi-Burst Digimon", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT5-046"], hand: [{ card: "BT5-102", as: "option" }] } }, { autoSelectCards: true });
    s.state.memory = 8;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("option").instanceId })).toEqual({ ok: true });
  ...
  it("security restricts only attacks for the turn and applies the memory clause", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT5-046"], security: [{ card: "BT5-102", as: "securityOption", faceUp: true }] }, 1: { battleArea: [{ card: "BT5-047", as: "target" }] } }, { autoSelectCards: true });
