...
describe("BT5-022 Bulucomon", () => {
  ...
  it("gains memory only once when sources of two opponent Digimon are trashed", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT4-028", as: "host", under: ["BT5-022"] }] }, 1: { battleArea: [
      { card: "BT4-073", as: "first", under: [{ card: "BT1-009", as: "source-a" }] },
      { card: "BT4-073", as: "second", under: [{ card: "BT1-010", as: "source-b" }] },
    ] } });
    await s.engine.recomputeContinuousEffects();
    const before = s.state.memory;
    await (s.engine as any).primitives.trashDigivolutionCards(s.perm("first").permanentId, [s.inst("source-a").instanceId], { byEffectSeat: 0 });
    await (s.engine as any).primitives.trashDigivolutionCards(s.perm("second").permanentId, [s.inst("source-b").instanceId], { byEffectSeat: 0 });
    await settle(() => s.state.memory !== before);
    expect(s.state.memory - before).toBe(1);
  });
});
