...
describe("BT5-026 Coelamon", () => {
  it("has Blocker and loses 2 memory when attacking", async () => {
    ...
    expect(
      s.engine.applyIntent(0, {
        type: "attack",
        attackerPermanentId: s.perm("coelamon").permanentId,
        target: { kind: "player" },
      }),
    ).toEqual({ ok: true });
  ...
  it("redirects an opponent attack through its Blocker window", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "BT5-026", as: "coelamon" }], security: ["BT1-009"] },
    ...
    expect(
      s.engine.applyIntent(0, { type: "declareBlock", blockerPermanentId: s.perm("coelamon").permanentId }),
    ).toEqual({ ok: true });
