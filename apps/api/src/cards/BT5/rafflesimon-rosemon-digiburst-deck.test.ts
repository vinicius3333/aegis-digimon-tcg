...
describe("BT5 Rafflesimon/Rosemon Digi-Burst deck gauntlet", () => {
  it("resolves inherited discard payoffs, restricts only once, buffs both attackers, and checks four security", async () => {
    ...
    preferredIds.push(s.perm("restrictedOpponent").permanentId, s.perm("rosemon").permanentId);
    s.state.memory = 0;
    await s.ready();

    const roseSource = (s.engine as any).cardSourceOf(s.perm("rosemon").topCard!);
    const roseEffectKey = effectsOf(EffectTiming.OnDeclaration, roseSource).find(({ effectKey }) =>
      effectKey.startsWith("BT5-057/"),
    )!.effectKey;
    expect(
    ...
    const restrictionChoice = s.decisions.find(
      ({ req }) =>
    ...
    expect(new Set(restrictionChoice?.options?.candidateInstanceIds ?? [])).toEqual(
      new Set([s.perm("restrictedOpponent").permanentId, s.perm("freeOpponent").permanentId]),
    );
    ...
    const raffleEffectKey = effectsOf(EffectTiming.OnDeclaration, raffleSource).find(({ effectKey }) =>
      effectKey.startsWith("BT5-056/"),
    )!.effectKey;
    expect(
      s.engine.applyIntent(0, {
        type: "activateEffect",
