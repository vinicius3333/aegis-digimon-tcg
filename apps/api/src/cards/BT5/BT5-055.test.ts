...
describe("BT5-055 BanchoLillymon", () => {
  it("returns a suspended opposing Digimon to deck bottom and trashes its sources when deleted", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT5-055", as: "bancho" }] }, 1: { battleArea: [{ card: "BT4-073", as: "target", suspended: true, under: [{ card: "BT1-009", as: "source" }] }] } }, { autoSelectCards: true });
