...
describe("BT5-100 Royal Nuts", () => {
  it("reveals five, adds exactly one Digisorption Digimon, and bottoms the rest", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT5-046"], hand: [{ card: "BT5-100", as: "option" }], deck: [{ card: "BT5-058", as: "digisorption" }, "BT1-009", "BT1-010", "BT1-011", "BT1-012"] } }, { autoSelectCards: true });
    s.state.memory = 4;
  ...
  it("bottoms all five cards when none has Digisorption", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT5-046"], hand: [{ card: "BT5-100", as: "option" }], deck: ["BT1-009", "BT1-010", "BT1-011", "BT1-012", "BT1-013"] } }, { autoSelectCards: true });
    s.state.memory = 4;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("option").instanceId })).toEqual({ ok: true });
