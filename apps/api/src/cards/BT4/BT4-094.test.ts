...
describe("BT4-094 Tai Kamiya", () => {
  it("gives all own Digimon +1000 DP while at 3 or fewer security", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT4-094", as: "tai" }, { card: "BT1-009", as: "red" }, { card: "BT4-076", as: "purple" }], security: ["BT1-010", "BT1-011", "BT1-012"] } });
  ...
  it("suspends to gain memory when an opposing Digimon is deleted at 0 DP", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT4-094", as: "tai" }], hand: [{ card: "BT3-101", as: "option" }] }, 1: { battleArea: [{ card: "BT1-010", as: "target", dp: 1000 }] } }, { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true });
    s.state.memory = 5;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("option").instanceId })).toEqual({ ok: true });
    await settle(() => s.perm("tai").isSuspended && s.state.players[1]!.battleArea.length === 0 && s.state.memory === 3, 5000);
  ...
  it("plays itself from security", async () => {
    const s = setupEngine({ 0: { security: [{ card: "BT4-094", as: "securityTamer", faceUp: true }] } }, { autoOrderTriggers: true });
