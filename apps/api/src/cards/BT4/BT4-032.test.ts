...
describe("BT4-032 MachGaogamon", () => {
  ...
  it("Digi-Bursts 2 to return a level 4 Digimon after trashing all of its sources", async () => {
          ...
          battleArea: [
            {
              ...
              as: "target",
              under: [
                { card: "BT1-001", as: "oppBottom" },
                { card: "BT2-001", as: "oppTop" },
              ],
            },
          ],
        },
    ...
    expect(
      s.engine.applyIntent(0, {
        type: "activateEffect",
        sourceInstanceId: s.perm("mach").topCard!.instanceId,
        effectKey,
      }),
    ).toEqual({ ok: true });
  ...
  it("gives +2000 DP to its host while you have a Tamer of any color", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "BT4-034", as: "host", under: ["BT4-032"] }, { card: "BT1-088" }] },
    });
