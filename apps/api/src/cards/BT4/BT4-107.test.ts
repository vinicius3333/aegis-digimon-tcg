...
describe("BT4-107 Pollen Spray", () => {
  it("adds every revealed Digi-Burst Digimon and suspends one opponent per card added", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: ["BT4-059"],
          hand: [{ card: "BT4-107", as: "option" }],
          deck: ["BT4-012", "BT4-059", "BT4-003"],
        },
        1: {
          battleArea: [
    ...
    await settle(
      () => s.state.players[0]!.hand.length === 2 && s.perm("first").isSuspended && s.perm("second").isSuspended,
    );
  ...
  it("activates the full Main effect from security", async () => {
    const s = setupEngine(
      {
        0: {
          security: [{ card: "BT4-107", as: "securityOption", faceUp: true }],
          deck: ["BT4-012", "BT4-003", "BT4-004"],
        },
