...
describe("BT4-103 Full Moon Blaster", () => {
    ...
    const s = setupEngine(
      {
        ...
        1: { battleArea: [{ card: "BT4-045", as: "target", under: [{ card: "BT4-044", as: "source" }] }] },
      },
    ...
    const s = setupEngine(
      {
        0: { battleArea: ["BT4-023"], hand: [{ card: "BT4-103", as: "option" }] },
        1: {
          hand: Array.from({ length: 8 }, () => "BT1-001") as string[],
  ...
  it("activates the threshold-aware Main effect from security", async () => {
    const s = setupEngine(
      {
        0: { security: [{ card: "BT4-103", as: "securityOption", faceUp: true }] },
