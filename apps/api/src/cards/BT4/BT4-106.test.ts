...
describe("BT4-106 Purge Shine", () => {
  it("reduces every opposing Digimon by 3000 DP", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT4-044"], hand: [{ card: "BT4-106", as: "option" }] }, 1: { battleArea: [{ card: "BT4-045", as: "first" }, { card: "BT4-090", as: "second" }] } }, { autoSelectCards: true });
    s.state.memory = 6;
  ...
  it("activates the Main effect from security", async () => {
    const s = setupEngine({ 0: { security: [{ card: "BT4-106", as: "securityOption", faceUp: true }] }, 1: { battleArea: [{ card: "BT4-045", as: "target" }] } });
