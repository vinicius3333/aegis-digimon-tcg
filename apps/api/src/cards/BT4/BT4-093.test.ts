...
describe("BT4-093 Thomas H. Norstein", () => {
  it("draws one card on play", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "BT4-093", as: "source" }], deck: [
      { card: "BT4-026", as: "drawn" },
    ] } });
  ...
  it("suspends to unsuspend a Gao Digimon while the opponent has 8 cards", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT4-093", as: "thomas" }, { card: "BT4-035", as: "gao", suspended: true }] }, 1: { hand: Array.from({ length: 8 }, () => "BT1-001") as string[] } }, { autoAcceptOptional: true, autoSelectCards: true });
    ...
    expect(s.engine.applyIntent(0, { type: "activateEffect", sourceInstanceId: s.perm("thomas").topCard.instanceId, effectKey })).toEqual({ ok: true });
