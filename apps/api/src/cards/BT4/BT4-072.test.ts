...
describe("BT4-072 Gogmamon", () => {
  it("Digi-Bursts 1 to give an own Digimon +2000 DP through the opponent's turn", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT4-072", as: "gog", under: ["BT1-009"] }] } }, { autoSelectCards: true });
    const gog = s.perm("gog");
    const before = gog.currentDP;
    const source = (s.engine as any).cardSourceOf(gog.topCard!);
    const effectKey = effectsOf(EffectTiming.OnDeclaration, source).find((effect) => effect.effectKey.startsWith("BT4-072/"))!.effectKey;
    expect(s.engine.applyIntent(0, { type: "activateEffect", sourceInstanceId: s.perm("gog").topCard!.instanceId, effectKey })).toEqual({ ok: true });
