...
describe("BT4-028 Piranimon", () => {
  it("trashes the top source of an opposing Digimon when its host attacks", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT1-038", as: "host", under: ["BT4-028"] }] }, 1: { battleArea: [{ card: "BT3-015", as: "target", under: [{ card: "BT1-001", as: "bottom" }, { card: "BT2-001", as: "top" }] }], security: ["BT1-010"] } }, { autoSelectCards: true });
