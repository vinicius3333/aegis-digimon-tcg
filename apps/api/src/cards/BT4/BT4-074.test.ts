...
describe("BT4-074 Darkdramon", () => {
  it("returns up to five [D-Brigade] Digimon from trash to the deck top and gains 2 memory each", async () => {
    ...
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("darkdramon").instanceId })).toEqual({ ok: true });
    await settle(
      () =>
        returnedIds.every((id) => player.deck.some((card) => card.instanceId === id)) &&
        s.state.memory === 4,
    );
    ...
    const played = player.battleArea.find((permanent) => permanent.topCard.instanceId === s.inst("darkdramon").instanceId)!;
  ...
  it("separates which D-Brigade cards return from their top-to-bottom order", async () => {
    ...
    await settle(() =>
      s.state.players[0]!.deck[0]?.instanceId === deckOrder[0] && s.state.memory === 6,
    );
