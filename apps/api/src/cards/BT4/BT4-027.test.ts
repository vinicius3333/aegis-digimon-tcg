...
describe("BT4-027 KendoGarurumon", () => {
  it("digivolves from hand onto a blue Tamer for 3 memory", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT1-086", as: "tamer" }], hand: [{ card: "BT4-027", as: "kendo" }], deck: ["BT1-001"] } });
    s.state.memory = 4;

    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("tamer").permanentId, instanceId: s.inst("kendo").instanceId })).toEqual({ ok: true });
  ...
  it("returns a level 3 Digimon and trashes all of that Digimon's sources when attacking", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT4-027", as: "kendo" }] }, 1: { battleArea: [{ card: "BT1-009", as: "target", under: [{ card: "BT1-001", as: "bottom" }, { card: "BT2-001", as: "top" }] }], security: ["BT1-010"] } }, { autoSelectCards: true });
    const targetId = s.perm("target").permanentId;
    const sourceIds = [s.inst("bottom").instanceId, s.inst("top").instanceId];

    expect(s.engine.applyIntent(0, { type: "attack", attackerPermanentId: s.perm("kendo").permanentId, target: { kind: "player" } })).toEqual({ ok: true });
