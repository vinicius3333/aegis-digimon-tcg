...
describe("EX9-042", () => {
  ...
  it("suspends one opposing Digimon and prevents it from unsuspending", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX9-042", as: "source" }] }, 1: { battleArea: [{ card: "BT1-009", as: "suspend" }, { card: "BT1-010", as: "restrict" }] } }, { autoSelectCards: true, autoAcceptOptional: true, autoOrderTriggers: true });
