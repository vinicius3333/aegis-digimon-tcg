...
describe("EX9-035", () => {
  ...
  it("suspends one opposing Digimon from the inherited attack effect", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX9-034", as: "host", under: ["EX9-035"] }] }, 1: { battleArea: [{ card: "BT1-009", as: "opponent" }] } }, { autoSelectCards: true, autoAcceptOptional: true, autoOrderTriggers: true });
    s.state.turnSeat = 0;
