...
describe("EX9-063", () => {
  ...
  it("trashes the bottom face-down source and plays one DM Digimon from trash on public digivolution", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [{ card: "EX9-009", as: "base", under: [{ card: "EX9-015", faceUp: false }, "EX9-010"] }],
          hand: [{ card: "EX9-063", as: "evo" }],
          trash: ["EX9-010"],
        },
    ...
    expect(s.perm("base").topCard.cardId).toBe("EX9-063");
    expect(s.perm("base").stack.map(({ cardId }) => cardId)).toEqual(["EX9-010", "EX9-009"]);
    expect(s.perm("base").stack[0]).toMatchObject({ cardId: "EX9-010", faceUp: true });
