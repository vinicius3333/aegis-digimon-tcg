...
describe("EX9-063", () => {
  ...
  it("once per turn plays a low-cost DM Digimon from trash by trashing a bottom source on digivolving", () => expect(compiled.effects?.find((entry) => entry.trigger === "WhenDigivolving")).toMatchObject({ frequency: "OncePerTurn", actions: [{ kind: "PlayWithoutCost", from: ["trash"], cost: { kind: "trash", target: { filter: { zone: "digivolutionCards" } } } }] }));
