...
describe("EX9-007", () => {
  it("reveals 3 for a DM card and a Ver.1 card, placing the latter under a DM Digimon", () => expect(compiled.effects?.find((entry) => entry.trigger === "OnPlay")?.actions[0]).toMatchObject({ kind: "RevealAdd", revealCount: 3, add: [{ count: 1, to: "hand" }, { count: 1, to: "placeUnder" }], rest: "deckBottom" }));
