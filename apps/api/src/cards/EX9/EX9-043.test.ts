...
describe("EX9-043", () => {
  ...
  it("accepts Tyrannomon name or DM trait as separate alternate evolution routes", () => expect(compiled.digivolutionRequirement).toEqual([
  ...
  it("trashes an eligible hand card and reduces the play cost by exactly 2", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "EX9-043", as: "metal" }, { card: "BT1-021", as: "payment" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
  ...
  it("does not reduce the cost or trash an ineligible hand card", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "EX9-043", as: "metal" }, { card: "BT1-009", as: "plain" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    const player = s.state.players[0] as PlayerState;
