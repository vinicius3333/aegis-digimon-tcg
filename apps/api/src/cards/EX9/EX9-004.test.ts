...
describe("EX9-004", () => {
  ...
  it("gains 1 memory and trashes its bottom face-down card when a Ver.4 Digimon is played", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "EX9-035", as: "host", under: [{ card: "BT1-009", faceUp: false }, "EX9-004"] }], hand: [{ card: "EX9-038", as: "played" }] },
    ...
    await settle(() => s.events.some((event) => event.kind === "memoryChanged" && "from" in event && event.from === 1 && event.to === 2));
  ...
  it("does not gain memory when the bottom card is face up", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "EX9-035", as: "host", under: [{ card: "BT1-009" }, "EX9-004"] }], hand: [{ card: "EX9-038", as: "played" }] },
    }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 4;
