...
describe("EX9-028", () => {
  ...
  it("inherits -3000 DP for opposing Security Digimon during your turn", () => {
    expect(compiled.effects?.find((entry) => entry.isInherited)).toMatchObject({ actions: [{ kind: "ModifySecurityDP", controller: "opponent", amount: -3000, duration: "permanent" }] });
  });
  it("applies the inherited Security-Digimon reduction through the live ledger", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT1-010", as: "host", under: ["EX9-028"] }] }, 1: { security: ["BT1-009", "BT1-090"] } });
