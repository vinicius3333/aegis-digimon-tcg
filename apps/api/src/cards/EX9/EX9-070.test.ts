...
describe("EX9-070", () => {
  ...
  it("can digivolve any own DM Digimon by two after placing a hand card underneath", () => expect(compiled.effects?.filter((entry) => entry.trigger === "Main")[1]).toMatchObject({ actions: [{ kind: "Digivolve", target: { filter: { controller: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["DM"], match: "trait" }] } }, reduceCost: 2, cost: { kind: "place", host: { filter: { controller: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["DM"], match: "trait" }] }, count: 1 }, faceDown: true, destination: "digivolutionStack" } }] }));
  ...
  it("activates Delay to place a hand card face-down and digivolve a DM Digimon", async () => {
    const options = { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true, preferInstanceIds: [] as string[] };
    const s = setupEngine({
      0: {
        battleArea: [{ card: "EX9-070", as: "option" }, { card: "EX9-007", as: "host" }],
        hand: [{ card: "BT1-009", as: "under" }, { card: "EX9-010", as: "evo" }],
      },
    }, options);
    ...
    const effect = JSON.parse(s.perm("option").activatableEffectsJson || "[]").find((entry: { description?: string }) => /Delay/i.test(entry.description ?? ""));
    expect(effect).toBeDefined();
    expect(s.engine.applyIntent(0, { type: "activateEffect", sourceInstanceId: s.perm("option").topCard.instanceId, effectKey: effect.effectKey })).toEqual({ ok: true });
    ...
    expect(s.state.players[0]!.battleArea.map((permanent) => ({ top: permanent.topCard.cardId, stack: permanent.stack.map((card) => card.cardId) }))).toEqual([{ top: "EX9-010", stack: ["BT1-009", "EX9-007"] }]);
