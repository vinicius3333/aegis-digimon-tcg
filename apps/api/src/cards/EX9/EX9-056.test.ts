...
describe("EX9-056", () => {
  ...
  it("allows either player's qualifying Digimon as the bottom-security payment and affects all own Ver.3 leaves", () => {
    for (const trigger of ["OnPlay", "WhenDigivolving"]) expect(compiled.effects?.find((entry) => entry.trigger === trigger)?.actions[0]).toMatchObject({
      ...
      cost: { targetIsPermanent: true, target: { filter: { kind: ["Digimon"], dp: { op: "lte", value: 8000 } }, count: 1 }, destination: "security", position: "bottom", faceDown: true },
    });
  ...
  it("places the opposing Digimon at security bottom and trashes that player's top security on play", async () => {
    const s = setupEngine({
      ...
      1: { battleArea: [{ card: "EX9-050", as: "target", dp: 4000, under: ["EX9-001"] }], security: ["BT1-010", "BT1-011"] },
