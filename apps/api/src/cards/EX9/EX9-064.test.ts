...
describe("EX9-064", () => {
  it("reduces play cost by two by trashing Cyborg or Ver.4 and deletes two low-cost Digimon after placing a trash source", () => {
    ...
    expect(compiled.effects?.find((entry) => entry.trigger === "OnPlay")).toMatchObject({ actions: [{ kind: "Delete", target: { count: 2, filter: { playCostLte: 4 } }, cost: { kind: "place", faceDown: true, destination: "digivolutionStack" } }, { kind: "CostModifier", amount: 1 }] });
  });
