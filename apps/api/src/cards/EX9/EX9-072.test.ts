...
describe("EX9-072", () => {
  it("waives color requirements when there are no face-up security cards", () => expect(compiled.effects?.find((entry) => entry.trigger === "Static")).toMatchObject({ actions: [{ kind: "WaiveColorRequirement", condition: { kind: "noFaceUpSecurity" } }] }));
  it("gives own DM Digimon +1000 DP per face-down digivolution card from security", () => expect(compiled.effects?.find((entry) => entry.trigger === "AllTurns")).toMatchObject({ isSecurity: true, actions: [{ kind: "ModifyDP", amount: 1000, scaling: { unit: "targetFaceDownDigivolutionCards", per: 1, filter: { faceDown: true } } }] }));
  ...
  it("scales the security DM Digimon DP bonus from its face-down sources", async () => {
    const s = setupEngine({
      0: { security: [{ card: "EX9-072", as: "source", faceUp: true }], battleArea: [{ card: "EX9-007", as: "host", dp: 3000, under: [{ card: "BT1-009", faceUp: false }, { card: "BT1-010", faceUp: false }] }, { card: "EX9-010", as: "secondHost", dp: 3000, under: [{ card: "BT1-009", faceUp: false }] }] },
    });
