...
describe("EX9-009", () => {
  ...
  it("places the deck top face-down and gains +1000 DP when attacking", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX9-009", as: "source" }], deck: ["BT1-009"] }, 1: { battleArea: [{ card: "BT1-010", as: "target", dp: 1000, suspended: true }] } }, { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true });
    s.state.turnSeat = 0;
    const source = s.perm("source");
    const before = source.currentDP;
    expect(s.engine.applyIntent(0, { type: "attack", attackerPermanentId: source.permanentId, target: { kind: "permanent", permanentId: s.perm("target").permanentId } })).toEqual({ ok: true });
    await settle(
      () =>
        s.state.players[0]!.deck.length === 0 &&
        s.state.players[0]!.battleArea.some((permanent) => permanent.currentDP === before + 1000),
    );
    ...
    expect(resolved.currentDP).toBe(before + 1000);
  });

  it("does not place a source or gain DP when its deck is empty", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX9-009", as: "source" }], deck: [] }, 1: { battleArea: [{ card: "BT1-010", as: "target", dp: 1000, suspended: true }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.turnSeat = 0;
    const source = s.perm("source");
    const before = source.currentDP;
    expect(s.engine.applyIntent(0, { type: "attack", attackerPermanentId: source.permanentId, target: { kind: "permanent", permanentId: s.perm("target").permanentId } })).toEqual({ ok: true });
