...
describe("BT15-048", () => {
  it("restricts unsuspension and conditionally suspends an opposing Digimon when Togemon/X Antibody is in stack", () => {
    ...
    expect(compiled.effects?.[0]?.actions[1]).toMatchObject({ kind: "Suspend", condition: { kind: "selfDigivolutionStackHasTrait", filter: { nameOrTrait: [{ tokens: ["Togemon"], match: "name" }, { tokens: ["X Antibody"], match: "trait" }] } });
  });
