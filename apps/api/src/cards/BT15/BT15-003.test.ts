...
describe("BT15-003", () => {
  it("may trash the top or bottom security card to gain 1 memory once per turn", () => {
    ...
    expect(compiled.effects?.[0]?.actions[0]).toMatchObject({ abortOnDecline: true, cost: { raw: "By trashing the top or bottom card of your security stack", target: { filter: { controller: "mine" } } } });
  });
});
