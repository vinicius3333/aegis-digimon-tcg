...
const bt15Directory = fileURLToPath(new URL(".", import.meta.url));
const catalog = JSON.parse(readFileSync(effectsPath, "utf8")) as CompiledEffects;
const cardIds = Array.from({ length: 102 }, (_, index) => `BT15-${String(index + 1).padStart(3, "0")}`);

describe("BT15 persisted IR", () => {
  it("has exactly the 102 BT15 catalog records", () => {
    expect(
      Object.keys(catalog)
        .filter((cardId) => cardId.startsWith("BT15-"))
        .sort(),
    ).toEqual(cardIds);
  });
  ...
  it("has exactly 102 IR-only production modules", () => {
    const productionFiles = readdirSync(bt15Directory)
      .filter((fileName) => /^BT15-\d{3}\.ts$/.test(fileName))
      .sort();
    expect(productionFiles).toEqual(cardIds.map((cardId) => `${cardId}.ts`));

    for (const cardId of cardIds) {
      const source = readFileSync(join(bt15Directory, `${cardId}.ts`), "utf8");
