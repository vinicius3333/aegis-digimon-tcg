...
describe("BT15-054", () => {
  it("suspends an opposing Digimon and Tamer and restricts their unsuspension", () => {
    expect(compiled.effects?.[0]).toMatchObject({ trigger: "WhenDigivolving", actions: [{ kind: "Suspend", target: { bindAs: "digimonTarget" } }, { kind: "Suspend", target: { bindAs: "tamerTarget" } }, { kind: "Restrict", target: { fromSelectionRef: "digimonTarget" } }, { kind: "Restrict", target: { fromSelectionRef: "tamerTarget" } }] });
  });
