...
describe("BT15-051", () => {
  it("gains memory with a suspended opposing Digimon and draws when Lillymon/X Antibody is stacked", () => {
    ...
    expect(compiled.effects?.[0]?.actions[1]).toMatchObject({ kind: "Draw", amount: 1, condition: { kind: "selfDigivolutionStackHasTrait", filter: { nameOrTrait: [{ tokens: ["Lillymon"], match: "name" }, { tokens: ["X Antibody"], match: "trait" }] } }, scaling: { per: 1, unit: "cards" } });
  });
