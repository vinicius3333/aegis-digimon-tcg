...
describe("BT15-013", () => {
  ...
  it("gains 1 memory once per turn when an opponent's security is removed", () => expect(compiled.effects?.[1]).toMatchObject({ trigger: "YourTurn", isInherited: true, frequency: "OncePerTurn", actions: [{ kind: "SubTrigger", event: "whenSecurityRemoved", sourceFilter: { controller: "opponent" } }] }));
});
