...
describe("BT15-070", () => {
  ...
  it("deletes the opposing battle partner when deleted after losing a battle", () => expect(compiled.effects?.[1]).toMatchObject({ trigger: "OnDeletion", isInherited: true, actions: [{ kind: "Delete", target: { filter: { sourceRef: "battleOpponent" } } }] }));
});
