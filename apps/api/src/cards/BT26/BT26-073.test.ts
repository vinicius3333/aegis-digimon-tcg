...
describe("BT26-073 Aegiochusmon: Dark", () => {
  it("encodes both exclusive costs, On Deletion play, inherited Security Attack, and Wizard", () => {
    ...
    expect(compiled.effects[3]).toMatchObject({ isInherited: true, keywords: [{ keyword: "SecurityAttack", amount: 1 }] });
    ...
    expect(compiled.assemblyRequirement).toEqual([{ reduceCost: 2, materials: [{ levelMax: 4, nameOrTrait: [{ tokens: ["Chronomon"], match: "text" }, { tokens: ["TS"], match: "trait" }], count: 1 }] }]);
  });
  ...
  it("pays the self-delete mode and deletes only an opponent level 5 or lower Digimon", async () => {
    const s = setupEngine({
      ...
      1: { battleArea: [{ card: "BT26-074", as: "low" }, { card: "BT26-060", as: "high" }] },
  ...
  it("publishes inherited Security A. +1 and the rule-granted Wizard trait", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "BT26-074", as: "host", under: ["BT26-073"] }, { card: "BT26-073", as: "standalone" }] },
    });
    await s.ready();
