...
describe("BT26-031 compiled fidelity", () => {
  it("encodes the Glowing Dawn waiver, recovery, Option DP sequence, and explicit most-security selector seam", () => {
    ...
    expect(card?.effects?.[1]?.actions).toMatchObject([{ kind: "RecoverByTrashingMostSecurity", recover: false }, { kind: "SelectBind", condition: { kind: "ifThisEffectActed" } }, { kind: "Restrict", restriction: "suspend", condition: { kind: "ifThisEffectActed" } }, { kind: "trashBottomFaceDownUnderTamer" }, { kind: "Recover", amount: 1, condition: { kind: "ifThisEffectActed" } }]);
