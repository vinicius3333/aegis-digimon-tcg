...
// A3 for BT26-028 (Medicmon, BT26): "[On Play] [When Digivolving] You may link 1 level 3
// Digimon card with the [Life], [System] or [Seven Code] trait from this Digimon's
// digivolution cards to this Digimon without paying the cost."
//
// FAILS-WHEN-REVERTED: dropping the `def.level === 3` or trait filter (or the link call)
// either offers an ineligible digivolution card or never links anything; this test
// asserts the link primitive is invoked with exactly the eligible stack card.
...
function makeSource(stack: { instanceId: string; cardId: string }[]): CardSource {
  return {
    ...
    permanent: () => ({ permanentId: "medicmon-perm", stack }) as never,
    isOnBattleArea: () => true,
    isOwnersTurn: () => true,
    hasColor: () => false,
  };
