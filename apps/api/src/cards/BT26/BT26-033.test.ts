...
describe("BT26-033 compiled fidelity", () => {
  it("encodes keywords, security recovery, leave prevention, and the explicit turn seam", () => {
    ...
    expect(card?.effects?.[0]?.actions).toMatchObject([
      { kind: "SecurityManipulation", op: "toHand" },
      {
        ...
        effectTextPart: expect.stringMatching(/^Then,/),
        options: [
          [{ kind: "PlayWithoutCost", reduceCostBy: 5, optional: true }],
          [{ kind: "UseOptionWithoutCost", reduceCostBy: 5, optional: true }],
        ],
        optional: false,
      },
    ]);
    expect(card?.effects?.[0]?.description).toBe("[When Digivolving] Add your top security card to the hand.");
  ...
  it("opens the shared card picker directly after the mandatory security move, including a DUAL candidate", async () => {
    const s = setupEngine({
      0: {
        ...
        hand: [
          ...
          { card: "BT26-060", as: "unaffordableIliad" },
        ],
        security: [{ card: "BT26-032", as: "securityDual" }],
      },
    });
    s.state.memory = 4;
    s.state.turnSeat = 0;
    ...
    expect(request.options).toMatchObject({
      effectText: "[When Digivolving] Add your top security card to the hand.",
      effectTextPart: expect.stringMatching(/^Then,/),
    });
