...
const linkedSource = { controllerDefault: "mine", zone: "digivolutionCards", kind: ["Digimon"], levels: [3], hasLinkRequirement: true, nameOrTrait: [
  { tokens: ["Navi"], match: "trait" }, { tokens: ["System"], match: "trait" }, { tokens: ["Seven Code"], match: "trait" },
] };
const opponentDigimon = { filter: { controllerDefault: "opponent", kind: ["Digimon"] }, count: 1 };
const link = { kind: "Link", target: { filter: linkedSource, count: 1 }, recipient: { filter: { isSelfRef: true }, count: 1, isSelf: true }, from: ["digivolutionCards"], payCost: false, optional: true };
const battle = [{ kind: "SubTrigger", event: "whenLinked", sourceFilter: { isSelfRef: true }, actions: [{ kind: "Battle", attacker: { filter: { isSelfRef: true }, count: 1, isSelf: true }, defender: opponentDigimon, optional: true }] }];
