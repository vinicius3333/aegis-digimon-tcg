...
describe("BT26-015 compiled fidelity", () => {
  it("encodes the shared play/evolution debuff, trash return deletion, deck-add buff attack, and explicit inherited seam", () => {
    ...
    expect(card?.effects?.[3]).toMatchObject({ isInherited: true, actions: [{ kind: "SubTrigger", event: "whenEffectAddsToDeck", hostFilter: { isSelfRef: true, nameOrTrait: [{ tokens: ["Chronomon"], match: "text" }] } }] });
  });
});
