...
const dataSquadDigimon = { filter: { controller: "mine", zone: "hand", kind: ["Digimon", "Tamer"], nameOrTrait: [{ tokens: ["DATA SQUAD"], match: "trait" }] }, count: 1 };
const dataSquadOption = { controller: "mine", zone: "hand", kind: ["Option"], nameOrTrait: [{ tokens: ["DATA SQUAD"], match: "trait" }] };
const playCostCeiling = { base: 3, raise: 1, per: 1, filter: { controller: "opponent", kind: ["Digimon", "Tamer"], suspended: true }, unit: "cards" };
const playOrUseDataSquad = { kind: "Modal", choose: 1, options: [
  [{ kind: "PlayWithoutCost", target: dataSquadDigimon, from: ["hand"], payCost: false, optional: true, playCostCeiling }],
  [{ kind: "UseOptionWithoutCost", filter: dataSquadOption, from: ["hand"], payCost: false, optional: true, playCostCeiling }],
] };
