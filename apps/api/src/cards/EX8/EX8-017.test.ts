...
describe("EX8-017", () => {
  ...
  it("gives a live friendly Digimon Blocker on play", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "EX8-017", as: "crabmon" }], battleArea: [{ card: "AD1-001", as: "target" }] } }, { autoSelectCards: true });
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("crabmon").instanceId })).toEqual({ ok: true });
