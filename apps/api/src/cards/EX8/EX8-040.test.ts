...
describe("EX8-040", () => {
  ...
  it("suspends the forced opposing Digimon on play", async () => {
    ...
    const s = setupEngine({ 0: { hand: [{ card: "EX8-040", as: "kab" }] }, 1: { battleArea: [{ card: "AD1-001", as: "target" }] } }, { autoAcceptOptional: true, autoSelectCards: true, preferInstanceIds });
    ...
    await settle(() => player.battleArea.some((p) => p.topCard?.cardId === "EX8-040") && s.state.players[1]!.battleArea[0]!.isSuspended);
