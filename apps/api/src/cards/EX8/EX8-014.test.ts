...
describe("EX8-014", () => {
  it("has Fortitude and may suspend a Digimon to delete an opposing Digimon with 8000 DP or less", () => {
    ...
    expect(compiled.effects?.find((entry) => entry.trigger === "OnPlay")?.actions).toMatchObject([{ kind: "Suspend", optional: true }, { kind: "SubTrigger", event: "whenSuspended", actions: [{ kind: "Delete", target: { count: 1, filter: { dp: { op: "lte", value: 8000 } } } }] }]);
  });
