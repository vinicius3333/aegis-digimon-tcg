...
describe("EX8-039", () => {
  ...
  it("reveals three cards, adds matching Insectoid and NSp cards, and bottoms the rest", async () => {
    const s = setupEngine({
      0: { hand: [{ card: "EX8-039", as: "tentomon" }], deck: [{ card: "EX8-040", as: "insectoid" }, { card: "EX8-039", as: "nsp" }, { card: "AD1-001", as: "rest" }] },
    }, { autoSelectCards: true });
    const player = s.state.players[0] as PlayerState;
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("tentomon").instanceId })).toEqual({ ok: true });
    await settle(() => player.hand.some((card) => card.instanceId === s.inst("insectoid").instanceId) && player.hand.some((card) => card.instanceId === s.inst("nsp").instanceId));
