...
describe("EX8-018", () => {
  ...
  it("selects the printed DS and Sea Beast/Plesiosaur matches from the live reveal", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "EX8-018", as: "gomamon" }], deck: [{ card: "EX8-020", as: "ds" }, { card: "EX8-027", as: "plesiosaur" }, { card: "AD1-001", as: "decoy" }] } }, { autoSelectCards: true });
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("gomamon").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.hand.some((card) => card.cardId === "EX8-020") && s.state.players[0]!.hand.some((card) => card.cardId === "EX8-027"));
