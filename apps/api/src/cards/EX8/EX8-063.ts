...
const fallenAngel = { controller: "mine", kind: ["Digimon"], playCostLte: 7, nameOrTrait: [{ tokens: ["Fallen Angel"], match: "trait" }] };
...
export const compiled: CompiledCard = {
  effects: [
    ...(["WhenDigivolving", "WhenAttacking"] as const).map((trigger) => ({
      ...
      actions: [
        { kind: "Trash", chooser: "opponent", target: { controller: "opponent", filter: { zone: "hand" }, count: 1 }, optional: true },
        { kind: "PlayWithoutCost", from: ["trash"], payCost: false, optional: true, condition: { kind: "ifThisEffectDidNotAct" }, target: { filter: fallenAngel, count: 1 } },
      ],
    })),
    {
      trigger: "AllTurns",
