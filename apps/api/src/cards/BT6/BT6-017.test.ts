...
describe("BT6-017 MagnaKidmon", () => {
  ...
  it("uses a cost-7 Option from hand for free", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "AD1-002", as: "base" }], hand: [{ card: "BT6-017", as: "evolving" }, { card: "BT6-095", as: "option" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
  ...
  it("deletes a 4000 DP opposing Digimon when the Option branch is declined", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "AD1-002", as: "base" }], hand: [{ card: "BT6-017", as: "evolving" }, { card: "BT6-095", as: "option" }] },
    ...
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("evolving").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[1]?.battleArea.some((permanent) => permanent.permanentId === targetId) === false);
