...
describe("BT6-072 Ogremon", () => {
  it("may trash a hand card to delete an opposing level 4 or lower Digimon", async () => {
    ...
    const s = setupEngine(
      {
        0: {
          hand: [
            { card: "BT6-072", as: "source" },
            { card: "BT6-073", as: "discard" },
          ],
        },
    ...
    expect((s.state.players[0] as PlayerState).trash.map((card) => card.instanceId)).toContain(
      s.inst("discard").instanceId,
    );
  });
