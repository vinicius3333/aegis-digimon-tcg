...
describe("BT6-086 Eosmon", () => {
  it("places Eosmon from trash for each Tamer and deletes after placing at least 2", async () => {
    ...
    const s = setupEngine({
      0: {
        ...
        trash: [{ card: "BT6-085", as: "first" }, { card: "BT6-085", as: "second" }],
      },
    ...
    expect(s.perm("base").stack.map((card) => card.instanceId)).toEqual(expect.arrayContaining([
  ...
  it("gains Security Attack +1 for every 3 digivolution cards", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT6-086", under: ["BT6-085", "BT6-085", "BT6-085", "BT6-085", "BT6-085", "BT6-085"], as: "eosmon" }] } });
    await s.ready();
