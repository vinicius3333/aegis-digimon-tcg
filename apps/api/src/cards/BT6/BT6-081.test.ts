...
describe("BT6-081 Titamon", () => {
  it("trashes a hand card then plays a purple level 4 from trash", async () => {
    ...
    const s = setupEngine({ 0: {
      ...
      hand: [{ card: "BT6-081", as: "evolving" }, { card: "BT1-010", as: "discard" }],
    ...
    expect(player.battleArea.some((permanent) => permanent.topCard.instanceId === s.inst("played").instanceId)).toBe(true);
  ...
  it("gains +2000 DP and Security Attack +1 only once per turn when a hand card is trashed", async () => {
    const s = setupEngine({ 0: {
      ...
      hand: [{ card: "BT1-010", as: "first" }, { card: "BT1-011", as: "second" }],
    } });
    await s.ready();
