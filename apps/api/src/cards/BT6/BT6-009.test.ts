...
describe("BT6-009 Huckmon", () => {
  it("offers every unchosen revealed card for explicit deck-bottom ordering", async () => {
    ...
    const offered = [s.inst("restOne").instanceId, s.inst("restTwo").instanceId, s.inst("restThree").instanceId];
    ...
    await settle(
      () => s.state.pendingDecision === undefined && s.state.players[0]!.deck[0]?.instanceId === requestedOrder[0],
    );
