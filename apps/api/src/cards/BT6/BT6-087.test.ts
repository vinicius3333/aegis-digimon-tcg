...
describe("BT6-087 Tai Kamiya", () => {
  it("gains 1 memory and draws 1 when Agumon moves from breeding", async () => {
    ...
    expect(s.engine.applyIntent(0, { type: "moveFromBreeding", permanentId: s.perm("agumon").permanentId })).toEqual({ ok: true });
  ...
  it("digivolves Agumon into Bond of Bravery and trashes 2 security", async () => {
    ...
    const s = setupEngine({ 0: { battleArea: [{ card: "BT1-010", as: "agumon" }, { card: "BT6-087", as: "tai" }], hand: [{ card: "BT6-018", as: "bond" }], security: ["BT1-001", "BT1-002", "BT1-003"] } }, { autoAcceptOptional: true, autoSelectCards: true, preferInstanceIds: preferred });
    preferred.push(s.perm("agumon").topCard!.instanceId);
    s.state.memory = 5;

    expect(s.engine.applyIntent(0, { type: "activateEffect", sourceInstanceId: s.perm("tai").topCard!.instanceId, effectKey: "BT6-087/main-digivolve-bond-of-bravery" })).toEqual({ ok: true });
    await settle(() =>
