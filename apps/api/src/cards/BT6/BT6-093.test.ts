...
describe("BT6-093 Judgment of the Blade", () => {
  it("may play Sistermon from security, then adds itself to hand", async () => {
    ...
    expect(s.state.players[0]!.battleArea.some((permanent) => permanent.topCard.instanceId === s.inst("sistermon").instanceId)).toBe(true);
  ...
  it("exposes eligible Huckmon-family Digimon as board targets", async () => {
    ...
    await settle(() => {
      ...
      return latest !== undefined &&
        ...
        latest.kind === "chooseTargets" && latest.sourceCardId === "BT6-093";
    });
    ...
    await settle(() =>
      ...
      s.state.players[0]!.trash.some((card) => card.cardId === "BT6-093"),
    );
