...
describe("BT6-028 Pukumon", () => {
  it("Digi-Bursts 2 to prevent all own Digimon from being blocked for the turn", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "BT6-028", under: ["BT1-001", "BT1-002"], as: "pukumon" }, { card: "BT1-010", as: "attacker" }] },
    ...
    const effectKey = effectsOf(EffectTiming.OnDeclaration, source).find((effect) => effect.effectKey.startsWith("BT6-028/"))!.effectKey;

    expect(s.engine.applyIntent(0, { type: "activateEffect", sourceInstanceId: s.perm("pukumon").topCard!.instanceId, effectKey })).toEqual({ ok: true });
    await settle(() =>
