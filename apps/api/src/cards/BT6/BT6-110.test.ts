...
describe("BT6-110 Cutting Edge", () => {
  it("activates its Main effect from security", async () => {
    ...
    expect(s.state.players[0]!.battleArea.some((permanent) => permanent.topCard.instanceId === s.inst("eosmon").instanceId)).toBe(true);
  ...
  it("deletes only an opponent Digimon whose live DP is at most the Eosmon this effect played", async () => {
    const s = setupEngine({
      0: {
        battleArea: ["BT6-082"],
        hand: [{ card: "BT6-110", as: "option" }, { card: "BT6-085", as: "eosmon" }],
      },
      1: { battleArea: [{ card: "BT6-044", as: "tooLarge", dp: 7000 }, { card: "BT1-014", as: "eligible", dp: 4000 }] },
    ...
    expect(s.state.players[0]!.battleArea.some((permanent) => permanent.topCard?.instanceId === eosmonInstanceId)).toBe(true);
