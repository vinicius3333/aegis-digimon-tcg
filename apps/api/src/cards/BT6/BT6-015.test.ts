...
describe("BT6-015 SaviorHuckmon", () => {
  it("plays a Sistermon from hand for free when digivolving", async () => {
    const s = setupEngine({ 0: {
      ...
      hand: [{ card: "BT6-015", as: "evolving" }, { card: "BT6-084", as: "sistermon" }],
    } }, { autoAcceptOptional: true, autoSelectCards: true });
    const sistermonId = s.inst("sistermon").instanceId;
    s.state.memory = 3;

    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("evolving").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.battleArea.some((permanent) => permanent.topCard?.instanceId === sistermonId));
