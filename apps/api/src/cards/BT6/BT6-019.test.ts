...
describe("BT6-019 Gabumon", () => {
  it("gains memory only once per turn when matching Matt Ishida Tamers are played", async () => {
    const s = setupEngine({ 0: {
      ...
      hand: [{ card: "BT1-086", as: "firstMatt" }, { card: "BT1-086", as: "secondMatt" }],
    } }, { autoSelectCards: true });
    s.state.memory = 10;
    await s.ready();

    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("firstMatt").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.memory === 7);
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("secondMatt").instanceId })).toEqual({ ok: true });
