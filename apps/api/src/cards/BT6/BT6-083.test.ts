...
describe("BT6-083 Eosmon", () => {
  ...
  it("may play a white Tamer, then lets the opponent play a Tamer", async () => {
    const s = setupEngine({
      0: { hand: [{ card: "BT6-083", as: "source" }, { card: "BT6-092", as: "myTamer" }] },
    ...
    await settle(() => mine.battleArea.some((p) => p.topCard?.instanceId === s.inst("myTamer").instanceId) && opponent.battleArea.some((p) => p.topCard?.instanceId === s.inst("opponentTamer").instanceId));
