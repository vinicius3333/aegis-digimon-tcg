...
describe("BT6-018 Agumon - Bond of Bravery", () => {
  it("deletes up to 13000 DP when attacking and trashes security only once per turn on deletion", async () => {
    const s = setupEngine({
      ...
      1: {
        battleArea: [{ card: "BT6-016", dp: 13000, as: "first", suspended: true }, { card: "BT1-009", as: "second" }],
    ...
    expect(engine.applyIntent(0, { type: "attack", attackerPermanentId: s.perm("bond").permanentId, target: { kind: "permanent", permanentId: firstId } })).toEqual({ ok: true });
    await settle(() =>
