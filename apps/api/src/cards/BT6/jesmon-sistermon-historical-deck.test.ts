    ...
    await settle(
      () =>
        ...
        s.state.players[0]!.battleArea.some(
          (permanent) => permanent.topCard?.cardId === "BT6-082" && observe(s.engine).hasKeyword(permanent, "Blocker"),
        ) &&
        s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("blancDraw").instanceId),
      5000,
    );

    const blanc = s.state.players[0]!.battleArea.find((permanent) => permanent.topCard?.cardId === "BT6-082");
