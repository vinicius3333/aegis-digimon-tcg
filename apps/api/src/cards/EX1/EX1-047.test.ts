...
describe("EX1-047 Guardromon", () => {
  it("has Blocker and can't attack on your turn", async () => {
    ...
    expect(s.engine.applyIntent(0, { type: "attack", attackerPermanentId: s.perm("guardromon").permanentId, target: { kind: "player" } }).ok).toBe(false);
  });

  it("trashes a Machine card to draw 2 through its inherited attack effect", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT1-042", as: "host", under: ["EX1-047"] }], hand: [{ card: "BT1-068", as: "machine" }], deck: ["BT1-009", "BT1-010"] }, 1: { security: ["BT1-001", "BT1-001"] } }, { autoAcceptOptional: true, autoSelectCards: true });
    await s.ready();
