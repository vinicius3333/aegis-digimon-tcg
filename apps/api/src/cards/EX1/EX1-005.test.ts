...
describe("EX1-005 Tyrannomon", () => {
  it("plays only Taiga for free when digivolving if none is in play", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-001", as: "base" }], hand: [{ card: "BT1-085", as: "notTaiga" }, { card: "BT2-088", as: "taiga" }, { card: "EX1-005", as: "evo" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 4;
  ...
  it("does not play a second Taiga when one is already in play", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-001", as: "base" }, { card: "BT2-088", as: "fieldTaiga" }], hand: [{ card: "BT2-088", as: "handTaiga" }, { card: "EX1-005", as: "evo" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 4;
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("evo").instanceId })).toEqual({ ok: true });
