...
describe("EX1-053 MetalEtemon", () => {
  it("gets +1000 DP per Etemon card in trash on the opponent's turn", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-053", as: "metalEtemon", dp: 11000 }], trash: ["EX1-052", "EX1-053"] } });
    s.state.turnSeat = 1;
    await s.ready();
  ...
  it("de-digivolves an opposing Digimon by 1 on deletion", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-053", as: "metalEtemon" }] }, 1: { battleArea: [{ card: "EX1-054", as: "target", under: [{ card: "EX1-050", as: "revealed" }] }] } }, { autoSelectCards: true });
