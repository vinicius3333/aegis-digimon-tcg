...
describe("EX1-021 MetalGarurumon", () => {
  it("gains 1 memory for every 4 cards in hand when digivolving", async () => {
    ...
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-017", as: "base" }], hand: [...filler, { card: "EX1-021", as: "evo" }] } });
    s.state.memory = 5;
  ...
  it("bottom-decks an On Deletion Digimon and trashes all of its sources with 8 cards and a Tamer", async () => {
    ...
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-021", as: "metalGarurumon" }, { card: "ST2-12", as: "tamer" }], hand }, 1: { battleArea: [{ card: "EX1-034", as: "target", under: [{ card: "BT1-069", as: "source1" }, { card: "BT1-070", as: "source2" }] }], security: ["BT1-001", "BT1-001"] } }, { autoSelectCards: true });
    const sourceIds = [s.inst("source1").instanceId, s.inst("source2").instanceId];
    await s.ready();
    expect(s.engine.applyIntent(0, { type: "attack", attackerPermanentId: s.perm("metalGarurumon").permanentId, target: { kind: "player" } })).toEqual({ ok: true });
