...
describe("EX1-021 MetalGarurumon", () => {
  ...
  it("does not return a target when eight cards are held but no Tamer is in play", async () => {
    ...
    const s = setupEngine({
      ...
      1: { battleArea: [{ card: "EX1-034", as: "target", dp: 10000, suspended: true, under: ["BT1-007"] }], security: ["BT1-001", "BT1-001"] },
    }, { autoSelectCards: true });
    await s.ready();
  ...
  it("does not return a target with a Tamer when fewer than eight cards are held", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "EX1-021", as: "metalGarurumon", under: ["EX1-017"] }, { card: "ST2-12", as: "tamer" }], hand: Array.from({ length: 7 }, () => "BT1-029") },
      1: { battleArea: [{ card: "EX1-034", as: "target", dp: 10000, suspended: true, under: ["BT1-007"] }], security: ["BT1-001", "BT1-001"] },
    }, { autoSelectCards: true });
    await s.ready();
    expect(s.engine.applyIntent(0, { type: "attack", attackerPermanentId: s.perm("metalGarurumon").permanentId, target: { kind: "player" } })).toEqual({ ok: true });
