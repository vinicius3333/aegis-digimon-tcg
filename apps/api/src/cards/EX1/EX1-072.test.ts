...
describe("EX1-072 Emergency Program Shutdown!", () => {
  it("prevents the opponent from using Option cards until the end of their next turn", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "EX1-072", as: "shutdown" }], battleArea: [{ card: "BT11-095", as: "blueSource" }] }, 1: { hand: [{ card: "EX1-069", as: "opponentOption" }], battleArea: [{ card: "EX1-047", as: "blackSource" }] } });
    s.state.memory = 10;
  ...
  it("from security restricts the opponent for the turn and returns to its owner's hand", async () => {
    ...
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("opponentOption").instanceId }).ok).toBe(false);
  });
});
