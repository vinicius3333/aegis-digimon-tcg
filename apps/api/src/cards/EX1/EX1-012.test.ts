...
describe("EX1-012 Gomamon", () => {
  it("trashes the bottom digivolution card of an opposing Digimon on play", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "EX1-012", as: "gomamon" }] }, 1: { battleArea: [{ card: "BT1-032", as: "target", under: [{ card: "BT1-029", as: "bottom" }, { card: "BT1-030", as: "topSource" }] }] } }, { autoSelectCards: true });
