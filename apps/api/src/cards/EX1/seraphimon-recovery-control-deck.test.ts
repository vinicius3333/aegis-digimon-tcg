...
describe("EX1 Seraphimon recovery control deck", () => {
  it("turns each Recovery into one card and caps both inherited reactions once per turn", async () => {
    ...
    expect(new Set(dpDecision.options?.candidateInstanceIds)).toEqual(
      new Set([s.perm("firstTarget").permanentId, s.perm("secondTarget").permanentId]),
    );
    expect(dpDecision.options?.candidateInstanceIds).not.toContain(s.perm("firstTarget").topCard.instanceId);
    expect(
      s.engine.applyIntent(0, {
    ...
    await settle(
      () =>
        s.state.players[0]!.security.length === 1 && s.state.memory === 7 && s.perm("secondTarget").currentDP === 3000,
    );
    ...
    expect(
      s.decisions.filter(({ req }) => req.sourceCardId === "EX1-030" && req.kind === "chooseTargets"),
    ).toHaveLength(1);
    assertNoLoudGap(s);
  });
});
