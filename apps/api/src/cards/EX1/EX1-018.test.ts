...
describe("EX1-018 Zudomon", () => {
  it("trashes the bottom digivolution card when digivolving", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-014", as: "base" }], hand: [{ card: "EX1-018", as: "evo" }] }, 1: { battleArea: [{ card: "BT1-032", as: "target", under: [{ card: "BT1-029", as: "bottom" }, "BT1-030"] }] } }, { autoSelectCards: true });
  ...
  it("can attack an unsuspended Digimon only when it has no digivolution cards", async () => {
    ...
    await s.ready();
    expect(s.engine.applyIntent(0, { type: "attack", attackerPermanentId: s.perm("zudomon").permanentId, target: { kind: "permanent", permanentId: s.perm("eligible").permanentId } })).toEqual({ ok: true });
  });
});
