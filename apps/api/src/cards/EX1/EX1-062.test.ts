...
describe("EX1-062 SkullGreymon", () => {
  it("has Security Attack +1, deletes itself after attacking, then may play exactly Agumon from trash", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-062", as: "skull" }], trash: [{ card: "BT1-010", as: "agumon" }] }, 1: { security: ["BT1-001", "BT1-001", "BT1-001"] } }, { autoAcceptOptional: true, autoSelectCards: true });
    await s.ready();
    expect(observe(s.engine).keywordAmount(s.perm("skull"), "SecurityAttack")).toBe(1);
    expect(s.engine.applyIntent(0, { type: "attack", attackerPermanentId: s.perm("skull").permanentId, target: { kind: "player" } })).toEqual({ ok: true });
  ...
  it("can play Agumon from its own stack after deleting itself", async () => {
    ...
    await settle(() => s.state.players[0]!.battleArea.some(
      (permanent) => permanent.topCard.instanceId === stackAgumonId,
    ));
