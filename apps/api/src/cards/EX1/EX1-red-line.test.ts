  ...
  it("combines search, Tamer play, Dragonkin Piercing, and Blocker deletion", async () => {
    const s = setupEngine({
      0: {
        ...
        deck: [
          { card: "EX1-001", as: "searchedAgumon" },
          "BT1-001",
          "BT1-002",
          "BT1-003",
        ],
    ...
    await settle(() =>
      s.state.players[0]!.battleArea.some((permanent) =>
        permanent.topCard.instanceId === s.inst("playedTai").instanceId,
      ) &&
      s.state.players[0]!.hand.some((card) =>
        card.instanceId === s.inst("searchedAgumon").instanceId,
      ) &&
    ...
    expect(s.state.players[0]!.hand.map((card) => card.instanceId)).toContain(
      s.inst("searchedAgumon").instanceId,
    );
    assertNoLoudGap(s);
  });
});
