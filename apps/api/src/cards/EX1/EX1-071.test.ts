...
describe("EX1-071 Win Rate: 60%!", () => {
  ...
  it("trashes a same-color Digimon to reduce the next battle-area digivolution by 4", async () => {
    ...
    const s = setupEngine({ 0: { hand: [{ card: "EX1-071", as: "option" }, { card: "EX1-052", as: "evo" }, { card: "EX1-050", as: "cost" }], battleArea: [{ card: "EX1-047", as: "base" }, { card: "BT1-085", as: "tamer" }], deck: ["BT1-009"] } }, { autoAcceptOptional: true, autoSelectCards: true, autoOrderTriggers: true, preferInstanceIds });
  ...
  it("does not trash a different-color Digimon or reduce the evolution", async () => {
    ...
    await settle(() =>
      s.state.memory === 8 &&
      s.state.players[0]!.trash.some((card) => card.cardId === "EX1-071"),
    );
    const before = s.state.memory;
    ...
    const control = setupEngine({
      0: {
        hand: [{ card: "EX1-052", as: "controlEvo" }, { card: "ST1-03", as: "controlRed" }],
        battleArea: [
    ...
    await control.ready();
    expect(control.engine.applyIntent(0, {
      type: "digivolve",
    ...
    expect(s.state.players[0]!.hand.map((card) => card.instanceId)).toContain(
      s.inst("redCard").instanceId,
    );
  });

  it("adds itself to hand from security", async () => {
    const s = setupEngine({ 0: { security: [{ card: "EX1-071", as: "securityOption", faceUp: true }] } }, { autoOrderTriggers: true });
