...
function mainEffectKey(s: ReturnType<typeof setupEngine>, alias: string): string {
  const source = (s.engine as unknown as { cardSourceOf: (card: unknown) => unknown }).cardSourceOf(s.perm(alias).topCard);
  return effectsOf(EffectTiming.OnDeclaration, source as never).find((effect) => effect.effectKey.startsWith("ST4-13/"))!.effectKey;
}
...
describe("EX1-020 Plesiomon", () => {
  ...
  it("draws 2 when an opponent's digivolution card is trashed", async () => {
    ...
    await s.ready();
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("de-digivolver").instanceId })).toEqual({ ok: true });
  ...
  it("does not draw when your own digivolution card is trashed", async () => {
    ...
    await s.ready();
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "activateEffect", sourceInstanceId: s.perm("own").topCard.instanceId, effectKey: mainEffectKey(s, "own") })).toEqual({ ok: true });
  ...
  it("draws only once when two opponent sources are trashed in one turn", async () => {
    const s = setupEngine({
      0: {
        battleArea: [{ card: "EX1-020", as: "plesiomon" }],
        hand: [{ card: "BT14-083", as: "first" }, { card: "BT14-083", as: "second" }],
  ...
  it("does not draw during the opponent's turn", async () => {
    const s = setupEngine({
      ...
      1: { battleArea: [{ card: "ST4-13", as: "opponent", under: ["ST4-03", "ST4-08"] }], hand: ["BT1-009"], deck: ["BT1-001"] },
    }, { autoAcceptOptional: true, autoSelectCards: true });
    ...
    await s.ready();
    expect(s.engine.applyIntent(1, { type: "activateEffect", sourceInstanceId: s.perm("opponent").topCard.instanceId, effectKey: mainEffectKey(s, "opponent") })).toEqual({ ok: true });
    await settle(() => s.perm("opponent").stack.length === 0 && s.state.pendingDecision === undefined);
