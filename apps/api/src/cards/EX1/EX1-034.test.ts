...
describe("EX1-034 Palmon", () => {
  it("suspends an opposing Digimon with 5000 DP or less on deletion", async () => {
    const s = setupEngine(
      {
        ...
        1: {
          battleArea: [{ card: "BT1-080", as: "attacker" }, { card: "BT1-070", as: "target", dp: 5000 }],
    ...
    await s.ready();
    expect(s.engine.applyIntent(0, { type: "attack", attackerPermanentId: s.perm("palmon").permanentId, target: { kind: "player" } })).toEqual({ ok: true });
    await settle(() => s.perm("palmon").isSuspended);
    expect(s.engine.applyIntent(0, { type: "endPhase" })).toEqual({ ok: true });
    await advance(s.engine).waitForMainPhase(1);
    await s.ready();
    expect(s.engine.applyIntent(1, { type: "attack", attackerPermanentId: s.perm("attacker").permanentId, target: { kind: "permanent", permanentId: s.perm("palmon").permanentId } })).toEqual({ ok: true });
    ...
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "EX1-034", as: "palmon" }], hand: ["BT1-009"], deck: ["BT1-001", "BT1-001"] },
        1: {
          battleArea: [{ card: "BT1-080", as: "attacker" }, { card: "BT1-070", as: "target", dp: 6000 }],
          security: ["BT1-001", "BT1-001"],
          hand: ["BT1-009"],
          deck: ["BT1-001", "BT1-001"],
        },
