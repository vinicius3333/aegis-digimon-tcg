...
describe("EX1-032 Magnadramon", () => {
  it("may trash the top security card to unsuspend when digivolving", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-029", as: "base", suspended: true }], hand: [{ card: "EX1-032", as: "evo" }], security: [{ card: "BT1-009", as: "security" }] } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 5;
  ...
  it("recovers 1 on attack with 3 or fewer security", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-032", as: "magnadramon" }], security: ["BT1-001", "BT1-001", "BT1-001"], deck: [{ card: "BT1-009", as: "recovered" }] }, 1: { security: ["BT1-001", "BT1-001"] } });
    await s.ready();
