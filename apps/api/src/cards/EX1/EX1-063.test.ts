...
describe("EX1-063 VenomMyotismon", () => {
  it("has Retaliation and once per turn may play a purple level-4 Retaliation Digimon from trash", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-063", as: "venom" }], trash: [{ card: "EX1-056", as: "played" }] }, 1: { security: ["BT1-001", "BT1-001"] } }, { autoAcceptOptional: true, autoSelectCards: true });
    await s.ready();
    expect(observe(s.engine).hasKeyword(s.perm("venom"), "Retaliation")).toBe(true);
    expect(s.engine.applyIntent(0, { type: "attack", attackerPermanentId: s.perm("venom").permanentId, target: { kind: "player" } })).toEqual({ ok: true });
  ...
  it("suppresses the played Digimon's On Play effect", async () => {
    ...
    await settle(() => s.state.players[0]!.battleArea.some((permanent) =>
      permanent.topCard.instanceId === s.inst("kaiserLeomon").instanceId,
    ));

    expect(s.state.players[1]!.battleArea.some((permanent) =>
      permanent.permanentId === s.perm("levelFour").permanentId,
    )).toBe(true);
  });
});
