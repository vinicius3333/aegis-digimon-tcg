...
describe("EX1-009 WarGreymon", () => {
  it("has Blitz and deletes an opposing Blocker when attacking a player with a Tamer", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX1-009", as: "attacker" }, { card: "ST1-12", as: "tamer" }] }, 1: { battleArea: [{ card: "BT1-072", as: "blocker" }, { card: "BT1-010", as: "nonBlocker" }], security: ["BT1-001", "BT1-001"] } }, { autoSelectCards: true });
