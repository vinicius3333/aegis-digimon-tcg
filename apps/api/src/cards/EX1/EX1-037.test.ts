...
describe("EX1-037 Kuwagamon", () => {
  it("suspends an opposing Digimon with 3000 DP or less at start of your turn", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "EX1-037", as: "kuwagamon" }] },
        1: { battleArea: [{ card: "BT1-066", as: "target", dp: 3000 }] },
      },
  ...
  it("prevents an opposing suspended Digimon from unsuspending after its host wins a battle", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "EX1-040", as: "host", under: ["EX1-037"] }] },
        1: { battleArea: [{ card: "BT1-070", as: "target", suspended: true }] },
      },
      { autoSelectCards: true },
    );
    await s.ready();
