...
const auditDocSource = readFileSync(new URL("../../../../../docs/audits/EX11.md", import.meta.url), "utf8");
const ex12Cards = allCards()
  .filter((card) => card.set === "EX11")
...
describe("EX11 collection audit proof", () => {
  it("matches the complete committed EX11 catalog inventory", () => {
    const expectedIds = Array.from({ length: 74 }, (_, index) => `EX11-${String(index + 1).padStart(3, "0")}`);
  ...
  it("documents every audited card in the collection audit doc", () => {
    ...
    const documented = auditDocSource
      .split("\n")
      .map((line) => line.match(/^### (EX11-\d{3}) — /)?.[1])
