...
const ex12Ids = Array.from({ length: 74 }, (_, index) => `EX11-${String(index + 1).padStart(3, "0")}`);
...
describe("EX11 persisted IR", () => {
  it("contains exactly the authoritative EX11 card keys", () => {
    const persistedCardIds = Object.keys(catalog)
      .filter((cardId) => /^EX11-\d{3}$/.test(cardId))
      .sort();
