...
describe("EX11-018 Ryugumon", () => {
  it("places an Aqua/Sea Animal card under itself and unsuspends one Digimon", async () => {
    const s = setupEngine(
      {
        0: {
          hand: [{ card: "EX11-018", as: "ryugumon" }, "BT1-033"],
          battleArea: [{ card: "EX11-018", as: "ally", suspended: true }],
        },
      },
    ...
    await settle(
      () =>
        s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "EX11-018" && perm.stack.length === 1),
      600,
    );
    expect(
      s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "EX11-018" && perm.stack.length === 1),
    ...
    expect(
      s.state.players[0]!.battleArea.some((perm) => perm.topCard?.cardId === "EX11-018" && !perm.isSuspended),
    ).toBe(true);
  });
            ...
            actions: [
              {
                ...
                target: expect.objectContaining({
                  filter: expect.objectContaining({ digivolutionCardsCompareToSource: "lte" }),
                }),
              },
            ],
          },
