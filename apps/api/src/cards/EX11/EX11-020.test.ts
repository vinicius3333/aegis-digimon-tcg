...
describe("EX11-020 Hanimon", () => {
  it("plays a Shoemon from hand when deleted by an effect", async () => {
    const s = setupEngine(
      { 0: { battleArea: [{ card: "EX11-020", as: "hanimon", dp: 1000 }], hand: ["EX11-019"] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );
    await (
      s.engine as unknown as { primitives: { deletePermanent(ids: string[], cause?: string): Promise<number> } }
    ).primitives.deletePermanent([s.perm("hanimon").permanentId], "byEffect");
      ...
      actions: [
        {
          ...
          payCost: false,
          optional: true,
          condition: { kind: "not", raw: "deleted other than in battle" },
        },
      ],
    });
