...
describe("EX7-022 ShogunGekomon", () => {
  it("suspends one opposing Digimon/Tamer on play", () => expect(compiled.effects?.find((entry) => entry.trigger === "OnPlay")?.actions[0]).toMatchObject({ kind: "Suspend", target: { count: 1, filter: { controller: "opponent", kind: ["Digimon", "Tamer"] } } }));
  ...
  it("suspends one opposing Digimon on play", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "EX7-022", as: "shogun" }] }, 1: { battleArea: [{ card: "BT1-009", as: "target" }] } }, { autoSelectCards: true });
    await s.ready();
