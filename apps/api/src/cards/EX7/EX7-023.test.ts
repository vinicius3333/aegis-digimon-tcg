...
describe("EX7-023 Hexeblaumon", () => {
  it("has Security Attack +1 and Ice Clad and trashes four evolution cards on digivolving", () => {
    ...
    expect(compiled.effects?.find((entry) => entry.trigger === "WhenDigivolving")?.actions[0]).toMatchObject({ kind: "TrashDigivolution", amount: 4, scope: "acrossDigimon", fromTop: false });
    expect(compiled.effects?.find((entry) => entry.trigger === "WhenDigivolving")?.actions[1]).toMatchObject({ kind: "Return", condition: { kind: "opponentHasNone" } });
  });
