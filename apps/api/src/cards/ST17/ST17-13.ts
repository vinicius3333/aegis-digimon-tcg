  ...
  effects: [
    ...
    { trigger: "Security", actions: [
      ...
      { kind: "SubTrigger", event: "whenSecurityBattleEnded", once: true, actions: [
        { kind: "Digivolve", target: { filter: { controller: "mine", kind: ["Digimon"] }, count: 1 }, into: { controllerDefault: "mine", nameOrTrait: [{ tokens: ["Magnamon"], match: "name" }] }, from: ["security", "trash"], source: "triggerSource", payCost: false, optional: true },
      ] },
    ], isSecurity: true },
