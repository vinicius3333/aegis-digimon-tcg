...
const st17Ids = allCards()
  .filter((card) => card.set === "ST17")
...
describe("ST17 collection audit proof", () => {
  it("matches the complete committed ST17 catalog inventory", () => {
    expect(st17Ids).toEqual(Array.from({ length: 13 }, (_, index) => `ST17-${String(index + 1).padStart(2, "0")}`));
  });

  it("keeps every catalog card imported, tested, and registered", () => {
    for (const cardId of st17Ids) {
