...
describe("BT22-009 Effecmon", () => {
  ...
  it("plays itself from security without cost at the security timing and then resolves On Play", async () => {
    const s = setupEngine(
      {
        0: { security: [{ card: "BT22-009", as: "securityEffecmon" }] },
        1: { battleArea: [{ card: "BT22-009", dp: 4000, as: "target" }] },
      },
      { autoSelectCards: true },
    );
    await s.ready();
    ...
    const s = setupEngine(
      {
        0: {
          battleArea: [
            ...
            { card: "BT22-009", dp: 3000, as: "victim" },
          ],
        },
  ...
  it("digivolves through the legal red level-3 route and resolves When Digivolving", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "BT1-009", as: "base" }], hand: [{ card: "BT22-009", as: "effecmon" }] },
        1: { battleArea: [{ card: "BT22-009", dp: 4000, as: "target" }] },
      },
      { autoSelectCards: true },
    );
    ...
    expect(
      s.engine.applyIntent(0, {
        ...
        instanceId: s.inst("effecmon").instanceId,
      }),
    ).toEqual({ ok: true });
  ...
  it("accepts the alternate non-red Stnd. level-3 route and rejects a non-Stnd. non-red source", async () => {
    const stnd = setupEngine({
      0: { battleArea: [{ card: "BT22-030", as: "stndBase" }], hand: [{ card: "BT22-009", as: "effecmon" }] },
    });
    await stnd.ready();
    stnd.state.memory = 3;
    expect(
      stnd.engine.applyIntent(0, {
        type: "digivolve",
        permanentId: stnd.perm("stndBase").permanentId,
        instanceId: stnd.inst("effecmon").instanceId,
      }).ok,
    ).toBe(true);
    await settle(() => stnd.perm("stndBase").topCard?.cardId === "BT22-009");
    expect(stnd.perm("stndBase").topCard?.cardId).toBe("BT22-009");
    const invalid = setupEngine({
      0: { battleArea: [{ card: "BT22-019", as: "nonStndBase" }], hand: [{ card: "BT22-009", as: "effecmon" }] },
    });
    await invalid.ready();
    ...
    expect(
      invalid.engine.applyIntent(0, {
        type: "digivolve",
        permanentId: invalid.perm("nonStndBase").permanentId,
        instanceId: invalid.inst("effecmon").instanceId,
      }).ok,
    ).toBe(false);
  });
  ...
  it("links from hand for 2 memory, adds 3000 DP, and resolves the 4000-DP boundary", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "BT22-035", as: "host" }], hand: [{ card: "BT22-009", as: "link" }] },
      1: {
        battleArea: [
          { card: "BT22-009", dp: 4000, as: "eligible" },
          { card: "BT22-010", dp: 5000, as: "tooLarge" },
  ...
  it("links Effecmon through BT22-039's effect-driven play on attack to a legal Appmon recipient", async () => {
    ...
    expect(
      s.state.players[0]!.battleArea.some((permanent) =>
        permanent.linked.some((card) => card.instanceId === s.inst("effecmon").instanceId),
      ),
    ).toBe(true);
    expect(s.state.players[1]!.trash.some((card) => card.cardId === "BT22-009")).toBe(true);
    expect(
