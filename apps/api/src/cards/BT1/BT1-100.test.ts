...
describe("BT1-100 Grace Cross Freezer", () => {
  it("prevents all opposing Digimon without sources from attacking", async () => {
    const s = setupEngine({ 0: { battleArea: ["BT1-028"], hand: [{ card: "BT1-100", as: "option" }] }, 1: { battleArea: [{ card: "BT1-010", as: "noSources" }, { card: "BT2-047", as: "withSource", under: ["BT1-001"] }] } }, { autoSelectCards: true });
    s.state.memory = 4;
  ...
  it("prevents opposing source-less Digimon from attacking from security", async () => {
    const s = setupEngine({ 0: { security: [{ card: "BT1-100", as: "securityOption", faceUp: true }] }, 1: { battleArea: [{ card: "BT1-010", as: "noSources" }, { card: "BT2-047", as: "withSource", under: ["BT1-001"] }] } });
