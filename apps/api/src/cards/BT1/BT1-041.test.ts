...
describe("BT1-041 Zudomon", () => {
  it("draws two cards on play", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "BT1-041", as: "zudomon" }], deck: [
      { card: "BT1-029", as: "drawnA" }, { card: "BT1-030", as: "drawnB" },
    ] } });
    ...
    expect(player.hand.map((card) => card.instanceId)).toEqual(expect.arrayContaining([s.inst("drawnA").instanceId, s.inst("drawnB").instanceId]));
  });
