...
describe("BT1 option IR coverage", () => {
  it("registers complete IR and preserves main/security clauses", () => {
    ...
    expect(nuclearLaser.effects[0]?.actions).toMatchObject([{ kind: "Draw", amount: 2 }, { kind: "ModifyDP", amount: 2000 }]);
    expect(boringStorm.effects).toMatchObject([{ trigger: "Main" }, { trigger: "Security" }]);
    expect(blastFire.effects[0]?.actions[0]).toMatchObject({ kind: "SetBaseDP", value: 3000, duration: "untilOpponentTurnEnd" });
