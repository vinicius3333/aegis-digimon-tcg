  ...
  effectsForTiming(timing: EffectTiming, source: CardSource): Effect[] {
    ...
    return [
      whenAttacking({
        ...
        description: "[When Attacking] Suspend 1 opposing Digimon with 3000 DP or less.",
        isInherited: true,
        resolve: async (ctx) => {
          const candidates = ctx.game
            .player(ctx.game.opponentOf(source.ownerSeat))
            .battleArea.filter(
              (p) => p.topCard !== undefined && isDigimon(ctx.game.definitionOf(p.topCard)) && p.currentDP <= 3000,
            )
            .map((p) => p.permanentId);
