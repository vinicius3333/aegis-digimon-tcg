...
describe("BT1-039 Cerberusmon", () => {
  it("can trash 3 cards to unsuspend when attacking, up to twice per turn", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "BT1-039", as: "attacker", dp: 20000 }], hand: ["BT1-010", "BT1-011", "BT1-012", "BT1-013", "BT1-014", "BT1-015"] },
    ...
    await settle(() => s.state.players[0]!.hand.length === 3 && !s.perm("attacker").isSuspended && !combat.combat.isAttacking);
    expect(attack()).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.hand.length === 0 && !s.perm("attacker").isSuspended && !combat.combat.isAttacking);
    expect(attack()).toEqual({ ok: true });
