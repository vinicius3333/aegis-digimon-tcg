...
const compiled: CompiledCard = {
  effects: [{ trigger: "YourTurn", actions: [{ kind: "SubTrigger", event: "onDeletionOf", sourceFilter: { controller: "opponent", kind: ["Digimon"] }, fireCondition: { kind: "triggerDeletedByDpZero" }, actions: [{ kind: "ModifyDP", target: { filter: { isSelfRef: true }, count: 1, isSelf: true }, amount: 1000, duration: "forTheTurn" }], raw: "When an opposing Digimon is deleted by dropping to 0 DP, this Digimon gets +1000 DP for the turn." }], isInherited: true, frequency: "OncePerTurn" }],
  coverage: "full",
  residual: [],
};
