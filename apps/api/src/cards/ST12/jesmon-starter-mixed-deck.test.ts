...
describe("ST12 Jesmon starter — mixed archetype flow", () => {
  it("chains red-egg Option access, Delay evolution, unsuspended battle and both Sistermon engines", async () => {
    ...
    expect(s.state.players[0]!.hand.some(
      (card) => card.instanceId === s.inst("revealHit").instanceId,
    )).toBe(true);
    expect(s.state.players[0]!.trash.map((card) => card.instanceId)).toEqual(expect.arrayContaining([
      s.inst("revealMissA").instanceId,
      s.inst("revealMissB").instanceId,
    ]));
    ...
    const delayEntry = (JSON.parse(delay.activatableEffectsJson) as Array<{
      instanceId: string;
      effectKey: string;
      description: string;
    ...
    await settle(() => s.state.players[0]!.trash.some(
      (card) => card.instanceId === delayCardId,
    ));
    await settle();
    ...
    await settle(() => s.perm("baoHuckmon").attackablePermanentIds.includes(
      s.perm("unsuspendedTarget").permanentId,
    ));
    ...
    await settle(() => {
      const sisters = s.state.players[0]!.battleArea.filter((permanent) =>
        permanent.topCard.cardId === "ST12-12" || permanent.topCard.cardId === "BT6-082");
      return sisters.length === 2 && s.state.players[1]!.security.length === 1;
    });

    const starterBlanc = s.state.players[0]!.battleArea.find(
      (permanent) => permanent.topCard.cardId === "ST12-12",
    )!;
    const classicBlanc = s.state.players[0]!.battleArea.find(
