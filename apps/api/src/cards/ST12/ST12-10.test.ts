...
describe("ST12-10 Jesmon", () => {
  it("gains Blitz when digivolving", async () => {
    const s = setupEngine(
      { 0: { battleArea: [{ card: "ST12-08", as: "base" }], hand: [{ card: "ST12-10", as: "evolving" }] } },
  ...
  it("plays a Sistermon when attacking and gains +3000 DP and Security Attack +1", async () => {
    ...
    await settle(
      () =>
        s.state.players[0]!.battleArea.some((p) => p.topCard.instanceId === sisterId) &&
  ...
  it("keeps the Main phase open when a Blitz attack restores memory", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [
            { card: "ST12-08", as: "jesmon" },
            { card: "ST12-03", as: "secondAttacker" },
          ],
          hand: [
            { card: "ST12-10", as: "evolving" },
    ...
    await settle(
      () => !s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("playAfterBlitz").instanceId),
    );
    await turn;
  });
});
