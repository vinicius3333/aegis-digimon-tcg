...
describe("ST10-12 LadyDevimon", () => {
  it("may trash a hand card to add one yellow and one purple Angel-trait card", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "ST10-09", as: "base" }], hand: [{ card: "ST10-12", as: "lady" }, { card: "ST10-07", as: "cost" }], deck: [{ card: "BT1-001", as: "evolutionDraw" }, { card: "ST10-05", as: "yellow" }, { card: "BT3-088", as: "purple" }, { card: "ST10-07", as: "rest" }] } }, { autoAcceptOptional: true, autoOrderTriggers: true, autoSelectCards: true });
    s.state.memory = 3;
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("lady").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.deck.length === 1 && s.state.players[0]!.trash.some((c) => c.instanceId === s.inst("cost").instanceId));
  ...
  it("gives all of your yellow Digimon Retaliation during your turn", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "ST10-13", as: "host", under: ["ST10-12"] }, { card: "ST10-02", as: "yellow" }] } }, { autoOrderTriggers: true });
    await s.ready();
