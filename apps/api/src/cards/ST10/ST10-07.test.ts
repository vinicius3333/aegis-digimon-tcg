...
describe("ST10-07 Ghostmon", () => {
  ...
  it("loses Blocker before reaction timing when the only yellow Digimon is deleted while attacking", async () => {
    const s = setupEngine(
      {
    ...
    expect(observe(s.engine).hasKeyword(s.perm("ghostmon"), "Blocker")).toBe(false);
    expect(s.perm("ghostmon").isSuspended).toBe(false);
  });
});
