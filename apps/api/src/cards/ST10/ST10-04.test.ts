...
describe("ST10-04 Gatomon", () => {
  it("adds one yellow and one purple Digimon from the revealed top 3", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "ST10-04", as: "gatomon" }], deck: [{ card: "ST10-02", as: "yellow" }, { card: "ST10-07", as: "purple" }, { card: "ST10-14", as: "rest" }] } }, { autoOrderTriggers: true, autoSelectCards: true });
    s.state.memory = 5;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("gatomon").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.hand.length === 2);
    expect(s.state.players[0]!.hand.map((c) => c.instanceId)).toEqual(expect.arrayContaining([s.inst("yellow").instanceId, s.inst("purple").instanceId]));
  ...
  it("shows the full reveal and confirms even a single eligible card", async () => {
    ...
    expect(first.options?.visibleInstanceIds).toEqual(expect.arrayContaining([
    ...
    expect(second.options?.visibleInstanceIds).toEqual(expect.arrayContaining([
      s.inst("yellow").instanceId,
      s.inst("purple").instanceId,
      s.inst("ineligible").instanceId,
    ]));
  ...
  it("reduces its digivolution cost by 2 into an Archangel", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "ST10-04", as: "gatomon" }], hand: [{ card: "ST10-05", as: "angewomon" }] } }, { autoOrderTriggers: true });
    s.state.memory = 3;
    await s.ready();
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("gatomon").permanentId, instanceId: s.inst("angewomon").instanceId })).toEqual({ ok: true });
  ...
  it("may DNA digivolve its host and another Digimon at end of turn", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "ST10-05", as: "yellow", under: ["ST10-04"] }, { card: "ST10-12", as: "purple" }], hand: [{ card: "ST10-06", as: "mastemon" }] } }, { autoAcceptOptional: true, autoOrderTriggers: true, autoSelectCards: true });
    await advance(s.engine).fireForInstance(EffectTiming.OnEndTurn, s.perm("yellow").stack.find((c) => c.cardId === "ST10-04")!);
    const result = s.state.players[0]!.battleArea.find((p) =>
      p.topCard.instanceId === s.inst("mastemon").instanceId,
    );
    expect(result).toBeDefined();
    expect(s.state.players[0]!.battleArea).toHaveLength(1);
    expect(result!.stack.map((card) => card.cardId)).toEqual(expect.arrayContaining([
      "ST10-04",
      "ST10-05",
      "ST10-12",
    ]));
  ...
  it("DNA digivolves two dual-color Ophanimon Falldown Mode into Ordinemon", async () => {
    ...
    const ordinemon = s.state.players[0]!.battleArea.find((permanent) =>
  ...
  it("performs the inherited DNA during the real end-of-turn flow", async () => {
    ...
    preferred.push(
      s.perm("yellow").permanentId,
      s.perm("purple").permanentId,
      s.inst("mastemon").instanceId,
    );
    s.state.memory = 3;
    ...
    expect(s.engine.applyIntent(0, { type: "endPhase" })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.battleArea.some((permanent) =>
      permanent.topCard.instanceId === s.inst("mastemon").instanceId,
    ));
    await turn;

    const mastemon = s.state.players[0]!.battleArea.find((permanent) =>
      permanent.topCard.instanceId === s.inst("mastemon").instanceId,
    );
    expect(mastemon).toBeDefined();
    expect(s.state.players[0]!.battleArea).toHaveLength(1);
    expect(mastemon!.stack.map((card) => card.cardId)).toEqual(expect.arrayContaining([
      "ST10-04",
      "ST10-05",
      "ST10-12",
    ]));
    expect(s.state.players[0]!.hand.some((card) =>
      card.instanceId === s.inst("normalEvolution").instanceId,
    )).toBe(true);
  });
  ...
  it("leaves both materials untouched when the optional DNA is declined", async () => {
    ...
    expect(s.state.players[0]!.hand.some((card) =>
      card.instanceId === s.inst("mastemon").instanceId,
    )).toBe(true);
  });
});
