...
describe("ST10-01 Nyaromon", () => {
  it("draws and trashes a card when its host attacks while you have a yellow Digimon", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "ST10-13", as: "host", under: ["ST10-01"] }, "ST10-02"], hand: [{ card: "ST10-11", as: "discard" }], deck: [{ card: "ST10-08", as: "drawn" }] }, 1: { security: ["ST10-14"] } }, { autoOrderTriggers: true, autoSelectCards: true });
    await s.ready();
