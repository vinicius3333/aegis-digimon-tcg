    ...
    if (timing === EffectTiming.OnEnterFieldAnyone) {
      return [
        onPlay({
          ...
          description:
            "[On Play] Reveal the top 3 cards of your deck. Add 1 [Entertainment] trait card " +
            "and 1 [Awakening]/[Tool]/[Navi] trait card among them to the hand. Return the rest " +
          ...
          resolve: async (ctx) => {
            ...
            const entCards = deckCards.filter((c) => hasTrait(ctx.game.definitionOf(c), "Entertainment"));
            const otherCards = deckCards.filter((c) => {
              const def = ctx.game.definitionOf(c);
              return hasTrait(def, "Awakening") || hasTrait(def, "Tool") || hasTrait(def, "Navi");
            });
            const added: string[] = [];
            if (entCards.length > 0) {
              const chosen = await ctx.ask.selectCards(ctx, {
                candidates: entCards.map((c) => c.instanceId),
                min: 0,
                max: 1,
              });
              added.push(...chosen);
    ...
    if (timing === EffectTiming.None) {
      return [
        staticModifier({
          ...
          description:
            "[Your Turn] When any of your Digimon get linked to a [Entertainment]/[Tool]/[Navi] " +
