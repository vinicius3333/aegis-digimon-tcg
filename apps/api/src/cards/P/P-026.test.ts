...
describe("P-026 BlackWarGreymon", () => {
  ...
  it("doesn't unsuspend when X Antibody leaves only 1 trashable Digi-Burst source", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "P-026", as: "blackWarGreymon", suspended: true, under: ["BT9-109", "P-010"] }] },
      },
      { autoSelectCards: true },
    );
  ...
  it("can't activate after turn ownership passes (Q4135)", async () => {
    const s = setupEngine({
      0: {
        battleArea: [{ card: "P-026", as: "blackWarGreymon", suspended: true, under: ["P-032", "P-010"] }],
      },
    });
    s.state.turnSeat = 1;
    await s.ready();
