...
describe("P-187 Mastemon", () => {
  ...
  it("shares one once-per-turn top-security cost across digivolving and attacking", () => {
    const card = runtimeCompiledCard("P-187")!;
    const effects = card.effects.filter((effect) => effect.trigger === "WhenDigivolving" || effect.trigger === "WhenAttacking");
    expect(effects).toHaveLength(4);
    const plays = effects.filter((effect) => effect.actions?.[0]?.kind === "PlayWithoutCost");
    expect(plays).toHaveLength(2);
    expect(plays.map((effect) => effect.sharedUseKey)).toEqual(["trashSecurityPlayDigimon", "trashSecurityPlayDigimon"]);
    expect(plays[0]).toMatchObject({
      frequency: "OncePerTurn",
      actions: [{ kind: "PlayWithoutCost", from: ["hand", "trash"], payCost: false, cost: { kind: "trash", target: { count: 1, filter: { controller: "mine", zone: "security", position: "top" } } }, target: { count: 1, filter: { colors: ["Yellow", "Purple"], dp: { op: "lte", value: 6000 } } } }],
    });
  });
});
