...
describe("P-163 Dokugumon", () => {
  ...
  it("encodes the matching When Digivolving effect and NSo requirement", () => {
    ...
    expect(compiled.effects.find((effect) => effect.trigger === "WhenDigivolving")).toMatchObject({ actions: [{ kind: "Suspend", target: { filter: { controller: "opponent", kind: ["Digimon"] }, count: 1 } }] });
