...
describe("Green Digi-Burst promo deck", () => {
  it("turns Palmon and Tyrannomon sources into Jamming plus Security Attack", async () => {
    ...
    preferred.push(s.inst("tyrannomon").instanceId, s.inst("palmon").instanceId, s.perm("granKuwagamon").permanentId);
    ...
    expect(
      s.state.players[0]!.battleArea.some((permanent) => permanent.permanentId === s.perm("granKuwagamon").permanentId),
    ).toBe(true);
    assertNoLoudGap(s);
  });
});
