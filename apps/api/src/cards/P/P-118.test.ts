  ...
  it("adds both matching reveal classes and bottoms the rest", async () => {
    const s = setupEngine({
      0: {
        ...
        deck: [{ card: "BT16-017", as: "multicolor" }, { card: "P-125", as: "ken" }, { card: "BT1-001", as: "filler" }],
      },
    }, { autoSelectCards: true, autoOrderCards: true });
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("wormmon").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("multicolor").instanceId) && s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("ken").instanceId));
