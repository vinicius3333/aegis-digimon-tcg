...
describe("P-053 Ophanimon", () => {
  it("gives one opponent Digimon -5000 DP with a Tamer", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "AD1-015", as: "base" }, { card: "BT1-089", as: "tamer" }], hand: [{ card: "P-053", as: "source" }], deck: ["BT1-009"] }, 1: { battleArea: [{ card: "BT1-009", as: "target", dp: 10000 }] } }, { autoSelectCards: true });
    s.state.memory = 10;
  ...
  it("does not give -5000 DP without a Tamer", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "AD1-015", as: "base" }], hand: [{ card: "P-053", as: "source" }], deck: ["BT1-009"] }, 1: { battleArea: [{ card: "BT1-009", as: "target", dp: 10000 }] } }, { autoSelectCards: true });
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("source").instanceId })).toEqual({ ok: true });
    await settle();
  ...
  it("gives one opponent Digimon and all opponent Security Digimon -2000 DP when attacking", async () => {
    ...
    await settle(
      () => s.perm("target").currentDP === 4000 && observe(s.engine).securityDp(1) === -2000,
    );
  ...
  it("applies the Security Digimon reduction to the actual security battle", async () => {
    ...
    expect(s.state.players[1]!.trash.some(
      (card) => card.instanceId === s.inst("securityDigimon").instanceId,
    )).toBe(true);
  });
});
