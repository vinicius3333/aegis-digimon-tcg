...
describe("P-106 (Agility Training)", () => {
  ...
  it("OnUseOption RevealAdd only adds GREEN cards to hand (card text + documented behavior HasCardColor(Green))", async () => {
    const source = makeSource();
    const effects = module!.effectsForTiming(EffectTiming.OnUseOption, source);
    expect(effects.length).toBeGreaterThanOrEqual(1);
  ...
  it("OnDeclaration <Delay> only digivolves into a GREEN Digimon in hand (Q4192 / documented behavior HasCardColor(Green))", async () => {
    const recorder: Recorder = { calls: [] };
