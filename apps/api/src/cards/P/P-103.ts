...
// P-103 Offense Training — hand-corrected IR override (no AUTO-GENERATED header, so the
// generator preserves this file). Sibling of P-104; the runtime record dropped the red-color
//   - RevealAdd `add[0].filter` was `{}`; corrected to `{ colors: ["Red"] }` ("add 1 red").
//   - Digivolve `into` was `{ kind: ["Digimon"] }`; corrected to add `colors: ["Red"]` so
