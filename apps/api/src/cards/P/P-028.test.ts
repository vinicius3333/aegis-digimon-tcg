...
describe("P-028 Pulsemon", () => {
  it("draws with three or more security cards", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "P-028", as: "pulsemon" }], deck: [{ card: "BT1-009", as: "drawn" }], security: ["BT1-028", "BT1-028", "BT1-028", "BT1-028"] } });
    const p0 = s.state.players[0]!;
    const drawn = s.inst("drawn").instanceId;
    s.state.memory = 3;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("pulsemon").instanceId })).toEqual({ ok: true });
  ...
  it("does both effects at exactly three security cards", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "P-028", as: "pulsemon" }], deck: [{ card: "BT1-009", as: "drawn" }], security: ["BT1-028", "BT1-028", "BT1-028"] } });
    const p0 = s.state.players[0]!;
    s.state.memory = 4;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("pulsemon").instanceId })).toEqual({ ok: true });
