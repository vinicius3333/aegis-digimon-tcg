  ...
  it("adds a yellow/black security card, recovers one, and keeps the stack size", async () => {
    const s = setupEngine({
      0: {
        ...
        security: [{ card: "BT11-036", as: "match" }, { card: "BT1-001", as: "other" }],
    ...
    await settle(() => s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("match").instanceId) && s.state.players[0]!.security.length === 2);
