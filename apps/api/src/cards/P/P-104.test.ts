...
describe("P-104 (Mental Training)", () => {
  ...
  it(
    // Card text: "Add 1 blue card among them to your hand."
    ...
    "OnUseOption RevealAdd only adds BLUE cards to hand (card text + documented behavior HasCardColor(Blue))",
    async () => {
      const source = makeSource();
      const effects = module!.effectsForTiming(EffectTiming.OnUseOption, source);
      expect(effects.length).toBeGreaterThanOrEqual(1);
  ...
  it(
    // Q4192 / documented behavior CanSelectCardCondition: IsDigimon && HasCardColor(Blue).
    ...
    "OnDeclaration <Delay> only digivolves into a BLUE Digimon in hand (Q4192 / documented behavior HasCardColor(Blue))",
    async () => {
      const recorder: Recorder = { calls: [] };
