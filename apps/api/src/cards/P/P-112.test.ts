...
describe("P-112 Morphomon", () => {
  it("reveals three and adds both Eosmon and Menoa Bellucci when both are present", async () => {
    ...
    await settle(() => s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("eosmon").instanceId) && s.state.players[0]!.hand.some((card) => card.instanceId === s.inst("menoa").instanceId));
