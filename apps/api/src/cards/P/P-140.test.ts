  ...
  it("encodes Evade, suspended immunity, Insectoid digivolution, and inherited security trash", () => {
    ...
    expect(compiled.effects).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({
        trigger: "AllTurns",
        actions: [{ kind: "GrantStatic", target: { filter: { isSelfRef: true }, count: 1, isSelf: true }, grant: "immuneToOpponentDigimonEffects", duration: "permanent", condition: { kind: "raw", raw: "this Digimon is suspended" } }],
      }),
      expect.objectContaining({
        trigger: "AllTurns",
        isInherited: true,
