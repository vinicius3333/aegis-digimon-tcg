...
describe("P-124 Davis Motomiya", () => {
  it("plays Veemon from hand through the first On Play mode", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "P-124", as: "davis" }, { card: "BT11-023", as: "veemon" }] } }, { autoChooseOption: true, preferOptionIndex: 0, autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 10;
