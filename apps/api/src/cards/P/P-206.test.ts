...
describe("P-206 Digimon Liberator", () => {
  it("can be used without a matching color source or a separate waiver prompt", async () => {
    ...
    expect(s.state.players[0]!.hand.map((card) => card.cardId)).toEqual(expect.arrayContaining(["BT1-009", "BT1-085"]));
  });

  it("reveals distinct Digimon and Tamer cards, then places itself", () => {
    expect(
      runtimeCompiledCard("P-206")!.effects.find((effect) => effect.trigger === "Main" && !effect.keywords?.length),
    ).toMatchObject({
      actions: [
        {
          kind: "RevealAdd",
          ...
          rest: "deckBottom",
          add: [
            { count: 1, to: "hand", filter: { kind: ["Digimon"] } },
            { count: 1, to: "hand", filter: { kind: ["Tamer"] } },
          ],
        },
      ...
      actions: [
        {
          ...
          from: ["hand", "trash"],
          payCost: false,
          target: { count: 1, filter: { kind: ["Digimon"], playCostLte: 3 } },
        },
        { kind: "AddToHandSelf" },
      ],
    });
