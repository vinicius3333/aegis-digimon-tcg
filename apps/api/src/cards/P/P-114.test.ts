...
describe("P-114 Diaboromon", () => {
  it("plays a Diaboromon Token when digivolving and counts the token for deletion scaling", async () => {
    ...
    expect(s.engine.applyIntent(0, { type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("diaboromon").instanceId })).toEqual({ ok: true });
    await settle(() => s.state.players[0]!.battleArea.some((p) => p.topCard?.cardId.startsWith("TOKEN-")) && !s.state.players[1]!.battleArea.some((p) => p.permanentId === lowId));
