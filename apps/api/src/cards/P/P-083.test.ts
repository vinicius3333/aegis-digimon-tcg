...
describe("P-083 Floramon", () => {
  it("prevents an opponent Digimon from unsuspending with a green Tamer", async () => {
    const s = setupEngine({
      0: { hand: [{ card: "P-083", as: "source" }], battleArea: [{ card: "BT1-088", as: "tamer" }], deck: ["BT1-009", "BT1-009"] },
  ...
  it("does not prevent unsuspending without a green Tamer", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "P-083", as: "source" }], battleArea: [{ card: "BT1-086", as: "blue-tamer" }] }, 1: { battleArea: [{ card: "BT1-009", as: "target", suspended: true }] } }, { autoSelectCards: true });
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("source").instanceId })).toEqual({ ok: true });
    await settle();
    expect(observe(s.engine).isRestricted(s.perm("target"), "unsuspend")).toBe(false);
