...
describe("P-108 (Wisdom Training)", () => {
  ...
  it(
    // Card text: "Add 1 purple card among them to your hand."
    ...
    "OnUseOption RevealAdd only adds PURPLE cards to hand (card text + documented behavior HasCardColor(Purple))",
    async () => {
      const source = makeSource();
      const effects = module!.effectsForTiming(EffectTiming.OnUseOption, source);
      expect(effects.length).toBeGreaterThanOrEqual(1);
  ...
  it(
    // Q4192 / documented behavior CanSelectCardCondition: IsDigimon && HasCardColor(Purple).
    ...
    "OnDeclaration <Delay> only digivolves into a PURPLE Digimon in hand (Q4192 / documented behavior HasCardColor(Purple))",
    async () => {
      const recorder: Recorder = { calls: [] };
