...
describe("P-205 Insane Synthetic Monster", () => {
  it("waives its color requirement only while you have a DM Digimon or Tamer", () => {
    expect(runtimeCompiledCard("P-205")!.effects.find((effect) => effect.trigger === "Static")).toMatchObject({
      condition: { kind: "youHave", filter: { controller: "mine", kind: ["Digimon", "Tamer"], nameOrTrait: [{ tokens: ["DM"], match: "trait" }] } },
  ...
  it("draws, trashes two, and places itself for Main and Security", () => {
    ...
    for (const effect of card.effects.filter((entry) => entry.trigger === "Main" && !entry.keywords?.length || entry.trigger === "Security")) {
  ...
  it("deletes your low-cost Digimon and plays a named card from your trash with cost reduced by 3", () => {
    expect(runtimeCompiledCard("P-205")!.effects.find((effect) => effect.keywords?.some((keyword) => keyword.keyword === "Delay"))).toMatchObject({
      keywords: [{ keyword: "Delay" }],
      actions: [
        { kind: "Delete", target: { count: 1, filter: { controller: "mine", kind: ["Digimon"], playCostLte: 7 } } },
        { kind: "PlayWithoutCost", from: ["trash"], payCost: true, reduceCostBy: 3, target: { count: 1, filter: { controller: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Kimeramon", "Millenniummon"], match: "name" }] } } },
      ],
    });
  });
});
