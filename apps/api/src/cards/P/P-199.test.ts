...
describe("P-199 Dan Yuki", () => {
  ...
  it("gives one of your Digimon +3000 DP when you have 4 or less memory", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "P-199", as: "dan" }, { card: "BT1-009", as: "digimon", dp: 3000 }] } });
    s.state.memory = 4;
    await s.ready();
  ...
  it("plays itself for free from Security", async () => {
    const s = setupEngine({ 0: { security: [{ card: "P-199", as: "securityDan" }] }, 1: { battleArea: [{ card: "BT1-009", as: "attacker" }] } });
    s.state.turnSeat = 1;
    const result = s.engine.applyIntent(1, { type: "attack", attackerPermanentId: s.perm("attacker").permanentId, target: { kind: "player" } });
