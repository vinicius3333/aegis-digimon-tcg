...
describe("P-079 Agumon", () => {
  it("deletes only a 3000-DP-or-less target with a red Tamer", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "P-079", as: "source" }], battleArea: [{ card: "BT1-085", as: "tamer" }] }, 1: { battleArea: [{ card: "BT1-009", as: "eligible", dp: 3000 }, { card: "BT1-009", as: "too-large", dp: 4000 }] } }, { autoSelectCards: true });
    s.state.memory = 10;
  ...
  it("does not delete without a red Tamer", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "P-079", as: "source" }], battleArea: [{ card: "BT1-086", as: "blue-tamer" }] }, 1: { battleArea: [{ card: "BT1-009", as: "target", dp: 3000 }] } }, { autoSelectCards: true });
    s.state.memory = 10;
