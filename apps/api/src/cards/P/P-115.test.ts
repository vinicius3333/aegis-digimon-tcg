  ...
  it("plays an errata-eligible Amano Tamer and Saves itself under that Tamer", async () => {
    const s = setupEngine({
      0: {
        ...
        hand: [{ card: "BT10-092", as: "nene" }, { card: "P-224", as: "unrelated" }],
    ...
    await settle(() => s.state.players[0]!.battleArea.some((p) => p.topCard?.instanceId === s.inst("nene").instanceId) && s.state.players[0]!.battleArea.find((p) => p.topCard?.instanceId === s.inst("nene").instanceId)!.stack.some((card) => card.instanceId === skullId));
