    ...
    expect(compiled.effects).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        actions: [expect.objectContaining({
          ...
          optional: true,
          abortOnDecline: true,
          target: expect.objectContaining({ filter: { controller: "opponent", kind: ["Digimon"], levels: [3] }, count: 1 }),
          condition: expect.objectContaining({ kind: "selfColorCount", value: 2 }),
          cost: expect.objectContaining({ kind: "trash", target: { filter: { zone: "hand", controller: "mine" }, count: 1 } }),
        })],
      }),
    ]));
  });
