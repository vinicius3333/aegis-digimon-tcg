...
describe("P-229 Unique Emblem: Narrative Ronde", () => {
  ...
  it("models the printed reactive Delay bullet", () => {
    const effect = runtimeCompiledCard("P-229")!.effects.find((entry) => entry.trigger === "YourTurn");
...
describe("P-229 engine behavior", () => {
    ...
    const s = setupEngine(
      {
        0: {
          ...
          deck: [{ card: "BT1-038", as: "puppet" }, { card: "BT18-060", as: "liberator" }, "BT1-009"],
  ...
  it("runs its Puppet/LIBERATOR reveal when checked from Security", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "P-229", as: "emblem" }], deck: ["BT1-038", "BT18-060", "BT1-009"] } },
      { autoAcceptOptional: true, autoSelectCards: true, autoChooseOption: true },
    );
    await s.ready();
