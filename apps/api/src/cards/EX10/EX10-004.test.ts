...
describe("EX10-004 Cupimon compiled contract", () => {
  it("models the inherited Lucemon breeding move effect and shared hand-trash cost", () => {
    ...
    expect(move).toMatchObject({ kind: "SubTrigger", event: "whenMovedFromBreeding", sourceFilter: { controller: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Lucemon"], match: "name" }] } });
    expect(move.actions).toEqual([
      expect.objectContaining({ kind: "Draw", amount: 1, cost: expect.objectContaining({ kind: "trash" }) }),
      expect.objectContaining({ kind: "GainMemory", amount: 1, condition: { kind: "ifThisEffectActed", raw: "if you did" } }),
    ]);
