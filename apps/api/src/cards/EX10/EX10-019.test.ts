...
describe("EX10-019 Warudamon", () => {
  it("proves linking sources, same-target unsuspend restriction, and the link effect", () => {
    ...
    expect(
      compiled.effects?.find((effect) => effect.trigger === "AllTurns" && effect.frequency === "OncePerTurn"),
    ).toMatchObject({
      actions: [
        {
          kind: "SubTrigger",
      ...
      actions: [
        {
          ...
          actions: [
            { kind: "trashSecurityTop", cost: { kind: "trash", target: { filter: { zone: "linked" }, count: 1 } } },
          ],
        },
      ],
    });
