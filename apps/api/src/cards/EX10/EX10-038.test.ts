...
describe("EX10-038 Copipemon", () => {
  it("records the exact catalog, Appmon evolution, and Link requirement", () => {
    expect(getCardDefinition(CARD_ID)).toMatchObject({
      ...
      forms: ["Stnd.", "Appmon"], attributes: ["System"], types: ["Copy & Paste", "Leviathan"], linkDp: 2000,
    });
  ...
  it("adds distinct Appmon and Leviathan cards from the top 3 and bottoms the rest", async () => {
    ...
    const s = setupEngine({ 0: {
      ...
      deck: [
        { card: "BT21-009", as: "appmon" }, { card: "EX10-062", as: "leviathan" },
        { card: "BT1-009", as: "rest" }, { card: "BT1-010", as: "below" },
      ],
    ...
    expect(s.state.players[0]!.hand.map(({ instanceId }) => instanceId)).toEqual(expect.arrayContaining([
      s.inst("appmon").instanceId, s.inst("leviathan").instanceId,
    ]));
  ...
  it("links only to Appmon for 1 and contributes +2000 DP", async () => {
    const s = setupEngine({ 0: {
      battleArea: [{ card: "BT21-009", as: "appmon" }, { card: "BT1-009", as: "plain" }],
    ...
    expect(s.engine.applyIntent(0, {
      type: "linkCard", instanceId: s.inst("copipemon").instanceId, targetPermanentId: s.perm("plain").permanentId,
    })).toEqual(expect.objectContaining({ ok: false }));
    expect(s.engine.applyIntent(0, {
      type: "linkCard", instanceId: s.inst("copipemon").instanceId, targetPermanentId: s.perm("appmon").permanentId,
    })).toEqual({ ok: true });
  ...
  it("Q5117/Q5118 returns an Appmon by trashing itself or another same-host link", async () => {
    for (const costAlias of ["copipemon", "sameHost"] as const) {
      ...
      const s = setupEngine({ 0: {
        battleArea: [
          { card: "BT21-009", as: "host", linked: [
            { card: CARD_ID, as: "copipemon" }, { card: "BT26-010", as: "sameHost" },
          ] },
