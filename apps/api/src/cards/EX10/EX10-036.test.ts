...
describe("EX10-036 Magneticdramon", () => {
  it("records the exact catalog", () => {
    expect(getCardDefinition(CARD_ID)).toMatchObject({
      colors: ["Black"], level: 7, playCost: 14, dp: 14000,
      evoCosts: [{ color: "Black", level: 6, memoryCost: 4 }],
      forms: ["Mega"], attributes: ["Virus"], types: ["Rock Dragon", "LIBERATOR", "Mineral"],
    });
  });
  it("proves Fragment, deletion plus security trash, shared unsuspend, and alternate digivolution", () => {
    ...
    expect(compiled.digivolutionRequirement).toEqual([{
      namesExact: ["Proganomon"], cost: 6, isAlternate: true,
  ...
  it("Q5113/Q5114 pays exactly 3 sources across stacks, deletes 1, and trashes top security", async () => {
    ...
    const s = setupEngine({
      0: { battleArea: [
        ...
        { card: "EX10-028", as: "secondHost", under: [
          { card: "EX10-003", as: "second" }, { card: "EX10-028", as: "third" },
        ] },
      ] },
      1: { battleArea: [{ card: "BT1-019", as: "target" }], security: ["BT1-009", "BT1-010"] },
    }, { autoAcceptOptional: true, autoSelectCards: true, preferInstanceIds: preferred });
    preferred.push(s.inst("first").instanceId, s.inst("second").instanceId, s.inst("third").instanceId, s.perm("target").permanentId);
    await s.ready();
    ...
    expect(s.state.players[0]!.trash.map(({ instanceId }) => instanceId)).toEqual(expect.arrayContaining([
      s.inst("first").instanceId, s.inst("second").instanceId, s.inst("third").instanceId,
    ]));
  ...
  it("Q5115 places exactly 3 matching trash cards at bottom before unsuspending", async () => {
    const s = setupEngine({ 0: {
      ...
      trash: [{ card: "EX10-025", as: "first" }, { card: "EX10-003", as: "second" }, { card: "EX10-028", as: "third" }],
    } }, { autoAcceptOptional: true, autoSelectCards: true });
    await s.ready();
    await advance(s.engine).fireForPermanent(EffectTiming.WhenDigivolving, s.perm("magnetic"));
    expect(s.perm("magnetic").stack.slice(0, 3).map(({ instanceId }) => instanceId)).toEqual(expect.arrayContaining([
      s.inst("first").instanceId, s.inst("second").instanceId, s.inst("third").instanceId,
    ]));
  ...
  it("uses the Close-gated Proganomon evolution for 6 and rejects it without Close", async () => {
    const valid = setupEngine({ 0: {
      battleArea: [{ card: "EX10-032", as: "base" }, { card: "EX10-063", as: "close" }],
      hand: [{ card: CARD_ID, as: "magnetic" }],
    } });
    valid.state.memory = 6;
    await valid.ready();
    expect(valid.engine.applyIntent(0, {
      type: "digivolve", permanentId: valid.perm("base").permanentId,
      instanceId: valid.inst("magnetic").instanceId, alternateRequirementIndex: 0,
    })).toEqual({ ok: true });
    await settle(() => valid.perm("base").topCard.cardId === CARD_ID);
    expect(valid.state.memory).toBe(0);

    const blocked = setupEngine({ 0: {
      battleArea: [{ card: "EX10-032", as: "base" }], hand: [{ card: CARD_ID, as: "magnetic" }],
    } });
    blocked.state.memory = 6;
    await blocked.ready();
    expect(blocked.engine.applyIntent(0, {
      type: "digivolve", permanentId: blocked.perm("base").permanentId,
      instanceId: blocked.inst("magnetic").instanceId, alternateRequirementIndex: 0,
