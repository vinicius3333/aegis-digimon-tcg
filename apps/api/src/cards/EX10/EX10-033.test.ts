...
describe("EX10-033 Pyramidimon", () => {
  it("records the exact catalog", () => {
    expect(getCardDefinition("EX10-033")).toMatchObject({
      colors: ["Black"], level: 6, playCost: 11, dp: 11000,
      evoCosts: [{ color: "Black", level: 5, memoryCost: 3 }],
      forms: ["Mega"], attributes: ["Virus"], types: ["Mineral", "LIBERATOR"],
    });
  });
  it("proves Fragment, shared once-per-turn placement, and scaled play-cost reduction", () => {
    ...
    for (const effect of reductions ?? []) {
      expect(effect.actions?.[0]).toMatchObject({
        ...
        cost: { kind: "trash", target: { filter: { controller: "mine" }, count: 3, upTo: true, minimum: 1, from: ["digivolutionCards"] } },
  ...
  it("Q5095/Q5096 places 1-3 Mineral/Rock cards, including a Digi-Egg, at its bottom", async () => {
    ...
    const s = setupEngine({ 0: {
      ...
      trash: [
        { card: "EX10-025", as: "mineral" }, { card: "EX10-003", as: "egg" },
        { card: "EX10-028", as: "rock" }, { card: "BT1-009", as: "near" },
      ],
    ...
    expect(s.perm("pyramid").stack.slice(0, 3).map(({ instanceId }) => instanceId)).toEqual(
  ...
  it("Q5097-Q5100 trashes 3 sources across stacks and reduces only the chosen permanent to 0", async () => {
    ...
    const s = setupEngine({
      0: { battleArea: [
        ...
        { card: "EX10-028", as: "secondHost", under: [
          { card: "EX10-028", as: "second" }, { card: "EX10-003", as: "third" },
        ] },
      ] },
      1: { battleArea: [{ card: "AD1-003", as: "chosen" }, { card: "AD1-003", as: "other" }] },
    }, { autoAcceptOptional: true, autoSelectCards: true, preferInstanceIds: preferred });
    preferred.push(
    ...
    expect(ledger.playCostFor({ def: definition, controllerSeat: 1, permanentId: s.perm("chosen").permanentId }, 5)).toBe(0);
    expect(ledger.playCostFor({ def: definition, controllerSeat: 1, permanentId: s.perm("other").permanentId }, 5)).toBe(5);
  });
});
