...
describe("EX10-029 Warpmon", () => {
  it("proves the link cost, selected target restriction, security play, and Blocker", () => {
    ...
    expect(compiled.effects?.find((effect) => effect.trigger === "Security")).toMatchObject({
      isSecurity: true,
      actions: [
        { kind: "PlayWithoutCost", payCost: false, target: { filter: { isSelfRef: true }, count: 1, isSelf: true } },
      ],
    });
    ...
    expect(
      compiled.effects?.find((effect) => effect.trigger === "Static" && effect.actions[0]?.kind === "SubTrigger"),
    ).toMatchObject({
      actions: [
        {
          kind: "SubTrigger",
