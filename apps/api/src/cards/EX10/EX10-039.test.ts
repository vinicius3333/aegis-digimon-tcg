...
describe("EX10-039 ChuuChuumon", () => {
  it("records the exact catalog", () => {
    expect(getCardDefinition(CARD_ID)).toMatchObject({
      colors: ["Purple"], level: 3, playCost: 4, dp: 1000,
      evoCosts: [{ color: "Purple", level: 2, memoryCost: 0 }],
      forms: ["Rookie"], attributes: ["Virus"], types: ["Beast", "Bagra Army"],
    });
  });
  it("proves bottom placement to Bagra Army Digimon or Tamers, Save, and inherited Draw 1", () => {
    ...
    expect(compiled.effects?.find((effect) => effect.isInherited)).toMatchObject({
      actions: [{
        kind: "SubTrigger", event: "onDigivolutionCardsDiscardedBatch", sourceFilter: { isSelfRef: true },
  ...
  it("Q5119 places a Bagra Army Digimon from trash at a Bagra Army Tamer's true bottom", async () => {
    ...
    const s = setupEngine({ 0: {
      ...
      trash: [{ card: "EX10-026", as: "material" }, { card: "BT1-009", as: "near" }],
    } }, { autoAcceptOptional: true, autoSelectCards: true, preferInstanceIds: preferred });
    preferred.push(s.inst("near").instanceId, s.inst("material").instanceId, s.perm("plainTamer").permanentId, s.perm("tamer").permanentId);
    await s.ready();
  ...
  it("Save moves the deleted card under a Tamer", async () => {
    const s = setupEngine({ 0: { battleArea: [
      { card: CARD_ID, as: "chuu" }, { card: "BT12-094", as: "tamer" },
