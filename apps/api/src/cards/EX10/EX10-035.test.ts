...
describe("EX10-035 catalog and offensive timings", () => {
  it("records the exact no-standard-evolution catalog", () => {
    expect(getCardDefinition("EX10-035")).toMatchObject({
      colors: ["Black"], level: 6, playCost: 11, dp: 11000, evoCosts: [],
      forms: ["Mega"], attributes: ["Virus"], types: ["Machine", "Dark Masters"],
    });
  });
...
describe("EX10-035 — delayed-delete-played gates to the reduced-cost [Hand][Main] play (KB Q5737)", () => {
  ...
  it("Q5109 allows the hand Main effect with no Digimon", async () => {
    ...
    expect(s.engine.applyIntent(0, {
      type: "activateEffect", sourceInstanceId: instance.instanceId, effectKey: reducedCostPlayEffectKey(s, instance),
    })).toEqual({ ok: true });
