...
const ex12Ids = Array.from({ length: 74 }, (_, index) => `EX10-${String(index + 1).padStart(3, "0")}`);
...
describe("EX10 persisted IR", () => {
  it("contains exactly the authoritative EX10 card keys", () => {
    const persistedCardIds = Object.keys(catalog)
      .filter((cardId) => /^EX10-\d{3}$/.test(cardId))
      .sort();
