...
describe("EX10-030 Cometmon compiled contract", () => {
  it("records the exact catalog and compiled contract", () => {
    expect(getCardDefinition(CARD_ID)).toMatchObject({
      colors: ["Black", "Yellow"], level: 5, playCost: 8, dp: 8000,
      evoCosts: [
        { color: "Black", level: 4, memoryCost: 4 },
        { color: "Yellow", level: 4, memoryCost: 4 },
      ],
      forms: ["Ult.", "Appmon"], attributes: ["Navi"], types: ["Astronomy", "Leviathan"], linkDp: 4000,
    });
    ...
    for (const trigger of ["OnPlay", "WhenDigivolving"]) {
      expect(compiled.effects?.find((effect) => effect.trigger === trigger)).toMatchObject({
        actions: [{
          kind: "Link", from: ["hand", "digivolutionCards"], optional: true,
          target: { filter: {
            controller: "mine", kind: ["Digimon"], levelComparison: { op: "lte", value: 4 },
            hasLinkRequirement: true, hostFilter: { isSelfRef: true },
          }, count: 1 },
      ...
      const s = setupEngine({ 0: {
        battleArea: [
          { card: "BT21-009", as: "host", linked: [
            { card: CARD_ID, as: "comet" }, { card: "BT26-010", as: "sameHost" },
          ] },
  ...
  it("its inherited once-per-turn replacement trashes the host's link and prevents leaving", async () => {
    const s = setupEngine({ 0: { battleArea: [{
      card: "BT21-009", as: "host", under: [{ card: CARD_ID, as: "comet" }],
