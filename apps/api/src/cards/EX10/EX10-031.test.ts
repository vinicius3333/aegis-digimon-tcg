...
describe("EX10-031 DarkKnightmon", () => {
  it("records the exact catalog and alternate Knightmon-text evolution route", () => {
    expect(getCardDefinition(CARD_ID)).toMatchObject({
      colors: ["Black", "Purple"], level: 5, playCost: 7, dp: 7000,
      evoCosts: [
        { color: "Black", level: 4, memoryCost: 4 },
        { color: "Purple", level: 4, memoryCost: 4 },
      ],
      forms: ["Ultimate"], attributes: ["Virus"], types: ["Dark Knight", "Bagra Army", "Twilight"],
    });
    expect(compiled.digivolutionRequirement).toEqual([
      { level: 4, texts: ["Knightmon"], cost: 4, isAlternate: true },
    ]);
  });
  ...
  it("binds protection and +3000 DP to the same selected Digimon through the opponent turn", async () => {
    ...
    const s = setupEngine({ 0: { battleArea: [
      { card: CARD_ID, as: "source" }, { card: "BT21-009", as: "chosen" }, { card: "BT1-009", as: "other" },
  ...
  it("plays only a cost-4-or-lower source for free when leaving, then leaves", async () => {
    const preferred: string[] = [];
    const s = setupEngine({ 0: { battleArea: [{
      card: CARD_ID, as: "darkKnight", under: [
        { card: "EX10-026", as: "eligible" }, { card: "BT1-019", as: "tooExpensive" },
      ],
    ...
    await settle(() => s.state.players[0]!.battleArea.some(({ topCard }) => topCard.instanceId === s.inst("eligible").instanceId));
  ...
  it("redirects an opposing player attack to the realistic inherited host once", async () => {
    ...
    expect(s.engine.applyIntent(1, {
      type: "attack", attackerPermanentId: attackerId, target: { kind: "player" },
    })).toEqual({ ok: true });
  ...
  it("DigiXroses with SkullKnightmon and DeadlyAxemon for 2 less and rejects a wrong slot", async () => {
    const s = setupEngine({ 0: { hand: [
      { card: CARD_ID, as: "darkKnight" }, { card: "EX10-026", as: "skull" },
      { card: "EX10-027", as: "deadly" }, { card: "BT1-009", as: "wrong" },
    ] } }, { autoSelectCards: true });
    s.state.memory = 7;
    await s.ready();
    expect(s.engine.applyIntent(0, {
      type: "playCard", instanceId: s.inst("darkKnight").instanceId,
    ...
    expect(played.stack.map(({ instanceId }) => instanceId)).toEqual(expect.arrayContaining([
      s.inst("skull").instanceId, s.inst("deadly").instanceId,
    ]));
  });
});
