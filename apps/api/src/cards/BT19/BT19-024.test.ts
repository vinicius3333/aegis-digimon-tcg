...
describe("BT19-024 MarineBullmon", () => {
  it.each([EffectTiming.OnPlay, EffectTiming.WhenDigivolving])(
    ...
    async (timing) => {
      const s = setupEngine({ 0: {
        battleArea: [{ card: "BT19-024", as: "marine" }, { card: "BT19-023", as: "other" }],
        hand: [{ card: "BT19-018", as: "aquatic" }, { card: "BT1-009", as: "nonmatching" }],
  ...
  it("Decode plays a blue level 4 source on non-battle leave and completes Q3058's bounce", async () => {
    const s = setupEngine({
      ...
      1: { battleArea: [
        { card: "BT19-028", as: "attacker" }, { card: "BT19-023", as: "level5" },
    ...
    await advance(s.engine).fireSubTrigger("whenOpponentAttacks", { attackerPermanentId: s.perm("attacker").permanentId });
  ...
  it("Decode does not activate for battle deletion", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT19-024", as: "marine", under: ["BT19-019"] }] } },
      { autoAcceptOptional: true, autoSelectCards: true });
  ...
  it("is always Aquatic and its inherited End of Attack plays one eligible source once per turn", async () => {
    const s = setupEngine({ 0: { battleArea: [
      { card: "BT19-028", as: "host", under: ["BT19-019", "BT19-024"] },
    ] } }, { autoAcceptOptional: true, autoSelectCards: true });
    await s.ready();
