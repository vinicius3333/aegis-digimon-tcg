...
describe("BT19-052 Vespamon", () => {
  it("publicly uses the Royal Base level-4 cost-3 route", async () => {
    ...
    const s = setupEngine({ 0: {
      battleArea: [{ card: "BT18-046", as: "base" }], hand: [{ card: "BT19-052", as: "vespa" }], deck: ["BT19-030"],
    } }, { autoSelectCards: true });
    s.state.memory = 5;
    expect(s.engine.applyIntent(0, {
      type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("vespa").instanceId,
  ...
  it("face-up security grants Blocker to all and only Royal Base Digimon on the opponent's turn", async () => {
    const s = setupEngine({ 0: {
      ...
      battleArea: [{ card: "BT19-045", as: "royal" }, { card: "BT19-046", as: "plain" }],
    } });
    await s.ready();
  ...
  it.each([[0, "BT1-009", "BT1-015"], [1, "BT1-015", "BT1-019"], [2, "BT1-019", "BT1-022"]] as const)(
    "with %s face-up security deletes at ceiling but not the next cost", async (count, target, over) => {
      const s = setupEngine({
        0: { battleArea: [{ card: "BT19-052", as: "vespa" }], security: Array.from({ length: count }, () => ({ card: "BT19-045", faceUp: true })) },
        1: { battleArea: [{ card: target, as: "target" }, { card: over, as: "over" }] },
      }, { autoSelectCards: true });
  ...
  it("has the Insectoid rule trait and inherited battle deletion trashes security once per turn", async () => {
    const s = setupEngine({
      0: { battleArea: [{ card: "BT19-052", as: "vespa" }, { card: "BT19-053", as: "host", under: ["BT19-052"] }] },
