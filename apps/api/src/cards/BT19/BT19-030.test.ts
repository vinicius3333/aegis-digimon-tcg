...
describe("BT19-030 Renamon", () => {
  ...
  it("naturally gains memory at the start of a real main phase with Rika", async () => {
    const s = setupEngine({
      0: {
        battleArea: [
          { card: "BT19-030", as: "rena" },
  ...
  it("gains no start-main memory without Rika or Calumon or on the opponent's turn", async () => {
    ...
    const opponentTurn = setupEngine({ 0: { battleArea: [{ card: "BT19-030", as: "rena" }, { card: "BT19-083" }] } });
  ...
  it("uses the effective Option use cost threshold and reduces one opponent by 2000 DP once", async () => {
    ...
    await advance(s.engine).fireSubTrigger("whenOptionUsed", {
      usedOptionCost: 2,
      subjectPermanentId: "pay-cost-reduced",
    });
