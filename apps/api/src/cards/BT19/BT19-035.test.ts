...
describe("BT19-035 ShootingStarmon", () => {
  ...
  it("triggers on its own play and gives the same opponent -1 security attack and -3000 DP (Q3090)", async () => {
    const s = setupEngine({
      0: { hand: [{ card: "BT19-035", as: "shooting" }, { card: "BT19-033", as: "second" }] },
      1: { battleArea: [{ card: "BT19-020", as: "first" }, { card: "BT19-021", as: "other" }] },
    }, { autoOrderTriggers: true, autoSelectCards: true });
    s.state.memory = 10;
    await s.ready();
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("shooting").instanceId })).toEqual({ ok: true });
  ...
  it.each(["hand", "trash"] as const)("On Deletion may place an eligible %s Digimon under a Tamer", async (zone) => {
    ...
    const s = setupEngine({ 0: {
      battleArea: [{ card: "BT19-035", as: "shooting" }, { card: "BT19-083", as: "tamer" }],
