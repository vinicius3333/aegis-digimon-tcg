...
describe("BT19-034 Kyubimon", () => {
  ...
  it("does not play Rika with 2 Tamers and may decline when eligible", async () => {
    ...
    const declined = setupEngine({ 0: {
      battleArea: [{ card: "BT19-034", as: "kyubi" }], hand: [{ card: "BT19-083", as: "rika" }],
  ...
  it("uses effective Option use cost 2+ and reduces exactly one opponent by 2000 only once", async () => {
    ...
    await advance(s.engine).fireSubTrigger("whenOptionUsed", { usedOptionCost: 2, subjectPermanentId: "payment-reduced" });
