...
describe("BT19-054 MegaGargomon", () => {
  ...
  it.each([EffectTiming.WhenDigivolving, EffectTiming.OnUseAttack])(
    "%s may bottom-deck only a suspended opponent Digimon",
    async (timing) => {
      const s = setupEngine(
        {
  ...
  it("resolves When Attacking bottom-decking from a public attack intent", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "BT19-054", as: "mega" }] },
        1: { battleArea: [{ card: "BT1-009", as: "target", suspended: true }] },
