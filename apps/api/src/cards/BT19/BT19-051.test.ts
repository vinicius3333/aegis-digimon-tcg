...
describe("BT19-051 AtlurBallistamon", () => {
  it("has the Xros Heart level-4 cost-3 route and limits Ballistamon naming to DigiXros (Q3105)", async () => {
    ...
    expect(
      runtimeCompiledCard("BT19-051")?.effects.find((effect) => effect.trigger === "OnDeletion")?.actions[0],
    ).toMatchObject({
      kind: "PlaceUnder",
  ...
  it("publicly evolves from an Xros Heart level 4 for 3", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [{ card: "BT19-033", as: "base" }],
  ...
  it("resolves On Play DP and return protection from a public play intent", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "BT19-051", as: "atlur" }] } }, { autoSelectCards: true });
    s.state.memory = 10;
    await s.ready();
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("atlur").instanceId })).toEqual({ ok: true });
    await settle(() => s.perm("atlur").currentDP === 10000);
  ...
  it.each([EffectTiming.OnPlay, EffectTiming.WhenDigivolving])(
    "%s gives the same target +3000 DP and return protection",
    async (timing) => {
  ...
  it("does not place an eligible Xros Heart card from the deck", async () => {
    ...
    const s = setupEngine(
      {
        0: {
          ...
          deck: [{ card: "BT19-047", as: "deckCandidate" }],
        },
      },
      {
    ...
    preferred.push(s.inst("deckCandidate").instanceId);
    // If the action accidentally falls back to the deck, this preference makes the
    // illegal candidate observable instead of silently selecting the deleted card.
    await s.ready();
    await advance(s.engine).verb.deletePermanent([s.perm("atlur").permanentId], "byEffect");
    await settle(() => s.perm("tamer").stack.length === 1);
    expect(s.perm("tamer").stack.map((card) => card.cardId)).toEqual(["BT19-051"]);
    expect(s.state.players[0]!.deck.map((card) => card.cardId)).toEqual(["BT19-047"]);
  });
