...
describe("BT19-014 Shoutmon EX6", () => {
  ...
  it("DigiXroses with all five exact named materials instead of the collapsed recipe", async () => {
    ...
    const s = setupEngine({
      0: {
        hand: [
          { card: "BT19-014", as: "ex6" }, { card: "BT19-012", as: "omni" },
          { card: "BT19-026", as: "zeig" }, { card: "BT19-051", as: "atlur" },
          { card: "BT19-038", as: "jaeger" }, { card: "BT19-061", as: "raptor" },
        ],
      },
    });
    s.state.memory = 0;
    ...
    expect(s.engine.applyIntent(0, {
      type: "playCard", instanceId: s.inst("ex6").instanceId,
      digiXros: { materialInstanceIds: ["omni", "zeig", "atlur", "jaeger", "raptor"].map((a) => s.inst(a).instanceId) },
    })).toEqual({ ok: true });
