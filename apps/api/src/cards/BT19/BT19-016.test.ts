...
describe("BT19-016 Gaossmon", () => {
  it.each([EffectTiming.OnPlay, EffectTiming.OnDestroyedAnyone])(
    ...
    async (timing) => {
      const s = setupEngine({
        0: {
          battleArea: [{ card: "BT19-016", as: "gaoss" }, { card: "BT19-081", as: "tamer" }],
          hand: [{ card: "BT19-020", as: "blueFlare" }, { card: "BT19-009", as: "nonmatching" }],
      ...
      expect(s.state.players[0]!.hand.map((card) => card.cardId)).toEqual(expect.arrayContaining(["BT19-009", "BT1-009"]));
