...
describe("BT19-050 Rapidmon", () => {
  it("retains ACE 3/Blast Digivolve hand metadata", () => {
    ...
    expect(runtimeCompiledCard("BT19-050")?.effects[0]).toMatchObject({ trigger: "Counter", isFromHand: true, keywords: [{ keyword: "BlastDigivolve" }] });
  });

  it.each([EffectTiming.OnPlay, EffectTiming.WhenDigivolving])("%s independently suspends a Tamer and locks a Digimon", async (timing) => {
    const s = setupEngine({
      0: { battleArea: [{ card: "BT19-050", as: "rapid" }] },
      1: { battleArea: [{ card: "BT19-081", as: "tamer" }, { card: "BT19-044", as: "digimon" }] },
  ...
  it("public green level-4 evolution pays 3 and retains its source", async () => {
    const s = setupEngine({ 0: {
      battleArea: [{ card: "BT19-049", as: "base" }], hand: [{ card: "BT19-050", as: "rapid" }], deck: ["BT19-030"],
    } }, { autoSelectCards: true });
    s.state.memory = 5;
    expect(s.engine.applyIntent(0, {
      type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("rapid").instanceId,
    })).toEqual({ ok: true });
