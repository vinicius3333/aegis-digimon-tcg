...
describe("BT19-021 Xiquemon", () => {
  it("naturally resolves On Play after being played from hand", async () => {
    const s = setupEngine(
      {
        0: { hand: [{ card: "BT19-021", as: "xique" }] },
        1: { battleArea: [{ card: "BT19-018", as: "level3" }] },
      },
  ...
  it("naturally resolves When Digivolving from a level-3 Digimon", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "BT19-018", as: "base" }], hand: [{ card: "BT19-021", as: "xique" }] },
        1: { battleArea: [{ card: "BT19-018", as: "level3" }] },
      },
      { autoSelectCards: true },
    );
