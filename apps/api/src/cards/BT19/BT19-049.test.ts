...
describe("BT19-049 Gargomon", () => {
  it.each([0, 1])("public evolution with %s existing Tamer plays exact Henry Wong free", async (tamerCount) => {
    const s = setupEngine({ 0: {
      ...
      hand: [{ card: "BT19-049", as: "gargo" }, { card: "BT19-085", as: "henry" }],
      deck: ["BT19-030"],
    } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 5;
    await s.ready();
    expect(s.engine.applyIntent(0, {
      type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("gargo").instanceId,
    })).toEqual({ ok: true });
  ...
  it("does not play Henry Wong when the controller already has 2 Tamers", async () => {
    const s = setupEngine({ 0: {
      battleArea: [
        { card: "BT19-044", as: "base" }, { card: "BT19-081" }, { card: "BT19-083" },
      ],
      hand: [{ card: "BT19-049", as: "gargo" }, { card: "BT19-085", as: "henry" }], deck: ["BT19-030"],
    } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 5;
    expect(s.engine.applyIntent(0, {
      type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("gargo").instanceId,
  ...
  it("cannot play Henry Wong & Shu-Chong Wong as Henry Wong (Q3104)", async () => {
    const s = setupEngine({ 0: {
      ...
      hand: [{ card: "BT19-049", as: "gargo" }, { card: "EX4-063", as: "pair" }], deck: ["BT19-030"],
    } }, { autoAcceptOptional: true, autoSelectCards: true });
    s.state.memory = 5;
    expect(s.engine.applyIntent(0, {
      type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("gargo").instanceId,
  ...
  it("may decline the free Henry Wong play", async () => {
    const s = setupEngine({ 0: {
      battleArea: [{ card: "BT19-049", as: "gargo" }], hand: [{ card: "BT19-085", as: "henry" }],
  ...
  it("inherited attack suspends one eligible opponent only once per turn", async () => {
    ...
    const s = setupEngine({
      ...
      1: { battleArea: [
        ...
        { card: "BT1-011", as: "first" }, { card: "BT1-012", as: "second" },
      ] },
