...
describe("BT19-025 MetalGreymon", () => {
  ...
  it("DigiXroses with exact Blue Greymon and MailBirdramon materials for cost 3", async () => {
    expect(digiXrosRequirementFor("BT19-025")).toEqual([{ materials: [
      { names: ["Greymon"], colors: ["Blue"] }, { names: ["MailBirdramon"] },
    ], count: 2 }]);
    const s = setupEngine({ 0: { hand: [
      { card: "BT19-025", as: "metal" }, { card: "BT19-020", as: "greymon" }, { card: "BT19-022", as: "mail" },
    ] } });
    s.state.memory = 0;
    await s.ready();
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("metal").instanceId,
      digiXros: { materialInstanceIds: [s.inst("greymon").instanceId, s.inst("mail").instanceId] } })).toEqual({ ok: true });
  ...
  it("when attacking de-digivolves an opponent, then freely evolves from under a Tamer", async () => {
    const s = setupEngine({
      0: { battleArea: [
        { card: "BT19-025", as: "metal" }, { card: "BT19-081", as: "tamer", under: ["BT19-026"] },
      ] },
  ...
  it("inherited End of Attack plays one level-4-or-lower Blue Flare from under a Tamer once per turn", async () => {
    ...
    await advance(s.engine).fireForPermanent(EffectTiming.OnEndAttack, s.perm("host"));
    expect(s.state.players[0]!.battleArea.filter((p) => ["BT19-020", "BT19-016"].includes(p.topCard?.cardId ?? ""))).toHaveLength(1);
