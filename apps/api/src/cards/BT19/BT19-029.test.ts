...
describe("BT19-029 Tapirmon", () => {
  it("naturally resolves On Play from hand", async () => {
    const s = setupEngine(
      {
        0: { hand: [{ card: "BT19-029", as: "tapir" }], security: ["BT19-030", "BT19-031"] },
      },
  ...
  it("On Play may trash the top security card to gain exactly 1 memory", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [{ card: "BT19-029", as: "tapir" }],
  ...
  it("trashes the top security card for inherited prevention", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [{ card: "BT19-030", as: "host", under: ["BT19-029"] }],
    ...
    expect(s.state.players[0]!.trash.map((card) => card.cardId)).toContain("BT19-031");
    expect(s.state.players[0]!.security.map((card) => card.cardId)).toEqual(["BT19-032"]);
  });
  ...
  it("does not protect a non-yellow Data host", async () => {
    const s = setupEngine(
      {
        0: {
          battleArea: [{ card: "BT19-027", as: "host", under: ["BT19-029"] }],
          security: ["BT19-031"],
        },
      },
      { autoAcceptOptional: true, autoSelectCards: true },
