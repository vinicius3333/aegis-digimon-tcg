...
describe("BT19-026 ZeigGreymon", () => {
  ...
  it("naturally resolves When Digivolving from a level-5 Digimon", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "BT19-025", as: "base" }], hand: [{ card: "BT19-026", as: "zeig" }] },
        1: {
          battleArea: [
            { card: "BT19-023", as: "stack", under: ["BT19-021", "BT19-019"] },
            { card: "BT19-020", as: "level4" },
  ...
  it("counts a temporarily exposed Digi-Egg as the second Digimon before rule cleanup (Q3079)", async () => {
    ...
    expect(s.state.players[1]!.trash.map((card) => card.cardId)).toEqual(
      expect.arrayContaining(["BT19-023", "BT19-001"]),
    );
