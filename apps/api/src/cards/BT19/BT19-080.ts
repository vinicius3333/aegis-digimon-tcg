...
const compiled = {
  effects: [
    ...
    { trigger: "AllTurns", actions: [{ kind: "SubTrigger", event: "whenOneOfYoursDigivolves", sourceFilter: { controllerDefault: "mine", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Growlmon", "Gallantmon"], match: "name" }] }, actions: [{ kind: "GainKeyword", target: { filter: { controllerDefault: "mine", kind: ["Digimon"] }, count: 1, sourceRef: "triggerSubject" }, keyword: { keyword: "Raid", raw: "＜Raid＞" }, duration: "forTheTurn", cost: { kind: "suspend", target: { filter: { isSelfRef: true }, count: 1, isSelf: true }, raw: "by suspending this Tamer" } }, { kind: "Attack", target: { filter: { isSelfRef: true }, count: 1, isSelf: true }, withoutSuspending: true, attackPlayer: true, mandatory: true }] }] },
