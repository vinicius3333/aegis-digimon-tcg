...
describe("BT19-037 Taomon", () => {
  ...
  it.each([EffectTiming.OnPlay, EffectTiming.WhenDigivolving])(
    ...
    async (timing) => {
      const s = setupEngine({ 0: {
        ...
        hand: [{ card: "BT1-102", as: "option" }, { card: "BT19-020", as: "nonOption" }],
  ...
  it.each([EffectTiming.OnPlay, EffectTiming.WhenDigivolving])(
    ...
    async (timing) => {
      ...
      const s = setupEngine({
        ...
        1: { battleArea: [{ card: "BT1-010", as: "chosen" }, { card: "BT1-011", as: "other" }] },
  ...
  it("inherited When Attacking gives exactly one opponent Digimon -4000 DP", async () => {
    const s = setupEngine({
      ...
      1: { battleArea: [{ card: "BT19-020", as: "chosen" }, { card: "BT19-021", as: "other" }] },
