...
describe("BT19-040 Sakuyamon", () => {
    ...
    const s = setupEngine(
      {
        0: {
          ...
          hand: [
            { card: "BT1-102", as: "single" },
            { card: "BT12-104", as: "multi" },
          ],
        },
      },
  ...
  it("rejects effective use cost 1 and does not trigger from a Security effect or opponent turn (Q5470/Q5471)", async () => {
    ...
    await lowCost.ready();
    await advance(lowCost.engine).fireSubTrigger("whenOptionUsed", {
      usedOptionCost: 1,
      subjectPermanentId: "reduced",
    });
  ...
  it("resolves When Digivolving from a public evolution intent", async () => {
    const s = setupEngine(
      {
        0: {
    ...
    await settle(() => s.perm("maid").topCard?.cardId === "BT19-040");
