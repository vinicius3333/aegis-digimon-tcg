...
describe("BT19-053 QueenBeemon", () => {
  it("publicly uses the Royal Base level-5 cost-3 route and has Alliance/Insectoid", async () => {
    ...
    const s = setupEngine({ 0: {
      battleArea: [{ card: "BT18-052", as: "base" }], hand: [{ card: "BT19-053", as: "queen" }], deck: ["BT19-030"],
    } });
    s.state.memory = 5;
    expect(s.engine.applyIntent(0, {
      type: "digivolve", permanentId: s.perm("base").permanentId, instanceId: s.inst("queen").instanceId,
  ...
  it("When Attacking plays a face-up Royal Base security Digimon with cost reduced by 8 only once", async () => {
    const s = setupEngine({ 0: {
      ...
      security: [{ card: "BT19-052", faceUp: true }, { card: "BT19-052", faceUp: true }, { card: "BT19-046", faceUp: true }],
  ...
  it("places all simultaneously leaving Royal Base Digimon, including self, face-up at security bottom (Q3107/Q3108)", async () => {
    const s = setupEngine({ 0: { battleArea: [
      { card: "BT19-053", as: "queen" }, { card: "BT19-045", as: "royal" }, { card: "BT19-046", as: "plain" },
    ] } }, { autoAcceptOptional: true, autoSelectCards: true });
    await s.ready();
