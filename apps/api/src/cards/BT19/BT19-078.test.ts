...
describe("BT19-078 ADR-01 Jeri", () => {
  it("applies -1000 DP per Mother D-Reaper digivolution card to one opposing Digimon", async () => {
    const mother = {
      permanentId: "mother",
      topCard: instance("MOTHER", 0),
      stack: [instance("STACK-1", 1), instance("STACK-2", 2)],
      linked: [],
      isSuspended: false,
      inBreeding: false,
    ...
    const players = [
      { battleArea: [mother], security: [], hand: [], deck: [], trash: [] },
      { battleArea: [target], security: [], hand: [], deck: [], trash: [] },
    ];
    const defs = new Map([
      ["MOTHER", def("MOTHER", "Mother D-Reaper")],
      ["TARGET", def("TARGET")],
      ["BT19-078", def("BT19-078")],
      ["STACK-1", def("STACK-1")],
      ["STACK-2", def("STACK-2")],
    ]);
    const game: GameAccess = {
      state: { memory: 0, players, turnSeat: seat } as unknown as GameState,
    ...
    const fx = {
      modifyDP: (_id: string, amount: number) => {
        changes.push(amount);
      },
  ...
  it("places itself under an eligible Mother D-Reaper and redirects after inherited Jeri play", async () => {
    ...
    const players = [
      { battleArea: [self, mother], security: [], hand: [], deck: [], trash: [] },
      { battleArea: [], security: [], hand: [], deck: [], trash: [] },
    ];
    const defs = new Map([
      ["MOTHER", def("MOTHER", "Mother D-Reaper")],
      ["BT19-078", def("BT19-078")],
      ["JERI", def("JERI", "ADR-01 Jeri")],
    ]);
    const game: GameAccess = {
      state: { memory: 0, players, turnSeat: seat } as unknown as GameState,
      player: (s: Seat) => players[s] as never,
    ...
    const fx = {
      relocatePermanent: async (destination: string, sourceId: string) => {
        relocated.push([destination, sourceId]);
      },
      playInstances: async () => [{ permanentId: "played-jeri" }],
      redirectAttack: async (ids: string[]) => {
        redirected.push(ids);
      },
    } as unknown as Primitives;
    const source: CardSource = {
