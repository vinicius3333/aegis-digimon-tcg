...
describe("ST4-16 Electro Shocker", () => {
  it("returns a suspended opponent and trashes all its sources", async () => {
    const s = setupEngine(
      {
        0: { battleArea: [{ card: "ST4-03", as: "ally" }], hand: [{ card: "ST4-16", as: "option" }] },
        1: {
          battleArea: [{ card: "ST4-10", under: [{ card: "ST4-03", as: "source" }], as: "target", suspended: true }],
        },
      },
  ...
  it("activates the same return effect from security", async () => {
    const s = setupEngine(
      {
        0: { security: [{ card: "ST4-16", as: "option", faceUp: true }] },
        1: { battleArea: [{ card: "ST4-10", under: ["ST4-03"], as: "target", suspended: true }] },
      },
      { autoSelectCards: true },
    );
