...
it("returns a purple Dark Animal and then trashes a hand card on play", async () => {
  const s = setupEngine({ 0: { hand: [{ card: "BT14-072", as: "fangmon" }, { card: "BT1-001", as: "discard" }], trash: [{ card: "BT14-071", as: "returned" }] } }, { autoSelectCards: true, autoAcceptOptional: true });
  s.state.memory = 10;
