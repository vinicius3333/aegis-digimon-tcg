...
describe("BT14-096", () => {
  it("suspends an already suspended opposing Digimon and restricts unsuspension with Mimi", () => {
    expect(compiled.effects?.[0]?.actions[0]).toMatchObject({ kind: "Suspend", target: { filter: { controller: "opponent", suspended: true } } });
    expect(compiled.effects?.[0]?.actions[1]).toMatchObject({ kind: "Restrict", restriction: "unsuspend", duration: "untilOpponentTurnEnd", condition: { kind: "youHave" } });
  });
