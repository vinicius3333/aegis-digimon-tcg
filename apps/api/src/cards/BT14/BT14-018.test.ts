...
describe("BT14-018", () => {
  ...
  it("deletes its tokens instead of leaving or digivolving and gains Recovery +1 when a token is deleted", () =>
    expect(compiled.effects?.find((entry) => entry.trigger === "AllTurns")).toMatchObject({
      actions: [
        { kind: "Replacement", event: "wouldLeavePlay" },
        { kind: "Replacement", event: "wouldDigivolve" },
        {
          kind: "SubTrigger",
          event: "onDeletionOf",
          actions: [{ kind: "GainKeyword", keyword: { keyword: "Recovery" } }],
        },
      ],
    }));
  it("plays both distinct token variants on play", async () => {
    const s = setupEngine(
      { 0: { hand: [{ card: "BT14-018", as: "source" }] } },
