...
describe("BT14-058", () => {
  ...
  it("runtime", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "BT14-058", as: "numemon" }, { card: "BT14-086", as: "satsuki" }] } }, { autoSelectCards: true, autoAcceptOptional: true });
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("numemon").instanceId })).toEqual({ ok: true });
    await settle(() => {
      const p = s.state.players[0]!.battleArea.find((perm) => perm.topCard?.cardId === "BT14-058");
      return p !== undefined && p.stack.some((card) => card.cardId === "BT14-086") && observe(s.engine).hasKeyword(p, "Rush");
    });
    const p = s.state.players[0]!.battleArea.find((perm) => perm.topCard?.cardId === "BT14-058")!;
