...
describe("BT14-077", () => {
  ...
  it("trashes the top two cards from both decks on play", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "BT14-077", as: "machinedramon" }], deck: ["BT1-001", "BT1-002", "BT1-003"] }, 1: { deck: ["BT1-004", "BT1-005", "BT1-006"] } }, { autoSelectCards: true, autoAcceptOptional: true });
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("machinedramon").instanceId })).toEqual({ ok: true });
