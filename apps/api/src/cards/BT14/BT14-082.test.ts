...
describe("BT14-082", () => {
  it("gives a Vaccine Digimon +2000 DP at the start of main phase", () =>
    expect(compiled.effects?.find((entry) => entry.trigger === "StartOfYourMainPhase")?.actions[0]).toMatchObject({
      kind: "ModifyDP",
      amount: 2000,
      target: { filter: { nameOrTrait: [{ tokens: ["Vaccine"], match: "trait" }] } },
    }));
  ...
  it("plays from the security stack without paying the cost", async () => {
    const s = setupEngine(
      { 0: { security: [{ card: "BT14-082", as: "securityTai", faceUp: true }] } },
