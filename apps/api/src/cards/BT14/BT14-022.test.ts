...
describe("BT14-022", () =>
  it("when attacking trashes one opposing source and returns a source-less level-five-or-lower Digimon", () =>
    expect(compiled.effects?.find((entry) => entry.trigger === "WhenAttacking")).toMatchObject({
      actions: [
        { kind: "TrashDigivolution", amount: 1 },
        {
          kind: "Return",
          to: "hand",
          target: { filter: { digivolutionCards: "none", levelComparison: { op: "lte", value: 5 } } },
        },
      ],
    })));
...
it("trashes one opposing source, then returns a source-less low-level Digimon", async () => {
  const s = setupEngine(
    {
      ...
      1: {
        battleArea: [
          { card: "BT1-015", as: "sourced", under: ["BT1-001"] },
          { card: "BT14-010", as: "returnable" },
        ],
      },
    },
  ...
  expect(
    s.engine.applyIntent(0, { type: "attack", attackerPermanentId: gesomon.permanentId, target: { kind: "player" } }),
  ).toEqual({ ok: true });
