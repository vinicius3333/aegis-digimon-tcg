...
describe("BT14-078", () => {
  ...
  it("trashes a qualifying hand card and deletes an opponent up to the scaled level", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT14-078", as: "source" }], hand: [{ card: "BT14-071", as: "darkAnimal" }] }, 1: { battleArea: [{ card: "BT14-074", as: "target" }] } }, { autoSelectCards: true, autoAcceptOptional: true });
    await advance(s.engine).verb.deletePermanent([s.perm("source").permanentId], "byEffect");
    await settle(() => s.state.players[0]!.trash.some((card) => card.cardId === "BT14-071") && s.state.players[1]!.battleArea.length === 0);
