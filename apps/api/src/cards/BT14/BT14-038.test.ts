...
describe("BT14-038", () => {
  it("plays a level-six Etemon from hand from security when three Sukamon are in trash", () =>
  ...
  it("inherits placing an Etemon from trash as security on deletion", () =>
  ...
  it("plays a level-six Etemon from hand from security when three Sukamon are trashed", async () => {
    const s = setupEngine(
      {
        0: {
          security: [{ card: "BT14-038", as: "securityEtemon", faceUp: true }],
