...
describe("BT14-074", () => {
  ...
  it("trashes a hand card, draws one, and gains memory when Eiji is underneath", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT14-074", as: "source", under: ["BT14-087"] }], hand: [{ card: "BT1-002", as: "discard" }], deck: ["BT1-003"] } }, { autoSelectCards: true, autoAcceptOptional: true });
    s.state.memory = 3;
