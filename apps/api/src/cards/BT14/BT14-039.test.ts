...
describe("BT14-039", () => {
      ...
      actions: [
        {
          ...
          cost: {
            ...
            host: "self",
            target: {
              filter: { zone: "trash", kind: ["Digimon"], nameOrTrait: [{ tokens: ["Numemon"], match: "name" }] },
            },
          },
        },
      ],
      ...
      actions: [
        {
          ...
          while: { kind: "selfHasNameContaining", names: ["Monzaemon", "Numemon"] },
        },
      ],
    });
  });
  ...
  it("places the exact trash Numemon at stack bottom and gains 2 memory on play", async () => {
    ...
    await settle(() =>
      s.state.players[0]!.battleArea.some((permanent) =>
        permanent.stack.some((card) => card.instanceId === s.inst("numemon").instanceId),
      ),
    );
  ...
  it("inherited Security Attack +1 applies only to a Monzaemon/Numemon-named host on its turn", async () => {
    const s = setupEngine({
      0: {
        battleArea: [
          { card: "BT14-066", as: "matching", under: ["BT14-039"] },
    ...
    expect(
      s.engine.applyIntent(0, {
        type: "attack",
        attackerPermanentId: s.perm("matching").permanentId,
        target: { kind: "player" },
      }),
    ).toEqual({ ok: true });
