...
describe("BT14-080", () => {
  ...
  it("trashes three opposing deck cards and grants Security Attack +1 at the ten-card thresholds", async () => {
    const s = setupEngine({ 0: { battleArea: [{ card: "BT14-080", as: "source" }], trash: Array(10).fill("BT1-001") }, 1: { deck: ["BT1-002", "BT1-003", "BT1-004", "BT1-005"], trash: Array(10).fill("BT1-006") } }, { autoSelectCards: true, autoAcceptOptional: true });
    ...
    await settle(() => s.state.players[1]!.trash.length >= 13 && observe(s.engine).keywordAmount(s.perm("source"), "SecurityAttack") === 1);
