...
describe("BT14-047", () => {
  ...
  it("suspends one opposing Digimon on play", async () => {
    const s = setupEngine({ 0: { hand: [{ card: "BT14-047", as: "dokugumon" }] }, 1: { battleArea: [{ card: "BT14-042", as: "target" }] } }, { autoSelectCards: true });
    s.state.memory = 10;
    expect(s.engine.applyIntent(0, { type: "playCard", instanceId: s.inst("dokugumon").instanceId })).toEqual({ ok: true });
