...
describe("BT14-098", () => {
  it("de-digivolves one opposing Digimon, then optionally deletes up to six by returning three D-Brigade/DigiPolice cards", () => {
    ...
    expect(compiled.effects?.[0]?.actions[1]).toMatchObject({ kind: "DeleteBudget", filter: { controller: "opponent", kind: ["Digimon"] }, budget: 6, upTo: true, cost: { kind: "return", target: { count: 3 } } });
  });
