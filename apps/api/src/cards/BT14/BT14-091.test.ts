...
describe("BT14-091", () => {
  it("trashes two opposing digivolution cards across Digimon and conditionally unsuspends the chosen Digimon", () => {
    ...
    expect(compiled.effects?.[0]?.actions[0]).toMatchObject({ kind: "TrashDigivolution", amount: 2, scope: "acrossDigimon" });
    expect(compiled.effects?.[0]?.actions[1]).toMatchObject({ kind: "SelectBind", target: { bindAs: "chosen" } });
    expect(compiled.effects?.[0]?.actions[2]).toMatchObject({ kind: "Unsuspend", target: { fromSelectionRef: "chosen" } });
    expect(compiled.effects?.[0]?.actions[0]).toMatchObject({ target: { filter: { controller: "opponent", digivolutionCards: "hasAny" } } });
