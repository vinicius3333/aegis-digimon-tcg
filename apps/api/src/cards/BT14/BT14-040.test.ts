  ...
  it("may place a Tamer from hand as the top security card on play and digivolution", () => {
    for (const trigger of ["OnPlay", "WhenDigivolving"])
      expect(compiled.effects?.find((entry) => entry.trigger === trigger)?.actions[0]).toMatchObject({
        ...
        from: ["hand"],
        toTop: true,
        source: { filter: { kind: ["Tamer"] } },
      });
  });
  it("once per turn plays a level-three Digimon from hand when a Tamer is played", () =>
      ...
      actions: [
        {
          ...
          actions: [{ kind: "PlayWithoutCost", from: ["hand"], payCost: false, target: { filter: { levels: [3] } } }],
        },
      ],
    }));
