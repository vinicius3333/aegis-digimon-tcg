...
const hasJellymonText = (def: CardDefinition): boolean =>
  def.nameEn.includes("Jellymon") ||
  (def.effectText as string | undefined)?.includes("Jellymon") === true;

const hasLevel5OrHigher = (def: CardDefinition): boolean =>
  def.level !== undefined && def.level >= 5;
...
const isOwnJellymonAttacking = (ctx: EffectContext, source: CardSource): boolean => {
  ...
  return isDigimon(ctx.game.definitionOf(attacker.topCard)) &&
    hasJellymonText(ctx.game.definitionOf(attacker.topCard));
};
...
const isOpponentLv5PlusAttacking = (ctx: EffectContext, source: CardSource): boolean => {
  ...
  if (!attacker.topCard) return false;
  return isDigimon(ctx.game.definitionOf(attacker.topCard)) &&
    hasLevel5OrHigher(ctx.game.definitionOf(attacker.topCard));
};
      ...
      return [
        whenAttacking({
          ...
          when: (ctx) => {
            ...
            return isOwnJellymonAttacking(ctx, source) || isOpponentLv5PlusAttacking(ctx, source);
          },
          canActivate: (ctx) => {
            if (!source.isOnBattleArea()) return false;
