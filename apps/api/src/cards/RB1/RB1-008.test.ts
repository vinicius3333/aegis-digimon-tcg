...
describe("RB1-008 BetelGammamon", () => {
  ...
  it("does not play a second Hiro when one is already in play", async () => {
    const s = setupEngine(
      { 0: { battleArea: [{ card: "RB1-008", as: "betel" }, { card: "RB1-032", as: "existing" }], hand: [{ card: "RB1-032" }] } },
      { autoAcceptOptional: true, autoSelectCards: true },
    );

    await advance(s.engine).fire(EffectTiming.WhenDigivolving, s.perm("betel"));

    expect(s.state.players[0]!.battleArea.filter((permanent) => permanent.topCard?.cardId === "RB1-032")).toHaveLength(1);
