/**
 * Frozen copy of the DDL that AccountStore.initialize() ran before the migrator existed.
 * Test-only: it stands in for a database created by a deployed pre-migrator build, so the
 * upgrade path is exercised against that real starting state rather than against migration 001
 * comparing with itself. Never edit it to match a migration — that is the drift it detects.
 */
export const PRE_MIGRATION_DDL = `
