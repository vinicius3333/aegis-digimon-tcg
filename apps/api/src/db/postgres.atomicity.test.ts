...
import { RoomHandoffStore } from "./roomHandoff/RoomHandoffStore.js";
...
describe.skipIf(!ENABLED)("transaction atomicity against a real Postgres", () => {
  ...
  it("fences a room with owner epochs and atomically claims transfers, commands, and outbox work", async () => {
    const pool = isolatedPool();
    const source = new RoomHandoffStore(pool);
    const destination = new RoomHandoffStore(pool);
    const sessionId = `logical-${randomUUID()}`;
    const participants = [
      { seat: 0 as const, kind: "account" as const, principalId: randomUUID() },
      { seat: 1 as const, kind: "account" as const, principalId: randomUUID() },
    ];
    const now = Date.now();
    try {
      await source.createSession({
        ...
        mode: "ranked",
        participants,
        owner: { generationId: "g-blue", processId: "p-blue", roomId: `room-${randomUUID()}` },
        now,
      });
      await source.saveCheckpoint({
        sessionId,
        ownerEpoch: 1,
        checkpointId: `checkpoint-${randomUUID()}`,
        snapshotSchemaVersion: 1,
        executionVersion: "engine-test",
        rulesVersion: "catalog-test",
        sourceRevision: "revision-test",
        commandSequence: 0,
        checksum: "sha256:fixture",
        snapshot: { board: "fixture" },
        now: now + 1,
      });
      await source.beginTransfer({
        transferId: `transfer-${randomUUID()}`,
        sessionId,
        ownerEpoch: 1,
        target: { generationId: "g-green", processId: "p-green", roomId: `room-${randomUUID()}` },
        now: now + 2,
      });
      const transferId = (await source.getSession(sessionId))!.activeTransferId!;
      expect(await source.getTransfer(transferId)).toMatchObject({
        transferId,
        sessionId,
        fromOwnerEpoch: 1,
        toOwnerEpoch: 2,
      ...
      await source.advanceTransfer({ transferId, ownerEpoch: 1, expected: "preparing", next: "frozen", now: now + 3 });
      const checkpointId = (await source.getCheckpoint(sessionId))!.checkpointId;
      await source.advanceTransfer({
        transferId,
        ownerEpoch: 1,
        expected: "frozen",
        next: "snapshot_saved",
      ...
      // Separate store calls acquire separate pool connections; PostgreSQL row locks decide the
      // winner, not a shared in-process mutex. A lost-response retry resolves to the committed owner.
      const claims = await Promise.all([
        source.claimTransfer({ transferId, expectedOwnerEpoch: 1, now: now + 6 }),
        destination.claimTransfer({ transferId, expectedOwnerEpoch: 1, now: now + 6 }),
      ]);
      expect(claims.every((claim) => claim.ok)).toBe(true);
      expect(claims.filter((claim) => claim.ok && !claim.replayed)).toHaveLength(1);
      expect(claims.filter((claim) => claim.ok && claim.replayed)).toHaveLength(1);
      ...
      expect(atomicFailure).toEqual({ ok: false, reason: "effect_key_conflict" });
      expect(
        (
          await pool.query<{ status: string }>(
            "SELECT status FROM room_commands WHERE session_id=$1 AND command_id=$2",
            [sessionId, commandId],
          )
        ).rows[0]?.status,
      ).toBe("admitted");
      ...
      for (let index = 0; index < 4; index += 1) {
        await destination.enqueueOutbox({
          id: `outbox-${randomUUID()}`,
          effectKey: `side-effect:${sessionId}:${index}`,
          sessionId,
          ownerEpoch: 2,
