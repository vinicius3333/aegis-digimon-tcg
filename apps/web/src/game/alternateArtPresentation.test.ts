...
import { cutInFromEvent } from "./cutIn";
...
describe("physical printing presentation", () => {
  ...
  it("passes selected printing through reveal, play showcase, and evolution cut-in", () => {
    ...
    expect(cutInFromEvent(event, 1, true)?.artId).toBe(event.artId);
