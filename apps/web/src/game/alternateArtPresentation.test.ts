...
describe("physical printing presentation", () => {
  it("keeps different printings of the same card through movement panels and merging", () => {
    const event: ServerEvent = {
      ...
      seat: 1,
      instanceIds: ["a", "b"],
      cardIds: ["BT1-010", "BT1-010"],
      artIds: ["BT1-010_P1", "BT1-010_P2"],
    };
  ...
  it("passes selected printing through reveal, play showcase, and evolution showcase", () => {
    const scene = buildSecurityClashScene({
      ...
      revealedArtId: "BT1-010_P2",
      defenderSeat: 1,
      viewerSeat: 0,
      resolution: "battle",
      attacker: { seat: 0, cardId: "BT1-010", artId: "BT1-010_P1" },
    });
  ...
  it("snapshots retain visible copy art without indexing concealed identities", () => {
    const state = {
      players: [
        {
          hand: [{ instanceId: "visible", cardId: "BT1-010", artId: "BT1-010_P1" }],
          security: [{ instanceId: "hidden", artId: "BT1-010_P2" }],
          trash: [],
          battleArea: [],
        },
      ],
