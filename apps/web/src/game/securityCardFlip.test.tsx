...
it("animates an in-place server flip and finishes with no retained identity or buff", () => {
  ...
  card.faceUp = true;
  const { container, rerender } = render(<Pile shield="you" count={1} label="Security" securityCards={[card]} />);
  ...
  card.faceUp = false;
  card.cardId = "";
  rerender(<Pile shield="you" count={1} label="Security" securityCards={[card]} />);
...
it("does not treat removing a revealed card as turning the next card face-down", () => {
  ...
  const { container, rerender } = render(
    <Pile shield="you" count={2} label="Security" securityCards={[card, hidden]} />,
  );
  rerender(<Pile shield="you" count={1} label="Security" securityCards={[hidden]} />);
  expect(container.querySelector(".game-security-card--turning-down")).toBeNull();
  expect(container.innerHTML).not.toContain("BT19-045");
});
