...
function overlaySource(name: string): string {
  ...
  if (source === undefined)
    throw new Error(`no overlay component file for ${name}`);
  return source;
}
  ...
  return readdirSync(new URL(folder, import.meta.url), {
    ...
    entry.isDirectory()
      ...
      : entry.name === "index.ts"
        ...
        : [
            readFileSync(
              new URL(`${folder}${entry.name}`, import.meta.url),
              "utf8",
            ),
          ],
  );
...
const boardPiecesSource = readdirSync(new URL("./piece/", import.meta.url), {
  ...
  .map((entry) =>
    readFileSync(new URL(`./piece/${entry.name}`, import.meta.url), "utf8"),
  )
  .join("\n");
/** The phone block: narrow, or short and on its side. */
const portraitRules = mediaRules(
  gameCss,
  "(width < 600px), (height < 520px) and (orientation: landscape)",
);
/** The landscape-phone block, which re-lays the board for a short viewport. */
const landscapeRules = mediaRules(
  gameCss,
  "(height < 520px) and (orientation: landscape)",
);
...
const phonePortraitRules = mediaRules(
  gameCss,
  "(width < 600px) and (orientation: portrait)",
);
...
describe("mobile portrait match layout", () => {
  it("keeps the match inside the viewport and limits scrolling to cards", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-field \{[^}]*min-height:\s*0[^}]*overflow:\s*hidden/,
    );
    expect(portraitRules).toMatch(
      /\.game-battle-zones \{[^}]*overflow:\s*hidden/,
    );
    expect(portraitRules).toMatch(
      /\.game-battle-zones > div:last-child \{[^}]*overflow-x:\s*auto[^}]*scroll-snap-type/,
    );
    expect(portraitRules).toMatch(
      /\[data-testid="hand"\] \{[^}]*overflow-x:\s*auto[^}]*scroll-snap-type/,
    );
  ...
  it("stacks the board into rows instead of sizing regions by hand", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-board \{[^}]*grid-template-rows:\s*auto minmax\(0, 1fr\) auto/,
    );
    expect(portraitRules).toMatch(
      /\.game-field \{[^}]*grid-template-columns:[^}]*minmax\(0, 1fr\)/,
    );
    expect(portraitRules).toMatch(
      /\.game-player-dock \{[^}]*grid-template-rows:\s*auto auto[^}]*overflow:\s*hidden/,
    );
    expect(portraitRules).toMatch(
  ...
  it("hides the sidebar panels that do not belong on a phone", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-action-bar--idle \{\s*display:\s*none/,
    );
  });
  ...
  it("sizes board pieces for real instead of scaling them with transforms", () => {
    ...
    expect(portraitRules).not.toMatch(
      /\.game-memory-gauge \{[^}]*width:\s*max-content/,
    );
  });
  ...
  it("lets a sideways swipe scroll the hand instead of dragging a card", () => {
    ...
    expect(portraitRules).toMatch(
      /\[data-testid="hand"\] > div \{[^}]*touch-action:\s*pan-x/,
    );
    expect(portraitRules).toMatch(
      /\.game-battle-zones > div:last-child > div \{[^}]*touch-action:\s*pan-x/,
    );
  });
  ...
  it("lays the hand out as whole cards a finger can hit", () => {
    ...
    expect(portraitRules).not.toMatch(
      /\[data-testid="hand"\] > div \{[^}]*margin-left:\s*-/,
    );
    expect(portraitRules).toMatch(
      /\[data-testid="hand"\] > div \{[^}]*margin-left:\s*var\(--game-hand-gap\)/,
    );
  });
  ...
  it("gives field cards and hand cards a touch sheet", () => {
    ...
    expect(portraitRules).toMatch(
      /\.card-action-sheet \{[^}]*position:\s*fixed[^}]*align-items:\s*end/,
    );
    expect(portraitRules).toMatch(
      /\.card-action-sheet__actions > button \{[^}]*min-height:\s*44px/,
    );
  });
  ...
  it("turns every centred match dialog into a bottom sheet", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-modal__panel \{[^}]*width:\s*100%[^}]*max-width:\s*none/,
    );
    // A bare panel brings no scrim, so it pins itself to the viewport bottom.
    expect(portraitRules).toMatch(
      /\.game-modal__panel--bare \{[^}]*position:\s*fixed[^}]*bottom:\s*0/,
    );
    ...
    expect(portraitRules).toMatch(
      /\.game-modal__panel > div \{\s*min-width:\s*0/,
    );
  });
  ...
  it("declares the board metrics where the portalled overlays can read them", () => {
    ...
    expect(portraitRules).toMatch(
      /:root \{[^}]*--game-hand-h:\s*calc\(var\(--hand-card-width\)/,
    );
    expect(phonePortraitRules).toMatch(/:root \{[^}]*--game-rail:\s*4rem/);
    expect(gameCss).not.toMatch(
      /\.game-layout \{[^}]*--game-(rail|hand-h|sidebar-h)/,
    );
  ...
  it("keeps the contextual action bar under the whole dialog tier", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-action-bar--contextual \{[^}]*z-index:\s*calc\(var\(--ds-z-dialog\) - 6\)/,
    );
    expect(landscapeRules).toMatch(
      /\.game-action-bar--contextual \{[^}]*z-index:\s*calc\(var\(--ds-z-dialog\) - 6\)/,
    );
    expect(gameCss).not.toMatch(
      /\.game-action-bar--contextual \{[^}]*var\(--ds-z-toast\)/,
    );
  });
  ...
  it("keeps the pending-decision return control below the mobile opponent bar", () => {
    ...
    expect(portraitRules).toMatch(
      /\.decision-board-return > button \{[^}]*min-height:\s*44px/,
    );
  });
  ...
  it("spends the decision sheet on the targets, not on its header", () => {
    ...
    expect(portraitRules).toMatch(
      /\.decision-overlay__header \{[^}]*align-items:\s*center[^}]*flex-wrap:\s*nowrap/,
    );
    expect(portraitRules).toMatch(
      /\.decision-overlay__heading \{[^}]*padding:\s*0[^}]*background:\s*none/,
    );
    expect(portraitRules).toMatch(
      /\.decision-overlay__title \{[^}]*font-size:\s*1\.0625rem/,
    );
  ...
  it("lets the effect text flow whole instead of hiding its tail in a scroll box", () => {
    ...
    expect(portraitRules).toMatch(
      /\.decision-overlay__effect-text \{[^}]*font-size:\s*0\.6875rem/,
    );
    expect(portraitRules).not.toMatch(
      /\.decision-overlay__effect-text \{[^}]*max-height/,
    );
  });
  ...
  it("shrinks the board-mode decision rail so the phone board stays readable", () => {
    ...
    expect(portraitRules).toMatch(
      /\.board-prompt \{[^}]*width:\s*min\(15rem, 46vw\)/,
    );
    ...
    expect(portraitRules).toMatch(
      /\.board-opponent-pill \{[^}]*top:\s*3\.5rem/,
    );
  });
});
...
describe("choice rows lead with the affirmative action", () => {
  ...
  it("reverses horizontal rows so the leading action sits on the right", () => {
    expect(gameCss).toMatch(
      /\.game-actions-row \{[^}]*flex-direction:\s*row-reverse/,
    );
    expect(gameCss).toMatch(
      /\.mulligan-actions \{[^}]*flex-direction:\s*row-reverse/,
    );
  });
  ...
  it.each([
    ...
    "%s lists its confirming action before %s",
    (name, confirming, trailing) => {
...
describe("the result splash owns the whole screen", () => {
  ...
  it("stacks result actions on narrow or short viewports", () => {
    ...
    expect(gameCss).toMatch(
      /\.game-result \.game-actions-row \{[^}]*grid-template-columns:\s*1fr/,
    );
    expect(gameCss).toMatch(
      /\.game-result \.game-actions-row > \.aegis-button \{[^}]*min-width:\s*0/,
    );
  ...
  it("keeps result content inside its panel when browser text is enlarged", () => {
    expect(gameCss).toMatch(
      /\.game-result__panel \{[^}]*container-type:\s*inline-size/,
    );
    expect(gameCss).toMatch(
      /\.game-result__title \{[^}]*font-size:\s*clamp\([^)]*cqi/,
    );
    expect(gameCss).toMatch(
      /\.game-result \.game-actions-row \{[^}]*grid-template-columns:\s*repeat\(/,
    );
    expect(gameCss).toMatch(
      /\.game-result__stats \{[^}]*grid-template-columns:\s*repeat\(3, minmax\(0, 1fr\)/,
    );
...
describe("the sidebar strip", () => {
  it("lays the footer buttons side by side", () => {
    ...
    expect(stripRules).toMatch(
      /\.game-sidebar__footer \{[^}]*flex-direction:\s*row/,
    );
  });
});
...
describe("the phone rules win the cascade", () => {
  ...
  it("keeps every width-based block after the last base rule", () => {
    ...
    expect(afterResponsive.replace(/@media[^{]*\{[\s\S]*/, "")).not.toMatch(
      /^\s*\.[\w-]+[^{]*\{/m,
    );
  ...
  it("shrinks the end-turn orb on a phone", () => {
    expect(portraitRules).toMatch(
      /\.game-end-turn-orb \{[^}]*width:\s*3\.4rem/,
    );
  });
});
...
describe("nothing on the phone board is clipped by its neighbour", () => {
  it("floors the hand dock so the action strip cannot squeeze it", () => {
    expect(portraitRules).toMatch(
      /\.game-hand-dock \{[^}]*min-height:\s*calc\(var\(--game-hand-h\)/,
    );
    expect(portraitRules).toMatch(
      /--game-hand-h:\s*calc\(var\(--hand-card-width\)/,
    );
  });
  ...
  it("fits the security shields inside their rail", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-security-shield \{[^}]*width:\s*3rem/,
    );
  });
  ...
  it("caps the action sheet's card by height", () => {
    ...
    expect(portraitRules).not.toMatch(
      /\.card-action-sheet__body > div:first-child \{/,
    );
  });
  ...
  it("lays the stack viewer out as one vertical sheet with wrapping card grids", () => {
    ...
    expect(portraitRules).toMatch(
      /\.stack-sheet \.card-action-sheet__panel \{[^}]*max-height:\s*85dvh/,
    );
    expect(portraitRules).toMatch(
      /\.stack-sheet__groups \{[^}]*display:\s*grid/,
    );
    expect(portraitRules).toMatch(
      /\.stack-sheet__grid \{[^}]*grid-template-columns:\s*repeat\(auto-fill, minmax\(4\.5rem, 1fr\)\)/,
    );
    expect(portraitRules).not.toMatch(
      /\.stack-sheet__grid \{[^}]*overflow-x:\s*auto/,
    );
    ...
    expect(portraitRules).toMatch(
      /\.stack-sheet \.stack-viewer-state__restrictions,[\s\S]{0,80}flex-wrap:\s*wrap/,
    );
  ...
  it("makes the digivolve cost chooser hug its content as a bottom sheet", () => {
    ...
    expect(portraitRules).toMatch(
      /\.evo-cost-prompt \{[^}]*top:\s*auto !important/,
    );
    expect(portraitRules).toMatch(
      /\.evo-cost-prompt \{[^}]*height:\s*auto !important/,
    );
    expect(portraitRules).toMatch(
      /\.evo-cost-prompt \{[^}]*bottom:\s*0 !important/,
    );
    // Bottom-sheet dressing, matching `.card-action-sheet__panel`.
    expect(portraitRules).toMatch(
      /\.evo-cost-prompt \{[^}]*padding:[^;]*env\(safe-area-inset-bottom\)[^;]*;/,
    );
    expect(portraitRules).toMatch(
      /\.evo-cost-prompt \{[^}]*border-radius:\s*var\(--ds-radius-lg\) var\(--ds-radius-lg\) 0 0 !important/,
    );
    expect(portraitRules).toMatch(
      /\.evo-cost-prompt::before \{[^}]*height:\s*4px[^}]*border-radius:\s*999px/,
    );
  });
...
describe("floating chrome keeps clear of the hand and the memory band", () => {
  ...
  it("lets a narration slot show whole from its own edge", () => {
    ...
    // reintroduce one.
    expect(phonePortraitRules).not.toMatch(
      /\.narration-slot\[data-slot="narration-[^"]+"\] \{[^}]*max-height/,
    );
    ...
    expect(phonePortraitRules).toMatch(
      /\.narration-slot\[data-slot="narration-text"\] \{[^}]*bottom:\s*calc\(17rem/,
    );
    expect(gameCss).toMatch(
      /\.narration-slot\[data-slot="narration-text"\] \{[^}]*bottom:\s*calc\(11rem/,
    );
  });
  ...
  it("drops the opponent's moment and the attack call-out below the phone feed", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /\.narration-slot\[data-slot="narration-cards"\] \{[^}]*top:\s*calc\(10\.5rem/,
    );
    expect(phonePortraitRules).toMatch(
      /\.attack-announcement \{[^}]*top:\s*calc\(10\.5rem/,
    );
  });
  ...
  it("gives the phone's single narration band a place of its own", () => {
    ...
    expect(narrowWidthRules).toMatch(
      /\.narration-slot\[data-slot="narration"\] \{[^}]*top:\s*calc\(3\.5rem/,
    );
  ...
  it("gives both dismiss buttons a finger-sized target", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /\.match-notice__close::after \{[^}]*width:\s*44px[^}]*height:\s*44px/,
    );
    expect(phonePortraitRules).toMatch(
      /\.side-panel__close::after \{[^}]*width:\s*44px[^}]*height:\s*44px/,
    );
    ...
    expect(phonePortraitRules).not.toMatch(
      /\.match-notice__close::after \{[^}]*translate:/,
    );
  });
  ...
  it("clears the full-size fanned hand at pointer widths", () => {
    ...
    expect(pointerWidthRules).toMatch(
      /\.narration-slot\[data-slot="narration-text"\] \{[^}]*bottom:\s*calc\(13rem/,
    );
  });
  ...
  it("moves the opponent's moment and the attack call-out off the landscape band", () => {
    ...
    expect(landscapeRules).toMatch(
      /\.narration-slot\[data-slot="narration-cards"\] \{[^}]*top:\s*calc\(3\.25rem/,
    );
    expect(landscapeRules).toMatch(
      /\.attack-announcement \{[^}]*top:\s*calc\(3\.25rem[^}]*translate:\s*none/,
    );
  });
});
...
describe("the drag intent label reads the pointer, not the viewport", () => {
  it("asks the pointer media query rather than guessing from the width", () => {
    ...
    expect(gameScreenSource).toMatch(
      /top:\s*y - dragIntentLabelOffsetPx\(coarsePointer\)/,
    );
  });
});
...
describe("the portrait phone spends its height on the cards, not on empty rows", () => {
  it("hands the recovered height to bigger hand cards", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /--hand-card-width:\s*clamp\(5\.5rem, 25vw, 7rem\)/,
    );
  });
  ...
  it("gives the hand dock one row of its own so the cards cannot be cut off", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /\.game-player-dock \[data-testid="hand"\] \{\s*margin:\s*0 !important/,
    );
  });

  it("drops the rotated breeding caption and keeps the band one row", () => {
    expect(phonePortraitRules).toMatch(
      /\.game-breeding-dock > div:first-child \{\s*display:\s*none/,
    );
    expect(phonePortraitRules).toMatch(
      /\.game-breeding-dock \{[^}]*grid-template-columns:\s*minmax\(0, 1fr\)/,
    );
  });
});
...
describe("nothing on the portrait phone board paints past the screen edge", () => {
  it("widens the rail to hold the pile's depth and the slot beside it", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /\.game-pile__layer:nth-child\(n \+ 4\) \{\s*display:\s*none/,
    );
  });
  ...
  it("floors the pile buttons at a finger's width", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /:has\(> \.game-pile\)[^}]*\{[^}]*min-width:\s*44px/,
    );
  });
  ...
  it("keeps the pile's depth layers off its caption", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /\.game-pile \+ span \{[^}]*margin-top:\s*6px/,
    );
  });
  ...
  it("keeps the end-turn orb inside the memory band", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /\.game-end-turn-orb \{\s*margin:\s*0 0\.25rem/,
    );
  });
  ...
  it("gives the memory band the whole screen, on one line", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /\.game-memory-gauge__phase \{\s*display:\s*none/,
    );
    expect(phonePortraitRules).toMatch(
      /\.game-memory-gauge \{[^}]*padding:\s*0 !important/,
    );
  });

  it("pulls the card sparkles inside the card they belong to", () => {
    expect(phonePortraitRules).toMatch(
      /\.game-card-sparkles \{\s*inset:\s*-18% 0 45%/,
    );
    expect(phonePortraitRules).toMatch(
      /\.game-card-sparkle \{\s*translate:\s*-50% 0/,
    );
  });
});
...
describe("the mulligan sheet keeps the opening hand next to its copy", () => {
  it("stops the card grid centring itself in the track it sits in", () => {
    expect(phonePortraitRules).toMatch(
      /\.mulligan-cards \{\s*align-content:\s*start/,
    );
  });
});
...
describe("the viewer's own moves on a phone", () => {
  it("puts the match log control on the header row with the bug and surrender ones", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-mobile-surrender,\s*\.game-mobile-log,\s*\.game-mobile-bug \{/,
    );
    expect(gameScreenSource).toMatch(
      /className="game-mobile-log"[\s\S]*?onClick=\{onOpenLog\}/,
    );
    expect(gameScreenSource).toMatch(
      /onOpenLog=\{\(\) => overlays\.setHistoryOpen\(true\)\}/,
    );
    expect(gameScreenSource).not.toMatch(
      /game-mobile-fullscreen|game-log-strip/,
    );
...
describe("one narration band on a phone in portrait", () => {
  it("folds the viewer's and the opponent's moments into a single top slot", () => {
    ...
    expect(narrowWidthRules).toMatch(
      /\.narration-slot\[data-slot="narration"\] \{[^}]*overflow:\s*visible/,
    );
    expect(narrowWidthRules).not.toMatch(
      /\.narration-slot\[data-slot="narration"\] \{[^}]*overflow:\s*hidden/,
    );
    ...
    expect(gameScreenSource).toMatch(
      /<NarrationStack[\s\S]*?compact=\{collapseNotices\}/,
    );
  });
});
...
describe("the board-mode rail becomes a bottom sheet on a phone in portrait", () => {
  it("anchors the prompt above the hand instead of over the field", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /\.board-prompt \{[^}]*animation-name:\s*battle-board-sheet-in/,
    );
    expect(gameCss).toMatch(/@keyframes battle-board-sheet-in \{/);
    // The pending decision's only exit outranks anything transient.
    expect(phonePortraitRules).toMatch(
      /\.board-prompt \{[^}]*z-index:\s*calc\(var\(--ds-z-dialog\) - 3\)/,
    );
    expect(phonePortraitRules).toMatch(
      /\.board-prompt__actions > button \{[^}]*min-height:\s*52px/,
    );
  });
  ...
  it("drops a yes/no prompt over the hand and keeps a hand selection above it", () => {
    ...
    expect(phonePortraitRules).toMatch(
      /\.board-prompt-scrim\[data-variant="prompt"\] \{[^}]*inset:\s*0;/,
    );
    expect(phonePortraitRules).not.toMatch(
      /\.board-prompt\[data-variant="selection"\]/,
    );
  });
...
describe("the trigger chooser is one readable column on a phone in portrait", () => {
  it("stacks the pending effects instead of splitting the width between them", () => {
    expect(phonePortraitRules).toMatch(
      /\.trigger-chooser \{[^}]*grid-template-columns:\s*1fr/,
    );
    expect(phonePortraitRules).toMatch(
      /\.trigger-chooser__option \{[^}]*min-width:\s*0/,
    );
  });

  it("shrinks the art to a thumbnail so the printed clause keeps the width", () => {
    expect(phonePortraitRules).toMatch(
      /\.trigger-chooser__card > div \{[^}]*width:\s*64px/,
    );
    expect(phonePortraitRules).toMatch(
      /\.trigger-chooser__meta \{[^}]*max-width:\s*100%/,
    );
  });
...
describe("a board-mode hand selection is answered by the finger that makes it", () => {
  it("takes the pick from the pointer instead of the click a touch may never send", () => {
    ...
    expect(boardPiecesSource).toMatch(
      /onPointerUp=\{selection \? \(e\) => finishPick\(/,
    );
    expect(boardPiecesSource).toMatch(/onPointerCancel=\{\s*selection\s*\?/);
    expect(boardPiecesSource).toMatch(
      /pressGesture\(\{ dx: event\.clientX - press\.x/,
    );
  ...
  it("keeps the row pannable while it is a selection surface", () => {
    ...
    expect(boardPiecesSource).toMatch(
      /cursor: pickable \? "pointer" : "default", touchAction: "pan-x"/,
    );
  });
...
describe("centre-stage cards are sized by the viewport", () => {
  it("scales the zone showcase by width on a phone and by height in landscape", () => {
    expect(portraitRules).toMatch(
      /\.battle-showcase__art > div \{[^}]*width:\s*min\(190px, 46vw\)/,
    );
    expect(landscapeRules).toMatch(
      /\.battle-showcase__art > div \{[^}]*height:\s*min\(58dvh, 266px\)/,
    );
  });
  ...
  it("drives the security branch's lift from a property the short blocks retune", () => {
    ...
    expect(gameCss).toMatch(
      /\.battle-security-branch \{[^}]*--security-branch-lift:\s*8rem/,
    );
    expect(gameCss).toMatch(
      /@keyframes battle-security-branch \{[^}]*var\(--security-branch-lift\)/,
    );
    expect(gameCss).not.toMatch(/translate:\s*[^;]*calc\(-50% - 8rem\)/);
    expect(landscapeRules).toMatch(
      /\.battle-security-branch \{[^}]*--security-branch-lift:\s*2\.5rem/,
    );
    expect(landscapeRules).toMatch(
      /\.battle-security-branch__frame > div \{[^}]*width:\s*92px/,
    );
  });
});
...
describe("the mulligan's three actions fit the row they are given", () => {
  it("wraps the peek button onto its own line at narrow widths only", () => {
    ...
    expect(narrowWidthRules).toMatch(
      /\.mulligan-actions \{\s*flex-wrap:\s*wrap/,
    );
    expect(narrowWidthRules).toMatch(
      /\.mulligan-actions > button:last-child \{\s*flex-basis:\s*100%/,
    );
...
describe("landscape phone match layout", () => {
  ...
  it("keys the phone layout on the short viewport as well as the narrow one", () => {
    expect(gameCss).toMatch(
      /@media \(width < 600px\), \(height < 520px\) and \(orientation: landscape\) \{/,
    );
    expect(gameScreenSource).toMatch(
  ...
  it("gives the strip a column of its own beside the board", () => {
    ...
    expect(landscapeRules).toMatch(
      /\.game-layout \{[^}]*grid-template-columns:\s*minmax\(0, 1fr\) auto/,
    );
    expect(landscapeRules).toMatch(
      /\.game-sidebar > div:nth-child\(2\) \{[^}]*width:\s*9rem/,
    );
  ...
  it("puts the breeding area beside the hand instead of above it", () => {
    ...
    expect(landscapeRules).toMatch(
      /\.game-player-dock \{[^}]*grid-template-columns:\s*auto minmax\(0, 1fr\)/,
    );
    expect(landscapeRules).toMatch(
      /\.game-breeding-dock > div:first-child \{\s*display:\s*none/,
    );
  ...
  it("keeps the memory band a single row", () => {
    ...
    expect(landscapeRules).toMatch(
      /\.game-battle-zones > div:nth-child\(2\) \{[^}]*display:\s*flex/,
    );
  });

  it("lays the rail piles two to a row", () => {
    expect(landscapeRules).toMatch(
      /\.game-pile-column > div \{[^}]*flex-direction:\s*row !important/,
    );
  });
  ...
  it("compacts the decision dialog instead of letting it swallow the board", () => {
    ...
    expect(landscapeRules).toMatch(
      /\.game-modal__panel > \.aegis-badge \{\s*display:\s*none !important/,
    );
    expect(landscapeRules).toMatch(
      /\.game-modal__panel > h2 \{[^}]*font-size:\s*var\(--ds-text-lg\)/,
    );
  ...
  it("keeps the result dialog's own buttons on screen", () => {
    expect(landscapeRules).toMatch(
      /\.game-over-dialog > div:first-child \{\s*display:\s*none !important/,
    );
    expect(landscapeRules).toMatch(
      /\.game-over-dialog > h1 \{[^}]*font-size:\s*var\(--ds-text-2xl\)/,
    );
  });
  ...
  it("keeps the turn orb a finger wide", () => {
    ...
    expect(landscapeRules).toMatch(
      /\.game-end-turn-orb \{[^}]*width:\s*2\.75rem[^}]*margin:\s*-0\.375rem/,
    );
  });
  ...
  it("names an explicit card width for the battle rows", () => {
    ...
    expect(gameScreenSource).toMatch(
      /const LANDSCAPE_PHONE_PERMANENT_WIDTH = \d+;/,
    );
    expect(gameScreenSource).toMatch(
...
describe("the phone hand strip during a board-mode selection", () => {
  it("separates the cards so two selection rings cannot merge", () => {
    expect(portraitRules).toBeDefined();
    expect(portraitRules).toMatch(
      /\[data-testid="hand"\] \{[^}]*--game-hand-gap:\s*16px/,
    );
    expect(portraitRules).toMatch(
    ...
    expect(portraitRules).toMatch(
      /\[data-testid="hand"\]\.game-hand--selecting \{\s*--game-hand-gap:\s*22px/,
    );
  });
  ...
  it("tells an eligible card from a picked one by colour, not by brightness", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-hand-card--picked \{[^}]*scale:\s*1\.03/,
    );
    ...
    expect(portraitRules).toMatch(
      /\.game-hand-card--unpickable \{\s*opacity:\s*0\.8/,
    );
    expect(gameCss).toMatch(
      /\.game-hand-card--unpickable > \* \{\s*filter:\s*brightness\(0\.55\)/,
    );
    // The same three states exist outside the phone block, so the pointer layout
    // reads the selection the same way.
    expect(gameCss).toMatch(
      /\.game-hand-card--pickable \{[^}]*var\(--ds-card-rim-ready\)/,
    );
  });
  ...
  it("keeps the pick-order badge inside the card the strip clips", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-hand-card__pick-badge \{\s*top:\s*0\.15rem;\s*right:\s*0\.15rem/,
    );
  });
  ...
  it("hangs the scroll cues over the ends of the strip", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-hand-scroller \{[^}]*display:\s*block[^}]*position:\s*relative/,
    );
    expect(portraitRules).toMatch(
      /\.game-hand-scroll-cue \{[^}]*position:\s*absolute[^}]*top:\s*50%/,
    );
    expect(portraitRules).toMatch(
      /\.game-hand-scroll-cue--start \{\s*left:\s*0/,
    );
    expect(portraitRules).toMatch(
      /\.game-hand-scroll-cue--end \{\s*right:\s*0/,
    );
  });
...
describe("the draw cue on a phone", () => {
  ...
  const useMatchCuesSource = [
    ...
    ...["", "steps/", "present/", "narration/"].flatMap((group) =>
      readdirSync(new URL(`./match/${group}`, import.meta.url), {
        ...
        .map((entry) =>
          readFileSync(
            new URL(`./match/${group}${entry.name}`, import.meta.url),
            "utf8",
          ),
        ),
    ),
  ].join("\n");
  const reducedMotionRules = gameCss.match(
  ...
  it("flies a hand-card-sized back instead of a 30px one", () => {
    ...
    expect(portraitRules).toMatch(
      /\.game-draw-burst \{[^}]*--draw-flight-w:\s*var\(--hand-card-width\)/,
    );
    expect(portraitRules).toMatch(
      /\.game-draw-flight \{[^}]*box-shadow:[^}]*var\(--battle-cyan\)/,
    );
  });
  ...
  it("centres the card back on the pile with its own margins, at any size", () => {
    ...
    expect(gameCss).toMatch(
      /\.game-draw-burst \{[^}]*margin-top:\s*calc\(var\(--draw-flight-h\) \/ -2\)/,
    );
  ...
  it("gives the trip its longer touch duration from the queue, not from CSS alone", () => {
    ...
    expect(useMatchCuesSource).toMatch(
      /isTouchLayout\(\) \? TIMINGS\.drawFlightTouch : TIMINGS\.drawFlight/,
    );
    ...
    expect(portraitRules).not.toMatch(
      /\.game-draw-flight \{[^}]*animation-duration/,
    );
  });
  ...
  it("still stands down for reduced motion", () => {
    expect(reducedMotionRules).toMatch(
      /\.game-draw-flight \{\s*display:\s*none/,
    );
    expect(reducedMotionRules).toMatch(
      /\.game-hand-card--drawn::after \{\s*display:\s*none/,
    );
  });
});
