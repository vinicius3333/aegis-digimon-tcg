...
export function AccountPanel({
  ...
  async function saveDisplayName(event: FormEvent) {
    ...
    try {
      ...
      if (error instanceof AccountApiError && error.code === "display_name_taken")
        setNameError(t("account.nickname.taken"));
      else if (error instanceof AccountApiError && error.code === "too_many_requests")
        setNameError(t("account.nickname.rateLimit"));
      else if (error instanceof AccountApiError && error.code === "invalid_display_name")
        setNameError(t("account.nickname.invalid"));
    ...
    return (
      <div className="account-panel account-panel--signed-out">
        <div className="account-panel__intro">
          <span className="account-panel__intro-icon">
        ...
        >
          {t("account.signInDiscord")}
        </Button>
        <div className="account-panel__or" role="separator">
        ...
        <form className="account-panel__magic-link" onSubmit={(event) => void sendLink(event)}>
          ...
          />
          <Button type="submit" variant="secondary" icon={Icons.Send}>
    ...
    <div className="account-panel">
      <div className="account-panel__identity">
        ...
        <div className="account-panel__identity-copy">
          ...
          <Badge tone="success" className="account-panel__connected">
            <span className="account-panel__connected-dot" />
            {t("account.connected")}
          </Badge>
        </div>
        <Button
          ...
          variant="secondary"
          icon={Icons.LogOut}
          onClick={() => void accountApi.logout().then(() => location.reload())}
        >
          {t("account.signOut")}
        </Button>
      </div>
        ...
        {editingName ? (
          <form className="account-panel__nickname-form" onSubmit={(event) => void saveDisplayName(event)}>
            ...
            <div className="account-panel__nickname-actions">
              ...
              <Button type="submit" size="sm" disabled={savingName || nameInput.trim() === account.displayName}>
                {savingName ? t("account.nickname.saving") : t("common.save")}
              </Button>
            </div>
          </form>
        ) : (
        ...
        <section className="account-panel__section">
          ...
          {profile.matches.slice(0, 5).map((match) => (
            <div key={match.id} className="account-panel__match" data-result={match.result}>
              ...
              <strong>
                {match.result === "win"
                  ? t("account.result.win")
                  : match.result === "loss"
                    ? t("account.result.loss")
                    : t("account.result.draw")}
              </strong>
            </div>
          ))}
        </section>
