...
scenario("alternate-arts", () => {
  ...
  it("synchronizes mixed printings of the same card from join options and keeps the played copy's art", async () => {
    ...
    // Seed 4 deals two copies of the Biyomon slots. Swap those slots with Agumon,
    // keeping the deck legal while giving each Agumon copy a distinct printing.
    const mainDeck = RED_DECK.mainDeck.map((cardId) =>
      cardId === "BT1-012" ? "BT1-010" : cardId === "BT1-010" ? "BT1-012" : cardId,
    );
    let agumonCopy = 0;
    const printings = ["BT1-010", "BT1-010_P1", "BT1-010_P2", "BT1-010_P3"];
    const mainDeckArts = mainDeck.map((cardId) => (cardId === "BT1-010" ? printings[agumonCopy++]! : cardId));
    ...
    try {
      ...
      const selected = images.find((image) => image.getAttribute("src")?.includes("BT1-010_P"))!;
      ...
      expect(within(screen.getByTestId("hand")).getAllByRole("img", { name: /^agumon$/i })).toHaveLength(1);
    } finally {
