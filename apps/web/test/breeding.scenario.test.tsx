...
scenario("breeding", () => {
  ...
  it(
    "hatching and moving a Digimon from breeding renders it in the battle area",
    async () => {
      ...
      await vi.waitFor(
        () => expect(within(yourBreedingSlot()).getByRole("img", { name: /^biyomon$/i })).toBeTruthy(),
        { timeout: 10_000 },
      );
      ...
      fireEvent.click(
        within(secondBreedingHeading.parentElement!).getByRole("button", { name: /move to battle area/i }),
      );
  ...
  it(
    "offers Mother D-Reaper's official DP-based move and renders it in battle",
    async () => {
      vi.stubEnv("VITE_AEGIS_API_URL", server.endpoint);
      const { GameScreen } = await import("../src/game/GameScreen");
      const joinOptions: AegisJoinOptions & { seed?: number } = {
