...
Copy to the session scratchpad, replace `BT26`, `eae2005bb57a0d443f2b7d02dc62cbd26e8a0a91` and the
exemplar list, and point every lane at it.

---

# BT26 re-audit worker brief (card-only lane)
...
- Rulings: `node tools/kb/query.mjs card <ID>` and
  ...
  is authoritative. `docs/audits/BT26-reaudit/KB-INDEX.md` lists the Q&A ids
  you must cover.
- Module and test: `apps/api/src/cards/BT26/<ID>.ts` and `<ID>.test.ts`.
- Context: `docs/audits/BT26-reaudit/REVIEW-NOTES.md` and `RUN.md` sections
  that mention your card; the mechanism docs for any engine seam named there.
- Exemplar tests in the accepted style: BT26-001.test.ts, BT26-016.test.ts, BT26-050.test.ts, BT26-104.test.ts. Reuse their
...
Only `<ID>.ts`, `<ID>.test.ts` and `docs/audits/BT26-reaudit/<ID>.md`.
...
```bash
pnpm --filter @aegis/api exec vitest run src/cards/BT26/<ID>.test.ts --maxWorkers=1 --no-file-parallelism
pnpm --filter @aegis/api typecheck
pnpm exec oxlint <files>; pnpm exec oxfmt --check <files>; git diff --check
```

Logs go to `docs/audits/BT26-reaudit/logs/<ID>-<what>.log`.
...
## Session resource contract

Do not run collection suites, workspace typecheck, pnpm install, builds, or more than one focused vitest process. The coordinator owns broad validation. Remove @ts-nocheck from every assigned module and resolve all resulting errors without suppressions.
