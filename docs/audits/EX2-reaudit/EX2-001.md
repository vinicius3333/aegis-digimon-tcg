...
| inherited effect | `isInherited: true` | Real stack route starts in `eggDeck`, hatches EX2-001, evolves EX2-008 onto it, and asserts the egg remains in the stack when the host attacks |
...
- Subsequent fixture sanitation replaced all accidental Digi-Egg `BT1-001` deck/security fillers with inert main-deck `BT1-009`–`BT1-014`; no rerun was started under the RAM guard.
...
No card-specific rules ambiguity or engine seam was exposed. The focused suite needs to rerun after the deck-size fixture correction, followed by scoped lint/format and `git diff --check`; no claim of a final 10/10 gate is made until those commands are green.
