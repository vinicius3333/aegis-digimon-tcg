...
The engine now also derives pay-before-payload behavior from either the explicit IR flag or a recognized printed `By` condition whose action text contains a later `may`. Therefore declining that payload cannot undo a paid cost or restore the Once Per Turn use. Focused stale expectations were corrected for BT22-011, BT25-080, BT26-006, BT26-070, BT26-074, and EX13-006. Their handwritten IR now carries the explicit pay-before-payload flag; BT26-006/070/074 also preserve their previously omitted printed `By` text.
...
Tests ran sequentially, one worker, with 1536 MiB V8 old-space ceilings for runner and worker. This is a heap ceiling, not a total RSS limit. No full card suite, browser/server, or parallel test processes were started. An attempted effects-sync command invoked the API build and hit the 1536 MiB heap ceiling; it emitted no TypeScript diagnostic and no further broad build was attempted.

Original report cards: 3 files / 45 tests passed (12.11 s cold). Red regression: 3 passed / 3 failed (8.38 s after rebuilding shared). Fixed regression: 6 passed / 6 (2.80 s). Final focused card, receipt, mandatory-prefix, and replacement verification: 9 files, **103 passed** (4.27 s). Scoped Oxlint/Oxfmt and `git diff --check` passed. Shared typecheck passed; API typecheck exceeded 1.5 GiB and 2 GiB heap limits, and the 3 GiB retry was stopped when another worktree's concurrent `tsc` made the machine unresponsive. No TypeScript diagnostic was emitted before those resource stops.
