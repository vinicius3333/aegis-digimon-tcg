...
`de4dda717d8c9e0c2420796cb387f68b1379b863`.
...
| No partial compound payment                                                    | BT16-090, BT23-090, EX3-035, ST17-10 and compound proofs below                     | verified for observed classes |

## Consumer coverage

EX10-052 public attacks and evolution exercise whole-clause costs. BT9-042
exercises payment with an absent payload target. The current finite consumer table below reconciles action costs, CostGatedBlock,
...
The finite consumer table and compound sections below are authoritative for the
22 observed cost kinds and 19 observed compound classes. Earlier queued or
...
| Resident Main declared condition must be paid before optional subsequent content | §15-8-4-4-1 | BT17-085 first optional prompt sees empty stack before payment                   | Verified for the observed declaration class                    |
...
## Ordered loose placement host checkpoint (2026-09-13)

Baseline `b88aeb69f22995641622ab4388086ff771b23dc0` reopens the historical BT14-090 accepting Option-use certificate. Pristine `git archive HEAD` reproduces 17 passed / 1 failed in `activation-processing-costs.test.ts`: accepting payable processing with no subsequent WarGreymon leaves Agumon's stack empty. The changed tree reproduces the same failure. A stronger material-placement wait also fails; the earlier wait-race explanation is not a verified resolution.

Static trace identifies a concrete compatibility gap in `payCost`'s `compound` / `orderPlacedCards` loose-card branch. It requires an object-valued first host and returns false otherwise. BT14-090 uses the supported string `host:"target"` plus an exact Agumon `underFilter` and `bindHostAs`; ordinary placement and payment preflight already support that shape. The ordered compound branch must resolve the same host filter, bind that physical host once and place both selected exact trash cards in the chosen bottom order. This is a payment primitive gap, not proof that targetless processing itself should be denied.

A serialized Luna lane is correcting only this host resolution while preserving object/bound-host, distinct-component, atomic placement and ordering semantics. Existing BT14-090, compound-assignment and object-host consumer tests are the required red/green controls. No corrected gate or full activation-cost completion is claimed yet; final regression/typecheck and committed delivery remain pending.

The narrow host compatibility correction now resolves the first `host:"target"` through its underFilter and retains object/bound-host behavior. The manual BT14-090 witness chooses the host before the ambiguous Greymon under the atomic branch; candidate identities, duplicate preservation, both payments before optional evolution and the same physical host are retained. Focus passes **7 files / 71 tests**. The coordinator broad checkpoint independently passes activation processing, compound assignment, BT14-090 and BT25-096, with **267 files / 2879 tests passed** and three unrelated EX6 fixture failures. All included engine controls are green; final typecheck, remaining collection clauses and delivery remain pending.
