...
| `compound`                        | BT14-090 named pair; BT23-060 borrowed; BT16/19/23/25/EX5/EX10/EX13 mixed shapes | `activation-cost-compound-assignment.test.ts`, `activation-cost-borrowed-effects.test.ts` | whole-clause and borrowed paths proved; 19 current consumers are classified by named, self/permanent, memory/placement, and stack-source shapes; no same-card overlap found, but only the named-pair and borrowed cases have behavioral proof |
