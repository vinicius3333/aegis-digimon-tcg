...
The selector now removes candidates individually above the aggregate cap, prevents both ordinary automatic selection shortcuts when a cap is present, and forwards the aggregate maximum to the existing decision API. The API publishes the maximum and clamps received selections in their given order to a valid subset. An over-budget response is accepted under this existing protocol but cannot play the discarded IDs; this correction does not change the response contract into rejection. The same remaining-budget check now covers the specialized required-name, distinct-name, distinct-card-number, and distinct-level branches, including their nested prompts, so no branch can return a combination above the aggregate cap. A dedicated `fromOwnDigivolutionStack` play path now applies the same individual and aggregate checks after matching its source permanent's stack.

The focused mechanism witness in `apps/api/src/engine/effects/interpreter/targeting/loose.test.ts` drives all four specialized loose branches with two cards costing four and three against a five-card budget; each returns only the first card in the decision order. Its exact-name case proves an all-or-none required set is rejected when its combined cost is six, while sequential exact-name and level prompts publish the remaining budget after the first choice. An out-of-group decision ID is ignored safely. `apps/api/src/engine/effects/interpreter/actions/play.test.ts` drives the own-stack path with the same 4+3 versus 5 case and observes only the first instance reaching `playInstances`.
...
| Enforce aggregate limits with specialized loose selection constraints | Complete combined contract/source denominator to establish      | specialized pickLoose branches           | Focused mechanism witness covers required names, distinct names, card numbers, and levels with 4+3 against 5; no current persisted combined consumer found | No bounded-field combined consumer found | mechanism verified |
...
Closing regression and delivery gates are recorded below. Remaining queued obligations prevent mechanism certification.

Full default API regression (`pnpm --filter @aegis/api exec vitest run --maxWorkers=1 --no-file-parallelism`) passed 5107 files / 42248 tests, with five declared expected failures (42253 total). At that checkpoint the remaining requirements were EX13-002 name exclusion, EX13-020 DP scaling, EX13-043 Option-use reduction, EX13-063 pooled Guard payment and EX13-076 battle-scoped grant expiry. The bounded EX13-076 defect was subsequently corrected in [battle-duration-boundaries.md](battle-duration-boundaries.md); EX13-002's name rule was subsequently corrected in [name-standardization.md](name-standardization.md). EX13-020, EX13-043 and EX13-063 remain open. This result is not full-audit certification. The configured opt-in Postgres lane was not executed; no database behavior changed.
