...
## Digi-Egg bottom-placement checkpoint

### Contract and sources

Complete committed catalog records and direct modules for BT13-007 King Drasil_7D6, EX6-006 Gate of Deadly Sins and the distinct top-placement consumer BT22-007 Mother Eater were read. King Drasil explicitly reveals the egg before placing it; Gate does not specify face-down placement. Full reviewed comprehensive-0292 §4-7-5 requires face-up bottom cards unless otherwise specified. Its source fingerprint is enforced in all four new public cases. Full local card queries were read: Drasil Q2263 requires as-much-as-possible processing, Q2264 additionally permits joint card-order selection; Gate Q3694 requires deletion even with an empty egg deck. Stale cross-card Drasil query hits are not imported as printed obligations. No fresh external card-page read is claimed.

### Implementation and public proof

`createBreedingVerbs.placeUnderFromEggDeck` previously took the real egg-deck top but marked it face-down, and emitted no insertion-position metadata. It now marks that placed source face-up and reports the actual insertion position (default bottom, explicit `belowTop` top). Physical identity/order, actual deck movement and effect-seat provenance are preserved. The separate top-placement verb is not corrected or certified in this slice. Direct module search finds these two bottom-placement authors and one separate top author; a complete dynamic/serialized producer denominator remains open.

Four new legal production turns in the placement conformance file stage a zero-DP breeding Digi-Egg over an existing source, a quiet opposing Digimon, a quiet own level-3 and neutral deck/security. `advance.runTurn` drives actual Active/Draw/Breeding/Main/End processing and public voluntary end intent; there is no injected timing or primitive. Occupied zero-DP breeding has no hatch/move action. These are seeded existing breeding stacks; their full initial formation history is not certified. Both consumers with an egg assert exact bottom order, face-up sources, remaining egg identity, original host/controller/zero DP, actual bottom addition event and full final zones. Their empty-deck controls assert no addition or movement; Gate still deletes its own neutral Digimon while Drasil preserves the non-Royal Knight. Opponent and security remain intact. These paths do not execute Drasil's Royal Knight collection or Gate's conditional Demon Lord addition.

### Baseline and gates

Valid baseline: **2 failed, 13 passed**, both paid placement visibility assertions fail while both empty controls and eleven earlier EX9 controls pass. Corrected four-file public/consumer/breeding regression focus passes **29 tests**. Reverting only visibility fails two/15; removing only insertion-position metadata independently fails two/15, with thirteen controls green in each experiment. Exact fixed bytes are restored in `finally`. Final strengthened focus `pnpm --filter @aegis/api exec vitest run src/engine/conformance/digivolution-card-placement.test.ts src/cards/BT13/BT13-007.test.ts src/cards/EX6/EX6-006.test.ts src/engine/cards/breedingCluster.test.ts src/cards/audit-docs.test.ts`: **5 files, 33 tests passed**, 2.33 seconds. Additional assertions establish first-turn skipped draw, unchanged own deck/hand, no optional processing decision and voluntary-end memory −3. Collection command `pnpm --filter @aegis/api exec vitest run src/cards/BT13 src/cards/EX6`: **187 files, 1135 tests passed**, 7.89 seconds. Full `pnpm --filter @aegis/api exec vitest run`: **5113 files, 42388 tests passed**, zero expected failures, 55.79 seconds. `pnpm typecheck`: shared/API/web passed. Scoped Oxlint has no warnings; all ten changed files pass Oxfmt; `pnpm audit:index --check` confirms 66 sets; `git diff --check` is clean. Independent read-only review read the full source and found no blocker in this bounded repair. No compiled card parameter or persisted IR changes: the direct module change only corrects its face-state comment; effects synchronization is unnecessary.

### Open obligations

Joint reveal/egg/Royal Knight ordering under Q2264 was not established by the original separate authored actions; the bounded BT13-007 repair and public proof are recorded below. Top-placement Mother Eater metadata, explicit top insertion, downstream event reaction, visible inherited-source consequences, cross-controller/face-state overrides, all collection clauses and a complete normative/producer denominator remain open. No whole-card ten-point or collection-completion claim is supported by these four public turns. Collection status and current consumer caps belong to [BT13.md](../BT13.md) and [EX6.md](../EX6.md).

## King Drasil joint Digi-Egg and Royal Knight placement checkpoint

BT13-007 Q2263 requires as-much-as-possible processing, and Q2264 says that when multiple cards are placed by the Start of Main effect, the activating player chooses their order; all such cards go under already placed digivolution cards. Comprehensive-0292 §3-1-3-4 and §4-7-5 provide the general owner-order and face-up stack rules. The full local card query and direct BT13-007/EX6-006 modules were read. EX6-006 Q3694 separately confirms its deletion clause still resolves with an empty Digi-Egg deck.

The `king-drasil-joint-placement.test.ts` cases run complete public turns with a seeded BT13-007 breeding host carrying an existing source, a face-down egg, two distinct Royal Knights, and quiet controls. The first case verifies the automatic path: the revealed egg and both Royal Knight tops are placed beneath the existing source in the expected bottom-first stack order, all moved cards are face-up, the egg deck is emptied, both Knights leave the battle area, and the quiet control remains. The second disables automatic ordering, observes one public `orderCards` decision containing exactly the egg and both Royal Knight top-card identities, responds with a contrasting order, and verifies that exact order in the resulting stack. Each moved Knight's own source card is trashed, while the host's pre-existing source is preserved. Focused result: **2 tests passed**.

This closes the bounded BT13-007 Q2264 joint-placement seam through a grouped compiled-IR action and the existing public relocation helper. The implementation keeps EX6-006's separate effects unchanged; its empty-deck deletion case remains a negative control. Other producers, timing interactions, and the complete card/collection audit remain open.

Serialized gate checkpoint: shared build and shared/API/web typechecks passed. Set-scoped effect synchronization reported 112 BT13 records synchronized; semantic comparison found exactly one changed record (`BT13-007`) and no out-of-set changes, and the subsequent BT13 parity check passed. Targeted behavior focus passed **12 files, 49 tests** across King Drasil, Digi-Egg placement, Mother Eater, EX6-006, Bagra, Ascension, trigger ordering, source placement, breeding replacement, DigiXros replacement, and replacement recomputation. The single authorized full API run passed **5,117 files, 42,402 tests**; it emitted existing tournament-deadline and unsupported legacy `AD1-002` combat log errors without test failures. Scoped Oxlint, Oxfmt, `git diff --check`, and `pnpm audit:index --check` passed. Repository-wide tests and release gates remain pending coordinator scheduling.
