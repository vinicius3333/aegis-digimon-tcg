...
- The positive hand Main case now uses a real `activateEffect` intent selected from `activatableEffectsJson`, proving the public declaration path before asserting exact Ludomon payment, bottom-stack placement, memory delta, and host DP. The no-host case still verifies that no Main effect is exposed.
- Status: behavioral verification passed in the final 404/404 collection gate.
