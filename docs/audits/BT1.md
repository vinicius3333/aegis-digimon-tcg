...
- 2026-09-23 re-run: `vitest run src/cards/BT1/` passed 830/830 tests (one fork, 2 GB heap) on `main` at `8058c2b32` plus the `effect.ts` fix to the up-front cost pass from `fc06f73b9`. The 115 ledger rows that still read 8/10 with 0 gate points are historical and were not rewritten. The current evidence supports the 10/10 aggregate.
