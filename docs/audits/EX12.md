...
covered by a direct mechanism test. The focused suite used the 2 GB Node cap
and one worker.
...
public opponent source-trash producer is still absent; the prevention clause
continues to rely on the existing direct mechanism proof. Focused suite:
**9/9** with the 2 GB Node cap and one worker.
