...
September 26, 2026 re-audit: a legal five-source evolution publicly deletes one target and applies the scaled −13000 DP choice. A same-turn public attack cannot use the shared once-per-turn effect again; after the opposing turn expires the debuff and resets that budget, a next-own-turn public attack deletes another target and applies the scaled choice again. The attacks and two option decisions are observed through the turn loop. Q6795's ten-source repeated-choice boundary remains in the injected timing test; a public attempt yielded two choices but no observable DP result and was not retained. Focused result: **11/11** under a 2 GB Node heap and one worker; scoped lint/format passed. Full EX1–EX12 delivery remains open (current 8/10).
