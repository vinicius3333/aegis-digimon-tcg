...
September 26, 2026 re-audit: a public attack now activates the inherited
trash evolution. With two pre-seeded same-level sources, the exact BT1-044
card leaves trash and becomes the top card for a reduced one-memory cost;
with a legal distinct-level Lv2 → Lv3 → Lv4 → Lv5 stack, a public attack
leaves that card in trash and memory unchanged. This supersedes the injected
attack positive/negative below. The same-level pair's creation is not yet
proved through public placement and remains a stack-provenance limit.
