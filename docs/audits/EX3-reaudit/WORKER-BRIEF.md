# EX3 re-audit worker brief (card-only lane)

Repository: `/Users/viniciusluiz/orca/workspaces/aegis-digimon-tcg/audit-ex3-luna-20260909`. Run everything from it. English only.
...
Use neighboring EX3 tests as exemplars. Behavioral proof uses public intents, full settlement, and observable game state. Injected timing (`advance.fire`, `fireSubTrigger`, `fireTiming`) earns no behavioral credit.
...
- Card behavior must be compiled IR registered only with `registerIrCard`; never introduce or preserve a duplicate `registerCard` registration.
- If an engine gap prevents fidelity, retain a named failing/skipped test and document the exact seam. Do not edit engine/shared/catalog files or weaken the expected behavior.

## Allowed edits and commands

Only the assigned `<ID>.ts`, `<ID>.test.ts`, and `docs/audits/EX3-reaudit/<ID>.md`. Never edit another card, engine/shared/catalog, ledger, RUN, REVIEW-NOTES, KB-INDEX, or this brief. Never run git writes.

Run:

```bash
pnpm --filter @aegis/api exec vitest run src/cards/EX3/<ID>.test.ts --maxWorkers=1 --no-file-parallelism
pnpm --filter @aegis/api typecheck
pnpm exec oxlint apps/api/src/cards/EX3/<ID>.ts apps/api/src/cards/EX3/<ID>.test.ts
pnpm exec oxfmt --check apps/api/src/cards/EX3/<ID>.ts apps/api/src/cards/EX3/<ID>.test.ts docs/audits/EX3-reaudit/<ID>.md
git diff --check
```
