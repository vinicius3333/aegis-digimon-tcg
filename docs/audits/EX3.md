...
evidence_commit: 08dba44700b5fc73e8dfbd9e5d43d24006721c56
---
...
A later catalog-wide Digi-Egg fixture scan found three EX3-037 evolution
tests seeding level-2 cards as bare battle-area Digimon. Those sources now
start in breeding, with the same public evolution and stack assertions.
EX3-062's six trash-threshold fixtures now use ordinary main-deck Digimon
instead of eleven preseeded Digi-Egg cards. The two focused suites pass
**2 files / 20 tests** under the 2 GB cap and one worker; scoped lint and
format checks pass. No executable IR or catalog record changed.
...
## Re-audit restart checkpoint — 2026-09-13

Branch `audit-EX3-20260913`, restart base `59825151d658436e70c3b9749612a3f491a089ad`. Three Luna read-only review lanes inspected EX3-001–025, EX3-026–050 and EX3-051–074 against the current catalog, KB queries, direct modules and existing tests. Existing proof is retained wherever sufficient; new proof is restricted to concrete defects.

- Current source sweep: all 74 modules exclusively register compiled IR, with zero `registerCard` and zero `@ts-nocheck`; no removal was necessary at this base.
- Restart collection: `TEST_HEAP_MB=2048 pnpm --filter @aegis/api exec vitest run src/cards/EX3 --maxWorkers=1 --no-file-parallelism` passed 75 files / 765 tests. This is a baseline, before the EX3-012 correction, and is not final certification.
- EX3-012 reopened: the catalog places its When Attacking security-trash clause in main text and has no inherited text. The old module and positive fixture incorrectly treated it as inherited. The historical EX3-012 clause mapping and completion claim below are superseded for this clause. Correction and final behavioral gates are pending.
- EX3-035 current source review confirms three pairwise distinct exact-name payment filters and existing public accept/refusal/insufficient-payment/player-order proofs. General hypothetical compound-filter overlap remains a separate engine obligation; the current exact-name consumer has disjoint candidates. Whole-card certification still awaits the closing gates.
- Only the coordinator runs tests, with one worker and a 2 GB heap ceiling. Dependency installation reused the offline lockfile cache.
- Collection/mechanism regression: `TEST_HEAP_MB=2048 pnpm --filter @aegis/api exec vitest run src/cards/EX3 src/engine/conformance src/engine/combat src/engine/effects src/engine/cards src/engine/simultaneousPlayEventCollapse.test.ts src/cards/audit-docs.test.ts --maxWorkers=1 --no-file-parallelism` passed 268 files / 3161 tests. The EX3-012 correction was finalized during this run; a final post-sync focused and collection run is still required.
- Independent Luna review of EX3-012 found no behavioral blocker: current top-card trigger, own-Tamer gate, opponent security, event ordering and source exclusion match catalog and interpreter semantics.
- Static gates: EX3 Oxlint, changed-module/test Oxfmt and `git diff --check` passed. The ledger was formatted separately.
- Effects check and sync attempts both hit the tool's fixed 120000 ms API build timeout under machine load. These are failed gates, not green synchronization evidence. A separate uncapped API build is in progress before retry; no production tool timeout was changed.

### September 13 recalculation by card (historical for this worktree)

Catalog/rules, IR trace, behavioral proof and comparative/stack proof were re-inspected in the September 13 Luna lanes. Each dimension scores 0–2; the last dimension is delivery, awarded after closing gates, the implementation commit and branch push. The detailed printed contracts, clause maps and KB ambiguities in the historical sections below remain the source inventory, except EX3-012's main/inherited classification superseded above. The table identifies comparative and negative proof titles in the colocated test files; final suite counts are recorded separately under closing gates. Its 10/10 rows belong to the September 13 completed audit, not the pending September 26 branch delivery.

| Card    | September 13 rubric (catalog / IR / behavior / stack / delivery) | Representative existing public proof                                                                                                                                                                                              |
| ------- | ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| EX3-001 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-001.test.ts`: is once per turn and lets two inherited copies trigger independently; does not activate for carriers outside the errata name gate                                                                              |
| EX3-002 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-002.test.ts`: retains the inherited source through a public evolution and breeding move; does not let the carrier itself or a non-Digimon satisfy the other D-Brigade requirement                                            |
| EX3-003 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-003.test.ts`: reveals 3 on attack, adds a Dragon-trait Digimon and bottoms the rest; matches the official errata identity and text                                                                                           |
| EX3-004 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-004.test.ts`: retains the inherited source through a public evolution and breeding move; may decline without trashing or drawing                                                                                             |
| EX3-005 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-005.test.ts`: its inherited attack effect is not once per turn; does not trigger for Hina played by the opponent or outside its controller's turn                                                                            |
| EX3-006 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-006.test.ts`: encodes Q3371's Dragonkin inclusion as a trait-only once-per-turn gate; does not draw for a carrier outside the Dragon/saur/Ceratopsian family                                                                 |
| EX3-007 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-007.test.ts`: retains the inherited source through public evolution and attack resolution; does not activate for a carrier without an On Play effect                                                                         |
| EX3-008 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-008.test.ts`: Q3374: may use its inherited end-of-turn DNA after its digivolving DNA passes memory; does not offer either DNA effect when hand cards have no printed DNA requirement                                         |
| EX3-009 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-009.test.ts`: keeps the saur clause trait-only; does not draw for a carrier outside the Dragon/saur/Ceratopsian family                                                                                                       |
| EX3-010 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-010.test.ts`: does not grant inherited Security Attack +1 during the opponent's turn; rejects a DNA evolution without both a red and purple level 4                                                                          |
| EX3-011 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-011.test.ts`: allows the optional Hina play to be declined without moving it from hand; rejects a blue level 4 source and keeps the card in hand                                                                             |
| EX3-012 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-012.test.ts`: Earth Dragon trait: Hina reactivates its On Play effect after digivolution; rejects a blue level 5 source and keeps Volcanicdramon in hand                                                                     |
| EX3-013 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-013.test.ts`: Cyborg trait: places up to 3 unique red/black level 5 Cyborgs from hand and trash, then De-Digivolves once per card; does not De-Digivolve when the placement cost is declined                                 |
| EX3-014 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-014.test.ts`: DigiXroses five differently named Dragon-family cards, scales deletion, and attacks with Rush; rejects two DigiXros materials with the same printed name                                                       |
| EX3-015 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-015.test.ts`: Q3378/Aqua trait: when played from digivolution cards, places the hand card under the Digimon that received Jamming; allows the optional placement to be declined without moving the eligible hand card        |
| EX3-016 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-016.test.ts`: Q3379 stacks two inherited copies for a total increase of 2; does not increase digivolution costs during its controller's turn                                                                                 |
| EX3-017 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-017.test.ts`: Q3385/Aquatic family: playing it from sources unsuspends the exact Digimon that gained Blocker; has its official identity and printed blue evolution cost                                                      |
| EX3-018 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-018.test.ts`: Dragon family: inherited Evade applies under Dramon/Examon names and not an unrelated host; rejects a non-Dracomon evolution source and keeps the card in hand                                                 |
| EX3-019 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-019.test.ts`: chooses a source-bearing opponent Digimon and then any exact card in its stack; rejects a red level 3 and a blue level 4 as evolution sources                                                                  |
| EX3-020 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-020.test.ts`: Sky Dragon family: treats Wingdramon as blue level 6 only for Examon and completes end-turn DNA; rejects the alternate route from a non-Coredramon source and keeps Wingdramon in hand                         |
| EX3-021 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-021.test.ts`: Dragonkin family: does as much as possible with 1 source, then may restrict that now-empty Digimon; rejects a level-5 source and keeps CrysPaledramon in hand                                                  |
| EX3-022 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-022.test.ts`: Aquatic family: offers only blue level 3 sources under blue Digimon and plays the chosen card for free; allows the optional play to be declined with one prompt and leaves the source in place                 |
| EX3-023 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-023.test.ts`: Plesiosaur family: supports both errata play branches and places a chosen blue hand card at its own bottom; rejects an invalid level-5 evolution source and keeps Plesiomon in hand                            |
| EX3-024 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-024.test.ts`: rejects an unrelated level-5 as the alternate evolution source; rejects an unrelated level-5 as the alternate evolution source                                                                                 |
| EX3-025 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-025.test.ts`: publishes no deletion decision without Trial in hand and resolves Draw 2 against an empty deck; rejects a red level 3 and a blue level 4 as evolution sources                                                  |
| EX3-026 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-026.test.ts`: Aqua/Sea Animal family: offers every errata OR branch only from a blue Digimon's sources; rejects a non-level-6 evolution source and keeps Aegisdramon in hand                                                 |
| EX3-027 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-027.test.ts`: does not draw from the opponent's card, an unrelated Digimon, the opponent's turn, or Agumon on top; rejects an invalid red level-3 evolution source                                                           |
| EX3-028 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-028.test.ts`: Four Great Dragons/Angel family: forces both independent adds, exposes the full reveal, and orders the rest; rejects non-yellow Cherub and yellow Three Great Angels while still returning every revealed card |
| EX3-029 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-029.test.ts`: Mythical Beast family: a yellow multicolor card found in security still enables Recovery; rejects a red level-3 evolution source                                                                               |
| EX3-030 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-030.test.ts`: Angel/Four Great Dragons family: the errata search adds both mandatory categories; rejects a non-yellow level 3 evolution source                                                                               |
| EX3-031 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-031.test.ts`: Dramon/Four Great Dragons family: overlapping cards fill only one mandatory search slot; rejects a non-yellow level 3 evolution source                                                                         |
| EX3-032 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-032.test.ts`: Deva/Four Sovereigns family: targets exactly 1 opponent Digimon and gains 2 memory; does not gain memory when Four Sovereigns exists only in a digivolution stack                                              |
| EX3-033 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-033.test.ts`: Four Great Dragons family: places Trial from hand without playing or activating its Main effect; rejects a red level-3 evolution source                                                                        |
| EX3-034 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-034.test.ts`: pays the yellow level-4 evolution cost, preserves the stack, and rejects an invalid source; honors the optional errata and leaves Trial in hand when declined                                                  |
| EX3-035 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-035.test.ts`: Q2613/Four Great Dragons family: returns Trial or a Digimon after one optional confirmation; does not pay with Magnadramon X Antibody when exact Magnadramon is absent                                         |
| EX3-036 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-036.test.ts`: Four Great Dragons family: gives every opponent Digimon Security Attack -1 on a non-Trial play; does not offer deletion placement without a Trial in hand                                                      |
| EX3-037 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-037.test.ts`: Dramon/Examon inherited family: buffs only its host once per turn when an ally suspends; rejects a non-level-2 source without changing memory or stack                                                         |
| EX3-038 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-038.test.ts`: Vegetation family: does not trigger when an effect suspends another Vegetation Digimon; rejects a non-green level 2 evolution source                                                                           |
| EX3-039 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-039.test.ts`: Dragon family: inherited Blocker applies to Dramon and Examon names but not an unrelated host; rejects a non-green or blue level 3 evolution source                                                            |
| EX3-040 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-040.test.ts`: Dinosaur family: inherited effect observes an effect suspending another own Digimon; rejects a non-green level 3 evolution source                                                                              |
| EX3-041 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-041.test.ts`: rejects an unrelated level-3 source without changing memory or stack; rejects an unrelated level-3 source without changing memory or stack                                                                     |
| EX3-042 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-042.test.ts`: pays the green level-4 evolution cost, preserves the stack, and rejects an invalid source; does not trigger its When Digivolving effect from an unsuspended base                                               |
| EX3-043 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-043.test.ts`: Vegetation family: pays Digisorption with the only legal ally, reduces cost by 3, then unsuspends at 2; rejects a non-green level 4 evolution source                                                           |
| EX3-044 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-044.test.ts`: Machine Dragon trait: surviving a battle won by Breakdramon trashes the opponent's top security; does not trash security when a Dramon loses the battle and fails to survive                                   |
| EX3-045 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-045.test.ts`: rejects a non-green level-5 evolution source without changing the stack or memory; rejects a non-green level-5 evolution source without changing the stack or memory                                           |
| EX3-046 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-046.test.ts`: does not protect an unrelated trait, itself, a battle loss, or its controller's own effect; can decline Decoy through the public decision and lets the protected Digimon be deleted                            |
| EX3-047 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-047.test.ts`: Bird Dragon family: its inherited effect gives +1000 DP to Jazardmon, which has an On Play effect; does not trigger from the opponent's Hina play                                                              |
| EX3-048 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-048.test.ts`: Machine/Bird Dragon family: searches Jazamon and Hina, then Hina's play triggers Jazamon; rejects a non-black/non-red level-4 evolution source without payment or movement                                     |
| EX3-049 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-049.test.ts`: D-Brigade family: grants Rush to the newly played Commandramon and permits its immediate attack; does not protect itself in a battle against an opposing non-Security Digimon                                  |
| EX3-050 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-050.test.ts`: stacks two independent inherited Cyberdramon sources; rejects an invalid level-3 source without payment or movement                                                                                            |
| EX3-051 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-051.test.ts`: D-Brigade family: an allied D-Brigade attack reveals 2, plays the chosen Commandramon, and trashes the other card; When Digivolving may decline the eligible play and trashes all 3 revealed cards             |
| EX3-052 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-052.test.ts`: On Play chooses 1 opposing stack, De-Digivolves exactly 1, then may play Hina for free; rejects an invalid level-3 evolution source without payment or movement                                                |
| EX3-053 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-053.test.ts`: On Play De-Digivolves every opposing stack, then deletes exactly 1 resulting cost-5 Digimon; does not install the digivolution lock when its Delete action succeeds                                            |
| EX3-054 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-054.test.ts`: D-Brigade family: Tankdramon's inherited attack plays Commandramon, which triggers Darkdramon; does not trigger for a non-D-Brigade play or during the opponent's turn                                         |
| EX3-055 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-055.test.ts`: Free-family battle: a red Dinobeemon carrying Wormmon gains Retaliation and deletes the winner; rejects an invalid level-3 evolution source without movement                                                   |
| EX3-056 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-056.test.ts`: Virus-family Evade prevention counts as no deletion and therefore mills both decks; rejects a non-level-2 source without moving Guilmon or spending memory                                                     |
| EX3-057 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-057.test.ts`: Virus-family attack pays the inherited cost and checks 2 security; can decline the inherited attack effect without deleting another Digimon or gaining Security Attack                                         |
| EX3-058 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-058.test.ts`: leaves both stacks intact when the hand has no compatible DNA result; rejects a non-red/non-purple level 3 source without moving Shadramon or memory                                                           |
| EX3-059 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-059.test.ts`: Dinosaur line: suspends another opponent after its MasterTyrannomon host loses a battle; rejects a wrong-color level-3 evolution source without movement                                                       |
| EX3-060 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-060.test.ts`: Puppet family: gaining a ToyAgumon source immediately restores attack and block permissions; has the official dual-color metadata, Blocker, and source restriction text                                        |
| EX3-061 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-061.test.ts`: On Deletion offers Wormmon from both the deleted stack and pre-existing trash, but no other Larva; rejects DNA evolution without purple and red level-4 materials                                              |
| EX3-062 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-062.test.ts`: mills as many as available from short decks and still evaluates the live threshold; rejects a red level 4 whose name does not contain Growlmon                                                                 |
| EX3-063 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-063.test.ts`: without Fighter Mode in hand, the mandatory DP bonus resolves with no impossible prompt; rejects DNA evolution without purple and red level-5 materials                                                        |
| EX3-064 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-064.test.ts`: Four Great Dragons family: Trial's real Delay gives Megidramon level-6 provenance; honors the errata's optional decline                                                                                        |
| EX3-065 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-065.test.ts`: family: Earth Dragon digivolution suspends Hina and genuinely reactivates Volcanicdramon's On Play; does not gain memory without an opposing battle-area Digimon, from breeding, or on the opponent's turn     |
| EX3-066 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-066.test.ts`: Machine family: pays with a hand Cyborg under the level 6 Machine, then deletes only 6000 DP or less; does not waive colors for no Machine, a level 5 Machine, breeding, or the opponent's Machine             |
| EX3-067 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-067.test.ts`: blue source-control family: strips all available sources and locks every source-less opposing Digimon; does not satisfy its blue color requirement without an own blue Digimon or Tamer                        |
| EX3-068 | 2 / 2 / 2 / 2 / 2 = **10/10**                                    | `EX3-068.test.ts`: Four Great Dragons family: offers Digimon and Trial, returns exactly one, and excludes unrelated trash; honors the errata's may and leaves the Four Great Dragons card in trash when declined                  |
...
- `pnpm typecheck` passed for shared, API and web after correcting the new test's possibly-undefined IR lookup. The first typecheck failure was card-local and is resolved.
- `TEST_HEAP_MB=2048 pnpm --filter @aegis/api exec vitest run src/cards/EX3/EX3-012.test.ts src/cards/EX3/EX3-035.test.ts --maxWorkers=1 --no-file-parallelism` passed 2 files / 28 tests, including EX3-012's main attack positive, buried-source negative and missing-Tamer negative, plus EX3-035's complete exact-name cost and Q2613 proof.
- Final post-sync `TEST_HEAP_MB=2048 pnpm --filter @aegis/api exec vitest run src/cards/EX3 src/engine/conformance src/engine/combat src/engine/effects src/engine/cards src/engine/simultaneousPlayEventCollapse.test.ts src/cards/audit-docs.test.ts --maxWorkers=1 --no-file-parallelism` passed **268 files / 3161 tests** with no failing, skipped or expected-failure tests. The run emitted an AD1-002 unsupported-effect diagnostic without any failing test.
- `pnpm exec oxlint apps/api/src/cards/EX3` passed; changed-module/test/effects Oxfmt passed. Documentation formatting, generated index and diff checks passed at closing delivery.
- Current source inventory matches all 74 catalog IDs to modules and colocated tests: complete compiled IR, one exclusive registration each, no `registerCard`, no `@ts-nocheck`, and no skipped/expected-failure/injected-timing tests.
- Independent Luna review of EX3-012 confirmed correct top-card timing, own-Tamer condition, opposing security destination, event order and neutral fixtures. The only new behavioral case protects the concrete inherited-source defect; adequate existing proof for other cards was retained.
...
Current score: **10/10** under the 2026-09-13 recalculation and closing gates. Historical worker rubrics, deferred gates and earlier command counts below describe the pre-closeout audit and are superseded by those current results.
...
- Main effect (no inherited text): `[When Attacking] If you have a Tamer in play, trash the top card of your opponent's security stack.`
...
| Main When Attacking security trash                                               | `EX3-012.ts`, main `WhenAttacking` with no `isInherited`, Tamer-gated `SecurityManipulation`                                                             | `trashes one security before its normal check when attacking with a Tamer` uses EX3-012 on top and checks event order; `does not trigger the security clause from an inherited EX3-012 source` proves source exclusion; `does not trash extra security when attacking without a Tamer` proves the condition. |
...
- 2026-09-13 correction: removed the erroneous `isInherited: true`. The earlier audit added that flag based on a mistaken inherited classification; its fix and source-stack claim were incorrect. The printed effect executes from EX3-012 as the top card, not as a source.
...
Current score: **10/10** under the 2026-09-13 whole-card reinspection and closing gates. The 2026-09-12 exact-name consumer-only checkpoint, historical worker score and pending/deferred verification below are superseded by the current focused pair (28 tests), collection/mechanism regression (3161 tests), full typecheck and effects sync/check.

#### Exact-name reopening, 2026-09-12

Baseline `2e0811eea`. Current catalog and the committed 2022-11-11 errata require one exact Magnadramon, Azulongmon and Megidramon for the attack's conditional return cost. Comprehensive §2-3-1-2 (`comprehensive-0034`, reviewed fingerprint `c0ee1524e24827189e2dcfae2543a217540028723a55d660c84d63e4f29505f2`) separates bracket-only exact names from explicit substring wording. `node tools/kb/query.mjs card EX3-035` returned the errata and Q2613; the [official Q&A](https://world.digimoncard.com/rule/?card_no=EX3-035), checked 2026-09-12, retains Q2613's simultaneous effect-order contract rather than an exception to name matching.

Added a public attack with only Magnadramon X Antibody, Azulongmon and Megidramon in trash. The first run had a fixture milestone typo (`suspended` instead of the schema's `isSuspended`); after replacing that milestone with the resolved public security result, the test failed because the opponent's four Security cards fell to one rather than three. This isolates two improperly discarded Security cards beyond the ordinary attack check. The strengthened existing positive/order case also failed: its first payment candidate list included Magnadramon X Antibody alongside the two exact copies. Red result: two failed / eleven passed.

All three bracket-only cost filters now use `nameExact`; the unchanged proof then passed 13/13. The refusal case retains all three physical payment cards in trash and leaves deck order unchanged while mandatory -6000 DP and the ordinary Security check resolve. The positive case exposes the near-named card for trash visibility but excludes it from payment candidates, chooses only one duplicate exact Magnadramon, preserves both unselected cards in trash and returns the three distinct paid instances in the player's chosen bottom order. All focused tests pin the reviewed source fingerprint. Azulongmon and Megidramon have no distinct near-named catalog variants; their existing candidate lists isolate their exact named instances from the other visible trash cards.

At the 2026-09-12 checkpoint this was an exact-name consumer correction, not a whole-card or whole-set closeout. The 2026-09-13 reinspection and closing gates now certify the whole card and set. Existing full-coverage IR metadata and the historical rubric below do not confer current 10/10. General compound candidate overlap and selection uniqueness remain separate engine obligations. Delivery gates are recorded in `docs/audits/engine/activation-costs.md`.
...
#### Historical implementation audit and changes
...
registration. The earlier audit reported its errata optionality, exact filters,
compound cost, ordering, abort behavior, DP target, duration, and security endpoint
as faithful. That exact-filter claim is contradicted by the 2026-09-12 reproduction above.
...
- `pnpm --filter @aegis/api exec vitest run src/cards/EX3/EX3-065.test.ts --maxWorkers=1 --no-file-parallelism` — **PASS** (18/18; the immunity scenario fails with the former `asEffectOf: "this Tamer"` clause and passes without it).
...
The September 13 recalculation certified all 74 cards at 10/10, and the September 26 source review found no new card-local behavior or peer/stack gap. Current branch delivery scores have not been awarded; at most the technical 8/8 is supported for each card until the branch closeout. EX3-012 and EX3-035 reopenings are closed. The following historical and KB items remain worth knowing.
