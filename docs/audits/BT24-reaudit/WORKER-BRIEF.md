...
- A named draw card must begin in the deck, not already in hand. Adding an initial-hand alias to satisfy a missing draw assertion is a false proof;078 Q5775 exposed this trap. Check fixture origin and destination together. For timing, compare independently identified `cardsMoved` events and assert both indices exist before comparing their order.
- Legal memory is -10 through10. For play costs above10, start at10 and assert the resulting negative memory. Voluntarily ending Main passes memory to the opponent (normally -3); do not interpret that turn-pass change as a free evolution cost.
- Effect-driven Link placement does not prove final retention: CR17-1-3-2-6/7 trashes a link whose recipient fails its printed requirement at rule-check timing. Root's077 fixture incorrectly used non-Appmon Titamon as a durable036 recipient. Use Appmon BT21-023 Globemon10000 on the opponent turn to isolate0779000 Happy Bullet deletion; Globemon's own WhenLinked watcher is YourTurn-only. Trace movement through the final rule check before alleging a Link engine gap.

- `preferTriggerKeys` matches only substrings of `triggerKeys`, not the corresponding `triggerCardIds`. System-B keys use `subtrigger/<id>/<description>` and may not contain the card ID. Assert the offered card metadata and select its corresponding key, or verify a stable unique discriminator. Never hardcode the subscription number, and never classify a fallback order caused by an unmatched preference as an engine defect.
