...
The EX2-041–050 review added public optional refusal on EX2-042, the scaled
When Attacking path on EX2-044, and a Q3337 opponent-hand choice regression on
EX2-043. The regression exposed a shared `HandManipulation(untilFive)` chooser
bug; the engine now asks each hand's owner. The full range passes **10 files /
64 tests** under a 2 GB heap and one worker. The full-collection count above
predates these additions.

The current post-fix EX2 collection passes **85 files / 499 tests**. Combined
...
| `[When Attacking][Once Per Turn] You may trash 1 card in your hand to gain 1 memory.` (inherited) | Inherited `WhenAttacking`, `frequency: "OncePerTurn"`, optional `GainMemory` amount 1 with one-card hand-trash cost | A public EX2-040→EX2-042→BT3-089 evolution stack activates the inherited effect on the higher host, verifies one hand card trashed and +1 memory, refuses a second same-turn activation, and verifies activation after the real next-own-turn reset. A separate public optional-decline case preserves the card in hand and gains no memory. |
...
- Removed `@ts-nocheck` and exported the typed `CompiledCard` IR for direct verification.
- Preserved exclusive compiled registration through `registerIrCard`.
- Replaced the fixture-only tests with catalog/IR assertions, exact On Play draw/trash endpoints, legal and illegal evolution proofs, and public evolved-host inherited-effect tests covering acceptance, refusal, same-turn once-per-turn suppression, and next-own-turn reset.

#### Verification status

The earlier coordinator verification passed 4/4 before the optional-decline regression was added. The September 26 EX2-041–050 range run passes **10 files / 64 tests**, including EX2-042's **5/5**. The reset fixture gives both seats a 24-card inert main deck for real-turn draws. Rubric remains catalog/rules 2/2, IR trace 2/2, behavioral proof 2/2, peer/evolution-stack proof 2/2; total **8/10** with final cross-EX delivery open.
...
| Q3337: each player decides their own discarded cards                                                                    | `HandManipulation(untilFive)` loops over both players and routes each decision to that hand's owner via `ctx.ask` or `requireOpponentAsk(ctx)` | The public regression isolates a five-card controller hand and six-card opponent hand, observes the opponent's `selectCards` request at seat 1, then has that player choose the exact card discarded.                                                                                                                |
...
- Replaced internal-context and injected-subtrigger tests with catalog/IR assertions, public Q3338/Q3339 hand-trim proofs, and a retained red public Q3337 chooser-ownership regression.
...
The first Q3337 public run confirmed a real chooser bug: the opponent's discard request was addressed to seat 0. `HandManipulation(untilFive)` now uses `requireOpponentAsk(ctx)` for the opponent's hand. The corrected EX2-043 focused suite passes **7/7** and the EX2-041–050 range passes **10 files / 64 tests** with a 2 GB heap and one Vitest worker. Rubric: catalog/rules 2/2, IR trace 2/2, behavioral proof 2/2, peer/evolution-stack proof 2/2; total **8/10** with final cross-EX delivery still open.
...
| `[When Digivolving]` and `[When Attacking]` may trash the top 2 cards                                          | Both triggers contain optional `TrashTopDeck` amount 2 with `abortOnDecline`                                                                | Public legal evolution exercises the When Digivolving mill; a new public attack case accepts the optional mill, reaches the 10-card trash threshold, and deletes a level-4 target. A separate attack declines and leaves the deck unchanged. |
...
- Expanded the colocated tests with catalog/IR assertions, exact 10-versus-9
  trash ceiling boundaries, positive When Attacking scaling, public direct-trash
  Impmon play, Q3340 reveal negative, optional attack refusal, legal paid
  evolution stack/draw proof, and invalid-source rejection.
- Sanitized all deck and security fixtures with inert ordinary Digimon.

#### Validation and score

The earlier coordinator run passed 7/7 after the declined-attack proof's exact
deck sequence was corrected. The September 26 EX2-041–050 range run passes
**10 files / 64 tests**, including EX2-044's **8/8** and the positive scaled
When Attacking case. No card or engine behavior change was needed for EX2-044.
...
The September 26 reviews closed EX2-012 Q3300's mixed-target choice gap and EX2-043 Q3337's opponent-hand chooser gap. Cross-EX delivery is still open. The following additional items remain worth knowing.
...
- Engine correction — EX2-043 Q3337. Gulfmon's `HandManipulation(untilFive)` formerly passed the effect controller's decision API for both hands. It now asks each hand's owner; the public opponent-seat decision and exact chosen discard pass.
