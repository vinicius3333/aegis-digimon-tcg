...
# EX11 re-audit worker brief (card-only lane)
...
- Rulings: `node tools/kb/query.mjs card <ID>` and
  ...
  is authoritative. `docs/audits/EX11-reaudit/KB-INDEX.md` lists the Q&A ids
  you must cover.
- Module and test: `apps/api/src/cards/EX11/<ID>.ts` and `<ID>.test.ts`.
- Context: `docs/audits/EX11-reaudit/REVIEW-NOTES.md` and `RUN.md` sections
  that mention your card; the mechanism docs for any engine seam named there.
- Exemplar tests in the accepted style: apps/api/src/cards/EX11/EX11-036-045-046.assembly.test.ts, EX11-link-play-rule-check.test.ts, and engine/testkit/harness.ts; inspect real turn-loop examples via rg startTurnLoop or endPhase. Reuse their
...
Only `<ID>.ts`, `<ID>.test.ts` and `docs/audits/EX11-reaudit/<ID>.md`.
...
```bash
pnpm --filter @aegis/api exec vitest run src/cards/EX11/<ID>.test.ts --maxWorkers=1 --no-file-parallelism
pnpm --filter @aegis/api typecheck
pnpm exec oxlint <files>; pnpm exec oxfmt --check <files>; git diff --check
```

Logs go to `docs/audits/EX11-reaudit/logs/<ID>-<what>.log`.
...
Worktree: /Users/viniciusluiz/orca/workspaces/aegis-digimon-tcg/audit-ex11-luna-20260908
Use this absolute workdir for every command; never edit the main checkout. Read AGENTS.md and .agents/skills/verify-card-implementation/SKILL.md first. The user requested Luna. Only one assigned card is authorized per dispatch; wait for coordinator follow-up after reporting. No git writes. Keep progress concise. Prior reports are hypotheses to falsify. Remove misleading handwritten override guidance if touching generated module comments. Card-specific Q&A including derived-trigger order must receive behavioral proof.
