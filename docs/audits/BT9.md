...
The direct module `BT9-083.ts` scales the first delete by the source Digimon's stack cards whose forms include Mega, then performs a separate up-to-ten opponent-trash return to deck bottom with any order. The Start of Your Turn sequence trashes the permanent's visible top card with `topCardOnly: true`, promotes the card directly beneath it, and only then trashes the opponent security top card under `ifThisEffectActed`. §2-3-1, §2-3-2, §4-7, §8-2, §15-5–§15-8, §15-16, and §16-6 support the source identity, stack, timing, conditional continuation, and deck/security ordering. The legal Omnimon alternate evolution was inspected in the direct metadata and shared evolution data.

The focused `BT9-083.test.ts` statically checks the Mega scaling, separate delete/return sequence, bottom-deck behavior, Digi-Egg routing, any-order return, visible-top trash with source promotion, and conditional opponent security trash. The shared `topCardOnly` primitive records a successful action for the following `ifThisEffectActed`, while `returnToDeck` routes Digi-Egg definitions to the egg deck.
...
Correction: the previous IR incorrectly targeted the top digivolution card. It now targets the visible top card and the focused behavioral proof confirms Omnimon is trashed, its source is promoted, and the opponent's top security is then trashed. A Q1885 negative proves that with no card beneath Omnimon, neither it nor the opponent's security is trashed. No ambiguity remains; 8/10 remains provisional because delivery gates are deferred.
