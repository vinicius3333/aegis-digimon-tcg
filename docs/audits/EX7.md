...
| `[Once Per Turn]`                               | The same effect has `frequency: "OncePerTurn"`; registration threads the stable once-per-turn watcher key.                                                                                                        | `draws only once across two legal attacks by the same inherited host` uses a public BT1-036 On Play effect to ready the host between attacks; `resets the inherited draw on the next own turn through the real turn loop` proves reset. |
...
- Same-turn frequency is checked over two legal attacks by the same inherited host. After the first attack, BT1-036 Garurumon is played from hand through `playCard`; its On Play effect unsuspends the host, and the second attack leaves the remaining deck card untouched.
...
- The same-turn negative uses only public `attack` and `playCard` intents; BT1-036 supplies the legal unsuspend between attacks. The next-turn reset remains exercised through the real turn loop.
...
**2026-09-23 [X Antibody] name-gate correction.** A bare `[X Antibody]` in the printed text names the card (BT9-109, or a card with "[Rule] Name: Also treated as [X Antibody]", per Q3679 and Q5907), not the X Antibody trait. The IR matched it by `trait`, so an X Antibody-trait Digimon such as WarGrowlmon (X Antibody) satisfied the clause. It now uses `nameExact`. `EX7-061.test.ts` asserts that BT9-109, EX11-053, and EX5-070 match and BT9-014 and EX8-015 do not. This is a focused correction, not a new collection-completion claim.
