...
- Implementation and primitive trace: `BT13-040.ts` was corrected so `PlayWithoutCost` is nested after `Draw` inside the same self-scoped `wouldLeavePlay` reaction. Previously it was an All Turns sibling and could offer/play Veemon during ordinary continuous recomputation without any leave event. The mode-less reaction installs the shared non-preventing `instead` observer, runs both ordered actions while the source stack still exists, then permits the original removal. The Static record installs Blocker and the alternate requirement names Veemon for 3. Traced through continuous installation without execution, leave consult/cause independence, self scoping, draw movement, optional hand/source candidate selection, source detachment, free permanent creation, original deletion continuation, keyword installation/combat exposure, alternate versus catalog evolution validation, and payment. Coverage is `full`, residual is empty, and registration is exclusively `registerIrCard("BT13-040", compiled)`.
...
- Behavioral proof: isolated `BT13-040.test.ts` passed 7 tests with 237 interpreter/evolution/advanced-keyword mechanism tests (244 total): exact nested full IR; live Blocker exposure; repeated continuous recomputation neither draws nor offers/plays Veemon; deletion draws, plays the exact aliased Veemon from the leaving source stack for free, preserves that Veemon from trash, and still sends Magnamon to trash; declining the play still draws and Magnamon leaves while hand Veemon remains; real Veemon uses the alternate route for exactly 3 while a yellow non-Veemon uses normal evolution for exactly 4; and explicit alternate evolution rejects the non-Veemon. Reverting nesting/ordering, leave/self scope, non-prevention, draw, optionality, origins, name/ownership/payment, Blocker, alternate name/cost, or normal color/level/cost makes focused observable evidence fail.
