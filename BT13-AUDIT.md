...
- Behavioral proof: isolated `BT13-033.test.ts` passed 7 tests with 224 mechanism tests (231 total): exact full IR; zero-cost Burst evolution returns the controller's Thomas and trashes the prior top at end of turn; an opponent-owned Thomas cannot pay the Burst requirement; normal evolution pays 5, bounces the opponent's Digimon, then gains memory from the resulting hand size; an attack against eleven hidden hand cards selects/orders exactly three without exposing definitions, bottoms them, and unsuspends; refusal against ten cards changes neither hand nor suspension; and eight cards do not meet the threshold. Reverting the timing split, normal/alternate evolution requirements, ownership, cleanup, bounce target, post-bounce count, threshold, optionality, dynamic excess count, hidden-information handling, ordering, payment-before-effect, or unsuspend target makes focused observable evidence fail.
