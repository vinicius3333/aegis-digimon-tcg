...
describe("rarity data across the legacy enum boundary", () => {
  it("contains exactly the 24 official SEC cards in the BT1-BT10/EX1-EX2 cutoff", () => {
    const inCutoff = cardList.filter((card) =>
      /^(BT(?:[1-9]|10)|EX[12])$/.test(card.set) && card.rarity === "SEC"
    );

    expect(inCutoff.map((card) => card.cardId).sort()).toEqual(
      [...HISTORICAL_SECRET_RARES].sort(),
    );
    expect(cardList.filter((card) =>
      /^(BT(?:[1-9]|10)|EX[12])$/.test(card.set) && card.rarity === "UR"
    )).toHaveLength(0);
  });
