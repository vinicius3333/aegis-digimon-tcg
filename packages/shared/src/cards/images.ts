...
export function cardImageUrls(cardId: string | undefined, artId?: string): string[] {
  if (!cardId) return [];
  const selectedId = resolveCardArt(cardId, artId).imageId.replace(/-Errata$/, "");
  const baseId = resolveCardArt(cardId).imageId.replace(/-Errata$/, "");
  const ids = selectedId === baseId ? [baseId] : [selectedId, baseId];
